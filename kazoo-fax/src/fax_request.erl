...
-define(POLL_INTERVAL, 5 * ?MILLISECONDS_IN_SECOND).
...
handle_cast({'retry_store_document', Error}, #state{monitor={_, Ref}, fax_store_count=?MAX_STORE_COUNT}=State) ->
    erlang:demonitor(Ref),
    system_report(State
                 ,<<"Store Fax Document Failed">>
                 ,kz_binary:format("maximum retry to save fax document: ~p", [Error])
                 ),
    lager:error("reached to maximum store document retry, going down"),
    {'stop', 'normal', State};
handle_cast({'retry_store_document', _Error}, #state{monitor={_, Ref}, fax_store_count=StoreCount}=State) ->
    erlang:demonitor(Ref),
    {'noreply', State#state{fax_store_count=StoreCount + 1
                           ,status = <<"storing document">>
                           }, ?POLL_INTERVAL};
handle_cast({'retry_store_attachment', 'file_not_found' = Error}, #state{monitor={_, Ref}}=State) ->
    erlang:demonitor(Ref),
    system_report(State
                 ,<<"Store Fax Attachment Failed">>
                 ,kz_binary:format("media server failed to save the attachment: ~p", [Error])
                 ),
    lager:error("media server failed to save the attachment, going down: ~p", [Error]),
    {'stop', 'normal', State};
handle_cast({'retry_store_attachment', Error}, #state{monitor={_, Ref}, fax_store_count=?MAX_STORE_COUNT}=State) ->
    erlang:demonitor(Ref),
    system_report(State
                 ,<<"Store Fax Attachment Failed">>
                 ,kz_binary:format("maximum retry to save fax attachment: ~p", [Error])
                 ),
    lager:error("reached to maximum store attachment retry, going down"),
    {'stop', 'normal', State};
handle_cast({'retry_store_attachment', _Error}, #state{monitor={_, Ref}, fax_store_count=StoreCount}=State) ->
    erlang:demonitor(Ref),
    {'noreply', State#state{fax_store_count=StoreCount + 1}, ?POLL_INTERVAL};
handle_cast({'store_attachment', FaxDoc}, #state{monitor={_, Ref}}=State) ->
    erlang:demonitor(Ref),
    {'noreply', State#state{fax_doc=FaxDoc
                           ,status = <<"storing attachment">>
                           }, ?POLL_INTERVAL};
...
handle_event(_JObj, State) ->
    {'reply', [], State#state{hangup = false}}.
...
terminate(_Reason, #state{call=Call, hangup = true}) ->
...
-spec maybe_update_fax_settings_from_account(state()) -> state().
...
-spec update_fax_settings(kapps_call:call(), kz_json:object()) -> 'ok'.
update_fax_settings(Call, JObj) ->
...
build_fax_settings(Call, JObj) ->
    [{<<"Fax-Identity-Number">>, fax_identity_number(Call, JObj)}
    ,{<<"Callee-ID-Number">>, callee_id_number(Call, JObj)}
...
callee_id_number(Call, JObj) ->
    case kz_json:is_true(<<"override_callee_number">>, JObj, 'false') of
        'false' -> kapps_call:to_user(Call);
        'true' -> overridden_callee_id(Call, JObj)
    end.

fax_identity_number(Call, JObj) ->
    case kz_json:is_true(<<"override_fax_identity">>, JObj, 'true') of
        'false' -> kapps_call:to_user(Call);
        'true' -> overridden_fax_identity(Call, JObj)
    end.
...
end_receive_fax(JObj, State) ->
    case kz_json:is_true([<<"Application-Data">>,<<"Fax-Success">>], JObj, 'false') of
        ...
        'false' ->
            maybe_partially_received(State#state{fax_result=JObj})
    end.

-spec end_receive_fax(state()) -> handle_cast_return().
end_receive_fax(#state{page=0, fax_result=JObj}=State) ->
    lager:info("fax status - receive fax failed with 0 pages"),
...
end_receive_fax(#state{page=Page}=State) ->
    lager:info("fax status - successfully received fax with ~p pages", [Page]),
    {'noreply', State#state{monitor=store_document(State)}}.

-spec maybe_partially_received(state()) -> handle_cast_return().
maybe_partially_received(#state{page=0, fax_result=JObj}=State) ->
    lager:info("fax status - receive fax failed"),
    notify_failure(JObj, State),
    {'stop', 'normal', State};
maybe_partially_received(#state{page=Page, fax_result=JObj}=State) ->
    lager:info("fax status - partially received fax with ~p pages",[Page]),
    Props = [{[<<"Application-Data">>,<<"Fax-Success">>], 'true'}
            ,{[<<"fax_info">>, <<"fax_result_text">>], <<"partial pages received">>}
            ],
    NewJObj = kz_json:set_values(Props, JObj),
    {'noreply', State#state{fax_result=NewJObj, monitor=store_document(State)}}.
...
store_document(Pid, #state{fax_result=JObj
    ...
    case create_fax_doc(JObj, State) of
        ...
        {'error', Error} ->
            ...
            gen_server:cast(Pid, {'retry_store_document', Error})
    end.
        ...
        {'error', StoreError} ->
            case check_fax_attachment(FaxDb, FaxId, AttachmentId) of
                ...
                {'missing', _} ->
                    ...
                    gen_server:cast(Pid, {'retry_store_attachment', StoreError});
                {'error', Error} ->
                    lager:debug("error '~p' saving fax attachment on fax id ~s",[Error, FaxId]),
                    timer:sleep(?RETRY_SAVE_ATTACHMENT_DELAY),
                    gen_server:cast(Pid, {'retry_store_attachment', StoreError})
            end
    end.
...
notify_failure(JObj, State) ->
    Reason = kz_json:get_ne_binary_value([<<"Application-Data">>, <<"Fax-Result-Text">>], JObj, <<"unknown error">>),
...
-spec system_report(state(), kz_term:ne_binary(), kz_term:ne_binary()) -> 'ok'.
system_report(#state{call=Call
                    ,owner_id=OwnerId
                    ,faxbox_id=FaxBoxId
                    ,fax_notify=FaxNotify
                    ,account_id=AccountId
                    ,fax_result=JObj
                    ,storage=#fax_storage{id=FaxId, db=FaxDb}
                    }
             ,Subject
             ,Message) ->
    Details = props:filter_undefined(
                [{<<"Fax-ID">>, FaxId}
                ,{<<"Owner-ID">>, OwnerId}
                ,{<<"FaxBox-ID">>, FaxBoxId}
                ,{<<"Account-ID">>, AccountId}
                ,{<<"Account-DB">>, FaxDb}
                ,{<<"Fax-Notifications">>, FaxNotify}
                | notify_fields(Call, JObj)
                ]),
    Prop = props:filter_undefined(
             [{<<"Subject">>, kz_binary:format("System Alert: ~s", [Subject])}
             ,{<<"Message">>, Message}
             ,{<<"Details">>, kz_json:from_list(Details)}
             ,{<<"Account-ID">>, AccountId}
             | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
             ]),
    kz_amqp_worker:cast(Prop, fun kapi_notifications:publish_system_alert/1).
