...
account_id(APIResp) ->
    kz_json:get_ne_binary_value([<<"metadata">>, <<"id">>], APIResp).
...
number_state(APIResp) ->
    kz_json:get_atom_value([<<"metadata">>, <<"state">>], APIResp).
...
-spec metadata(pqc_cb_api:response() | kz_json:object()) ->
          kz_json:object() | kz_json:objects() | kz_term:ne_binaries().
metadata(JSON) when is_binary(JSON) ->
    metadata(kz_json:decode(JSON));
metadata({'error', RespBody}) when is_binary(RespBody) ->
    metadata(kz_json:decode(RespBody));
metadata(APIResp) ->
    kz_json:get_value(<<"metadata">>, APIResp).
