    ...
    ?FORALL(Cmds
           ...
           ,?TRAPEXIT(
               begin
                   ...
                   seq_accounts:cleanup_accounts(pqc_kazoo_model:api(Model), ?ACCOUNT_NAMES),

                   ?WHENFAIL(io:format("Final Model : ~p~nFailing Cmds: ~p~n"
                                      ,[pqc_kazoo_model:pp(Model), zip(Cmds, History)]
                                      )
                            ,aggregate(command_names(Cmds), is_result_ok(Result))
                            )
               end
              )
           ).
    ...
    ?FORALL(Cmds
           ...
           ,?TRAPEXIT(
               begin
                   ...
                   seq_accounts:cleanup_accounts(?ACCOUNT_NAMES),
...
-spec is_result_ok(any()) -> boolean().
is_result_ok('ok') -> 'true';
is_result_ok(_) -> 'false'.
...
command(Model, 'false') ->
    pqc_accounts:command(Model, name());
command(Model, 'true') ->
    API = pqc_kazoo_model:api(Model),
    AccountId = pqc_accounts:symbolic_account_id(Model, name()),
