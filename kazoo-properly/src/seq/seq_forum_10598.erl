...
seq() ->
    ...
    CreateMetaResp = pqc_cb_media:create(API, ParentAccountId, seq_media:new_media_doc()),
...
new_menu(AccountId, MediaId) ->
    ...
    kzd_menus:set_media_greeting(seq_menus:new_menu(), Greeting).

-spec create_account(pqc_cb_api:state(), kz_term:ne_binary()) -> kz_term:ne_binary().
create_account(API, Name) ->
    AccountResp = properly_accountant:create_account(API, Name),
...
-spec cleanup() -> 'ok'.
cleanup() ->
    _ = seq_accounts:cleanup_accounts([<<?MODULE_STRING, "_parent">>, <<?MODULE_STRING, "_child">>]),
    cleanup_system().
