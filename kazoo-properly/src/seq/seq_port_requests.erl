...
create_account(API, AccountName, State) ->
    ...
    properly_accountant:create_account(API, AccountName, AccountId).
...
create_admin_user(API, AccountId, AccountName) ->
    ...
    User = kz_json:set_value(<<"send_email_on_creation">>
                            ,'false'
                            ,kz_doc:setters(kzd_users:new(), Setters)
                            ),
    pqc_cb_users:create(API, AccountId, User).
...
create_ports_seq(State, AccountName, Resp) ->
    ...
    Metadata = pqc_cb_response:metadata(Resp),
    PortId = kz_doc:id(Metadata),
    ...
    ,{"account name"
     ,?_assertEqual(AccountName, kz_json:get_ne_binary_value(<<"account_name">>, Metadata))
     }
    ,{"authority id"
     ,?_assertEqual(AuthorityId, kz_json:get_ne_binary_value(<<"port_authority">>, Metadata))
     }
    ,{"authority name '" ++ kz_term:to_list(AuthorityName) ++ "'"
     ,?_assertEqual(AuthorityName, kz_json:get_ne_binary_value(<<"port_authority_name">>, Metadata))
     }
    ].
...
self_list_seq(State, AccountName, Resp) ->
    ...
    JObjs = pqc_cb_response:data(Resp),

    AccountList = [Acc || Acc <- JObjs,
...
create_port(API, AccountName) ->
    ...
    pqc_cb_port_requests:create(API, AccountId, seed_ports(AccountName)).
...
list_account_ports(API) ->
    pqc_cb_port_requests:summary(API, pqc_cb_api:auth_account_id(API)).

-spec list_descendants_ports(pqc_cb_api:state()) -> pqc_cb_api:response().
list_descendants_ports(API) ->
    pqc_cb_port_requests:fetch_descendants(API, pqc_cb_api:auth_account_id(API)).
...
seed_ports(AccountName) ->
    ...
    Setters = [{fun kzd_port_requests:set_bill_account_number/2, <<"Seed">>}
              ...
              ,{fun kzd_port_requests:set_signee_name/2, <<"properly">>}
