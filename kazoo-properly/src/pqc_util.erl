...
-type querystring() :: kz_term:ne_binary() | kz_term:proplist() | kz_json:object().

-export_type([querystring/0]).
...
-spec to_querystring(querystring()) -> string().
to_querystring(<<QueryString/binary>>) ->
    kz_term:to_list(QueryString);
to_querystring(Props) when is_list(Props) ->
    kz_term:to_list(iolist_to_binary(kz_http_util:props_to_querystring(Props)));
to_querystring(JObj) ->
    kz_term:to_list(iolist_to_binary(kz_http_util:json_to_querystring(JObj))).

-spec create_number(1..10) -> kz_term:ne_binary().
create_number(Size) ->
    %% random {Size}-digit extension
    TopEnd = round(math:pow(10, Size) - 1),
    BottomEnd = round(math:pow(10, Size-1)),

    kz_term:to_binary(rand:uniform(TopEnd - BottomEnd) + BottomEnd).
