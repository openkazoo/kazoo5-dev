...
-export([send_email/3, send_email/4, send_email/5
        ,render_subject/2
        ,render/3
        ,system_params/0
        ,account_params/1
...
send_email(Emails, Subject, RenderedTemplates) ->
    send_email(Emails, Subject, RenderedTemplates, 'undefined', []).

-spec send_email(email_map(), kz_term:ne_binary(), rendered_templates(), kz_term:api_ne_binary()) ->
          'ok' | {'error', any()}.
send_email(Emails, Subject, RenderedTemplates, AccountId) ->
    send_email(Emails, Subject, RenderedTemplates, AccountId, []).

-spec send_email(email_map(), kz_term:ne_binary(), rendered_templates(), kz_term:api_ne_binary(), attachments()) ->
          'ok' | {'error', any()}.
send_email(Emails0, Subject, RenderedTemplates, AccountId, Attachments) ->
    ...
    From = props:get_ne_binary_value(<<"from">>, Emails),
    Email = {<<"multipart">>
            ,<<"mixed">>
            ,email_headers([{<<"To">>, To}
                           ,{<<"Cc">>, props:get_value(<<"cc">>, Emails)}
                           ,{<<"Bcc">>, props:get_value(<<"bcc">>, Emails)}
                           ,{<<"X-Teletype-Log-ID">>, get_call_id()}
                           ]
                          ,[{<<"From">>, From}
    ...
    Options = maybe_set_dkim_options(AccountId, From),
    case relay_email(To, From, Email, Options) of
        {'ok', Receipt} ->
...
-spec get_call_id() -> binary().
get_call_id() ->
    case kz_log:get_callid() of
        'undefined' ->
            kz_log:find_callid([]);
        CallId ->
            kz_term:to_binary(CallId)
    end.
...
-spec email_headers(kz_term:proplist(), kz_term:proplist()) -> kz_term:proplist().
email_headers([], Params) ->
    lists:reverse(props:filter_empty(Params));
email_headers([{_Key, 'undefined'}|T], Params) ->
    email_headers(T, Params);
email_headers([{Key, V}|T], Params) ->
    email_headers(T, [{Key, V} | Params]).

-spec relay_email(kz_term:api_binaries(), kz_term:ne_binary(), mimemail:mimetuple(), kz_term:proplist()) ->
          {'ok', kz_term:ne_binary()} |
          {'error', any()}.
relay_email(To, From, {_Type, _SubType, Headers, _ParamsMap, _Body}=Email, Options) ->
    try mimemail:encode(Email, Options) of
        Encoded ->
            RelayResult = relay_encoded_email(To, From, Encoded),
            maybe_relay_to_bcc(From, Encoded, props:get_value(<<"Bcc">>, Headers)),
            RelayResult
    catch
        'error':'missing_from' ->
            lager:warning("no From address: ~s: ~p", [From, Headers]),
            {'error', 'missing_from'};
        ?STACKTRACE(_E, _R, ST)
...
-spec send(kz_term:ne_binaries(), kz_term:ne_binary(), kz_term:ne_binary()) -> gen_smtp_send_resp().
...
-spec wait_for_response(kz_term:binaries(), kz_term:ne_binary()) ->
          {'ok', kz_term:ne_binary()} |
          {'error', any()}.
wait_for_response(To, From) ->
...
handle_relay_response(_To, _From, {'exit', Reason}) ->
    ...
    {'error', kz_term:to_binary(io_lib:format("failed to send email, exit relaying message: ~p", [Reason]))}.
...
smtp_options() ->
    ...
    props:filter_empty(
      ...
      ,{'trace_fun', fun(Format, Args) -> lager:info(Format, Args) end}
      | SSLSockOpts
      ]).
...
add_rendered_templates_to_email([{ContentType, Content}|Rs], Acc) ->
    [Type, SubType] = binary:split(ContentType, <<"/">>),
    CTEncoding = kapps_config:get_ne_binary(?CONFIG_CAT
                                           ,[<<"mime-encoding">>
                                            ,ContentType
...
%% This function is supposed to sanitize the email content. For now
...
user_params(UserJObj) ->
    Ks = [{<<"first_name">>, fun kzd_users:first_name/1}
         ...
         ,{<<"full_name">>, fun kzd_users:full_name/1}
         ,{<<"email">>, fun kzd_users:email/1}
         ,{<<"username">>, fun kzd_users:username/1}
...
-spec branding_params(kz_json:object()) -> kz_term:proplist().
branding_params(DataJObj) ->
    case kapi_notifications:account_id(DataJObj) of
        'undefined' ->
            [{<<"account">>, []}
            ,{<<"reseller">>, []}
        ...
        AccountId ->
            [{<<"account">>, find_branding_params(AccountId)}
            ,{<<"reseller">>, find_branding_params(kz_services_reseller:get_id(AccountId))}
            ]
    end.

-spec find_branding_params(kz_term:api_binary()) -> kz_term:proplist().
find_branding_params('undefined') -> [];
find_branding_params(AccountId) ->
    case kzd_whitelabel:fetch(AccountId) of
        {'ok', WhiteLabelJObj} ->
            kz_json:recursive_to_proplist(
              kz_doc:public_fields(WhiteLabelJObj)
             );
        {'error', _E} ->
            ?LOG_DEBUG("failed to fetch whitelabel doc for ~s: ~p", [AccountId, _E]),
            []
    end.
...
- spec should_notify(kz_json:object()) -> boolean().
should_notify(ApiJObj) ->
    kz_json:is_true(<<"Should-Notify">>, ApiJObj, 'true').
...
fetch_attachment_from_url(URL) ->
    case kz_http:get(kz_term:to_list(URL)) of
        ...
        {'ok', _Code, _Headers, _Body} ->
            lager:debug("failed to get attachment from url ~s for reason: ~p : ~p"
                       ,[kz_log:redactor(URL), _Code, _Headers]
                       ),
            {'error', 'not_handled'};
        Error ->
            lager:debug("failed to get attachment from url ~s for reason: ~p"
                       ,[kz_log:redactor(URL), Error]
                       ),
            Error
    end.
...
-spec maybe_set_dkim_options(kz_term:api_ne_binary(), kz_term:ne_binary()) -> kz_term:proplist().
maybe_set_dkim_options('undefined', _From) -> [];
maybe_set_dkim_options(AccountId, From) ->
    case kzd_whitelabel:fetch(AccountId) of
        {'ok', WhiteLabelJObj} ->
            maybe_set_dkim_options(AccountId, From, WhiteLabelJObj);
        {'error', _E} ->
            lager:debug("failed to fetch whitelabel doc for ~s: ~p", [AccountId, _E]),
            []
    end.

-spec maybe_set_dkim_options(kz_term:api_ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
          kz_term:proplist().
maybe_set_dkim_options(AccountId, From, WhiteLabelJObj) ->
    case kzd_emails:fetch(AccountId, From) of
        'undefined' -> [];
        EmailJObj ->
            set_dkim_options(read_pem_file(AccountId, kz_doc:id(EmailJObj))
                            ,kzd_emails:dkim_selector(EmailJObj)
                            ,kzd_whitelabel:domain(WhiteLabelJObj)
                            )
    end.

-spec read_pem_file(kz_term:ne_binary(), kz_term:ne_binary()) -> kz_term:api_ne_binary().
read_pem_file(AccountId, DocId) ->
    case kz_datamgr:fetch_attachment(kzs_util:format_account_db(AccountId)
                                    ,{kzd_emails:type(), DocId}
                                    ,kzd_emails:dkim_filename()
                                    )
    of
        {'ok', PvtKey} -> PvtKey;
        {'error', _Reason} ->
            lager:debug("unable to open DKIM pem file ~s/~s: ~p"
                       ,[AccountId, DocId, _Reason]
                       ),
            'undefined'
    end.

-spec set_dkim_options(kz_term:api_ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
          kz_term:proplist().
set_dkim_options(<<PvtKey/binary>>, <<DKIMSelector/binary>>, <<Domain/binary>>) ->
    DKIMOptions = [{'s', DKIMSelector}
                  ,{'d', Domain}
                  ,{'private_key', {'pem_plain', PvtKey}}
                  ],
    [{'dkim', DKIMOptions}];
set_dkim_options(_PvtKey, _DKIMSelector, _Domain) ->
    [].

-spec get_template_account_id(kz_json:object(), kz_term:ne_binary()) -> kz_term:ne_binary().
get_template_account_id(TemplateMetaJObj, AccountId) ->
    case kz_doc:account_id(TemplateMetaJObj, AccountId) of
        <<"system_config">> ->
            AccountId;
        TemplateAccountId ->
            TemplateAccountId
