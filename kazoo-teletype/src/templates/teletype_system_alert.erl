...
-spec handle_req(kz_json:object()) -> template_response() | template_responses().
handle_req(JObj) ->
    handle_req(JObj, kapi_notifications:system_alert_v(JObj)).

-spec handle_req(kz_json:object(), boolean()) -> template_response() | template_responses().
handle_req(_, 'false') ->
...
process_req_as_email(DataJObj) ->
    ...
    case teletype_util:send_email(Emails, Subject, RenderedTemplates, TemplateAccountId, Attachments) of
...
details_macros(DataJObj) ->
    case kz_json:get_value(<<"details">>, DataJObj) of
        'undefined' -> [];
        <<Details/binary>> -> [{<<"details">>, [{<<"message">>, Details}]}];
