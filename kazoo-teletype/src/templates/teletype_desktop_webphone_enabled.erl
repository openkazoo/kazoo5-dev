...
%%% @end
%%%-----------------------------------------------------------------------------
-module(teletype_desktop_webphone_enabled).
...
id() -> <<"desktop_webphone_enabled">>.

-spec macros() -> kz_json:object().
macros() ->
    kz_json:from_list(
      [?MACRO_VALUE(<<"desktop.app_name">>, <<"desktop_webphone_enabled">>, <<"Desktop App Name">>, <<"Desktop App Name">>)
      | ?USER_MACROS
       ++ ?BRANDING_MACROS ++ ?COMMON_TEMPLATE_MACROS
      ]).

-spec subject() -> kz_term:ne_binary().
subject() -> <<"You're webphone is now enabled">>.

-spec category() -> kz_term:ne_binary().
category() -> <<"user">>.

-spec friendly_name() -> kz_term:ne_binary().
friendly_name() -> <<"Desktop Webphone App Enabled">>.
...
handle_req(JObj) ->
    handle_req(JObj, kapi_notifications:desktop_webphone_enabled_v(JObj)).
