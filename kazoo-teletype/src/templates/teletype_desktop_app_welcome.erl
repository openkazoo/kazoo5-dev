...
%%% @end
%%%-----------------------------------------------------------------------------
-module(teletype_desktop_app_welcome).
...
id() -> <<"desktop_app_welcome">>.

-spec macros() -> kz_json:object().
macros() ->
    kz_json:from_list(
      [?MACRO_VALUE(<<"desktop.app_name">>, <<"desktop_app_name">>, <<"Desktop App Name">>, <<"Desktop App Name">>)
      ,?MACRO_VALUE(<<"desktop.webphone_enabled">>, <<"webphone_enabled">>, <<"Webphone Enabled">>, <<"Indicates if webphone is enabled for desktop app">>)
      | ?USER_MACROS
       ++ ?BRANDING_MACROS ++ ?COMMON_TEMPLATE_MACROS
      ]).

-spec subject() -> kz_term:ne_binary().
subject() -> <<"Welcome to your new desktop app">>.

-spec category() -> kz_term:ne_binary().
category() -> <<"user">>.

-spec friendly_name() -> kz_term:ne_binary().
friendly_name() -> <<"Welcome to Desktop App">>.
...
handle_req(JObj) ->
    handle_req(JObj, kapi_notifications:desktop_app_welcome_v(JObj)).
...
create_macros(DataJObj) ->
    ...
    DesktopMacros = [{<<"app_name">>, kz_json:get_ne_binary_value(<<"desktop_app_name">>, DataJObj, <<"comm.land">>)}
                    ,{<<"webphone_enabled">>, kz_json:is_true(<<"webphone_enabled">>, DataJObj, 'false')}
                    ],

    [{<<"system">>, teletype_util:system_params()}
    ,{<<"account">>, teletype_util:account_params(DataJObj)}
    ,{<<"branding">>, teletype_util:branding_params(DataJObj)}
    ,{<<"desktop">>, DesktopMacros}
    ,{<<"user">>, UserParams}
    ].
