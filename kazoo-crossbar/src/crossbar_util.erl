...
-export([get_user_lang/2
        ...
        ,get_scope_restrictions/1, get_scope_restrictions/2
        ]).
...
-spec response_200(kz_json:json_term(), cb_context:context()) ->
          cb_context:context().
response_200(JTerm, Context) ->
    create_response('success', 'undefined', 200, JTerm, Context).
...
%% @end
%%------------------------------------------------------------------------------
-spec response(fails() | success(), kz_term:api_ne_binary(), kz_term:api_integer(), kz_json:json_term(), cb_context:context()) -> cb_context:context().
...
response('success'=Status, Msg, Code, JTerm, Context) ->
...
move_account(?MATCH_ACCOUNT_RAW(AccountId), ToAccount=?NE_BINARY) ->
    case validate_move(AccountId, ToAccount) of
        ...
        {'ok', AccountJObj, NewTree} ->
            move_account(AccountId, AccountJObj, ToAccount, NewTree)
    end.
...
move_account(AccountId, AccountJObj, ToAccount, NewTree) ->
    ...
    Updates = [{[kzd_services:path_tree()], NewTree}
              ,{[kzd_services:path_tree_previously()], PreviousTree}
    ...
    case kzd_accounts:update(AccountId, Updates) of
        {'error', _E}=Error ->
            lager:info("failed to update account ~s tree: ~p", [AccountId, _E]),
            Error;
        {'ok', AccountDoc} ->
            lager:info("moved account ~s (~s)", [AccountId, kz_doc:revision(AccountDoc)]),

            NewResellerId = get_new_reseller_id(ToAccount),
            DescendantsNewResellerId = get_descendants_new_reseller_id(AccountId, NewResellerId),

            lager:info("account ~s has new reseller ~s", [AccountId, NewResellerId]),
            {'ok', _} = move_descendants(AccountId, NewTree, DescendantsNewResellerId),
            lager:info("moved descendants of ~s", [AccountId]),
            _Services = kazoo_services_maintenance:update_tree(AccountId, NewTree, NewResellerId),
            lager:info("updating sevices tree for ~s", [AccountId]),
            _ = descendants_count(lists:last(NewTree)),
            _ = descendants_count(lists:last(PreviousTree)),
            {'ok', AccountDoc}
    end.

-spec get_new_reseller_id(kz_term:ne_binary()) -> kz_term:api_ne_binary().
get_new_reseller_id(ToAccountId) ->
    case kz_services_reseller:is_reseller(ToAccountId) of
        'true' -> ToAccountId;
        'false' -> kz_services_reseller:get_id(ToAccountId)
    end.

-spec get_descendants_new_reseller_id(kz_term:ne_binary(), kz_term:ne_binary()) ->
          kz_term:ne_binary().
get_descendants_new_reseller_id(AccountId, NewResellerId) ->
...
validate_move(AccountId, ToAccount) ->
    case {kzd_accounts:fetch(AccountId), kzd_accounts:fetch(ToAccount)} of
        {{'error', _E}=Error, _} ->
            lager:info("failed to fetch account ~s: ~p", [AccountId, _E]),
            Error;
        {{'ok', _AccountJObj}, {'error', 'not_found'}} ->
            lager:info("to account ~s not found", [ToAccount]),
            {'error', 'not_found'};
        {{'ok', _AccountJObj}, {'error', _E}} ->
            lager:info("failed to load to account ~s: ~p", [ToAccount, _E]),
            {'error', 'forbidden'};
        {{'ok', AccountJObj}, {'ok', ToAccountJObj}} ->
            NewTree = kzd_accounts:tree(ToAccountJObj) ++ [ToAccount],
            case lists:member(AccountId, NewTree) of
                'true' ->
                    lager:info("account ~s is already under account ~s tree: ~p", [AccountId, ToAccount, NewTree]),
                    {'error', 'forbidden'};
                'false' ->
                    lager:info("updating account ~s tree to be under account ~s", [AccountId, ToAccount]),
                    {'ok', AccountJObj, NewTree}
            end
    end.
...
update_descendants_tree([Descendant|Descendants], Tree, NewResellerId, MovedAccountId) ->
    case kzd_accounts:fetch(Descendant) of
        ...
        {'ok', AccountJObj} ->
            ...
            ToTree = Tree ++ Tail,
            Updates = [{[kzd_services:path_tree()], ToTree}
                      ,{[kzd_services:path_tree_previously()], PreviousTree}
                      ,{[kz_doc:path_modified()], kz_time:now_s()}
                      ],
            %%FIXME: do something about setting pvt_auth_*_id
...
populate_resp(JObj, AccountId, UserId) ->
    ...
    UserInfo = get_user_info(AccountId, UserId),
...
-spec get_user_info(kz_term:ne_binary(), kz_term:api_ne_binary()) -> kz_term:api_object().
get_user_info(AccountId, UserId) ->
    case kzd_users:fetch(AccountId, UserId) of
        {'ok', Doc} ->
            Setters = [{fun kzd_users:set_first_name/2, kzd_users:first_name(Doc)}
                      ,{fun kzd_users:set_last_name/2, kzd_users:last_name(Doc)}
                      ,{fun kzd_users:set_email/2, kzd_users:email(Doc)}
                      ,{fun(J, V) -> kz_json:set_value(<<"full_name">>, V, J) end, kzd_users:full_name(Doc)}
                      ,{fun kzd_users:set_username/2, kzd_users:username(Doc)}
                      ,{fun kzd_users:set_priv_level/2, kzd_users:priv_level(Doc)}
                      ,{fun kzd_users:set_has_avatar/2, kzd_users:has_avatar(Doc)}
                      ],
...
format_app(Lang, AppJObj) ->
    Label = kz_json:get_first_defined([[Lang, <<"label">>]
                                      ,[?DEFAULT_LANGUAGE, <<"label">>]
                                      ,[<<"en-US">>, <<"label">>]
                                      ]
                                     ,kzd_app:i18n(AppJObj)
                                     ),
    kz_json:from_list(
...
get_path(<<RawPath/binary>>, Relative) ->
...
-spec get_token_restrictions(atom(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
          kz_term:api_object().
...
-spec get_account_token_restrictions(kz_term:api_ne_binary(), atom()) -> kz_term:api_object().
get_account_token_restrictions('undefined', _) ->
    'undefined';
...
-spec get_scope_restrictions(kz_term:api_object()) -> kz_term:api_object().
get_scope_restrictions('undefined') ->
    'undefined';
get_scope_restrictions(AuthDoc) ->
    AccountId = kz_json:get_ne_binary_value(<<"account_id">>, AuthDoc),
    OwnerId = kz_json:get_ne_binary_value(<<"owner_id">>, AuthDoc),
    get_scope_restrictions(AccountId, OwnerId).

-spec get_scope_restrictions(kz_term:api_ne_binary(), kz_term:api_ne_binary()) -> kz_term:api_object().
get_scope_restrictions('undefined', _) ->
    'undefined';
get_scope_restrictions(_, 'undefined') ->
    'undefined';
get_scope_restrictions(AccountId, OwnerId) ->
    AccountDB = kzs_util:format_account_db(AccountId),
    case kz_datamgr:open_cache_doc(AccountDB, OwnerId) of
        {'ok', Doc} ->
            Restrictions = kzd_users:scope_restrictions(Doc),
            maybe_merge_scope_restrictions(AccountDB, Restrictions);
        {'error', _} -> 'undefined'
    end.

-spec maybe_merge_scope_restrictions(kz_term:ne_binary(), kz_term:api_binaries()) -> kz_term:api_object().
maybe_merge_scope_restrictions(_AccountDb, 'undefined'=ScopeRestrictions) ->
    ScopeRestrictions;
maybe_merge_scope_restrictions(AccountDb, ScopeRestrictions) ->
    ViewOptions = [{'keys', ScopeRestrictions}],
    case kz_datamgr:get_results(AccountDb, <<"scope_restrictions/crossbar_listing">>, ViewOptions) of
        {'ok', Docs} ->
            merge_scope_restrictions(Docs);
        {'error', _} -> 'undefined'
    end.

-spec merge_scope_restrictions(kz_json:objects()) -> kz_term:api_object().
merge_scope_restrictions(Docs) ->
    Merged = lists:foldl(fun merge_scope_restrictions_fold/2, kz_json:new(), Docs),
    case kz_json:is_empty(Merged) of
        'true' -> 'undefined';
        'false' -> Merged
    end.

-spec merge_scope_restrictions_fold(kz_doc:doc(), kz_json:object()) -> kz_json:object().
merge_scope_restrictions_fold(Doc, Acc) ->
    case kz_json:get_json_value([<<"value">>, <<"token_restrictions">>], Doc) of
        'undefined' -> Acc;
        Restrictions ->
            kz_json:merge(Restrictions, Acc)
    end.
...
descendants_count() ->
    Limit = kapps_config:get_integer(<<"kazoo_couch">>, <<"default_chunk_size">>, 1000),
...
maybe_update_descendants_count(AccountId, _, Try) when Try =< 0 ->
    lager:warning("too many attempts to update descendants count for ~s~n", [AccountId]);
maybe_update_descendants_count(AccountId, NewCount, Try) ->
    case kzd_accounts:fetch(AccountId) of
        {'error', _E} ->
            lager:info("could not load account ~s: ~p~n", [AccountId, _E]);
        {'ok', AccountJObj} ->
