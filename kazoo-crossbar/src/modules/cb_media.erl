...
create_update_tts(Context, <<"update">>) ->
    ...
    maybe_update_media_file(Context1, <<"update">>, is_tts_changed(cb_context:doc(Context1)), cb_context:resp_status(Context1)).
...
maybe_delete_tts(Context, Reason, <<"update">>) ->
    crossbar_util:response('error', Reason, Context).
...
load_media_docs_by_language(Context, Language, 'undefined') ->
    Options = [{'startkey', [Language]}
              ,{'endkey', [Language, kz_term:high_unicode_value()]}
              ,{'reduce', 'false'}
...
load_media_docs_by_prompt(Context, PromptId, AccountId) ->
    Options = load_docs_by_prompt_options(PromptId, AccountId),
    crossbar_view:load(Context, ?CB_LIST_BY_PROMPT, Options).

load_docs_by_prompt_options(PromptId, AccountId) ->
    [{'startkey', [PromptId]}
    ,{'endkey', [PromptId, kz_term:high_unicode_value()]}
    ,{'reduce', 'false'}
    ,{'mapper', fun normalize_prompt_results/2}
    ,'include_docs'
    | load_docs_by_prompt_dbs(AccountId)
    ].

load_docs_by_prompt_dbs('undefined') ->
    [{'databases', [?KZ_MEDIA_DB]}];
load_docs_by_prompt_dbs(_AccountId) ->
    [].
...
-spec update_media_binary(cb_context:context(), path_token(), req_files()) ->
          cb_context:context().
update_media_binary(Context, _MediaId, []) -> cb_context:set_resp_status(Context, 'success');
