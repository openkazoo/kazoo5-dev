...
validate_delete_metaflows(Context) ->
    case cb_context:resp_status(Context) of
        'success' ->
            Doc = cb_context:doc(Context),
            Context1 = cb_context:set_doc(Context, kz_json:delete_key(<<"metaflows">>, Doc)),
            crossbar_util:response(kz_json:new(), Context1);
        _ ->
            Context
    end.
...
after_post(Context, <<"account">>, 'success') ->
    _ = kzd_accounts:save_accounts_doc(cb_context:doc(Context)),
