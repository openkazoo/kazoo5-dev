...
validate_patch(Context, WebhookId) ->
    crossbar_doc:patch_and_validate(WebhookId, Context, fun update/2).

-spec post(cb_context:context(), path_token()) -> cb_context:context().
post(Context, _Id) ->
    Context1 = maybe_enable_webhook(Context),
...
read(Id, Context) ->
    ...
    case cb_context:resp_status(Context1) of
        'success' -> maybe_add_metadata_fields(Context1);
        _Status -> Context1
    end.

-spec maybe_add_metadata_fields(cb_context:context()) -> cb_context:context().
maybe_add_metadata_fields(Context) ->
    Doc = cb_context:doc(Context),
    cb_context:add_metadata_value(Context
                                 ,<<"disable_reason">>
                                 ,kzd_webhooks:disabled_message(Doc)
                                 ).
...
summary_attempts(Context, HookId) ->
    ...
    Options = summary_attempts_options(HookId),
    crossbar_view:load_modb(Context, ViewName, Options).

summary_attempts_options(HookId) ->
    [{'mapper', fun normalize_attempt_results/2}
    ,{'range_keymap', HookId}
    ,{'field_key', 'filtermap'}
    ,'include_docs'
    ].
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_enable_webhook(cb_context:context()) -> cb_context:context().
maybe_enable_webhook(Context) ->
    Doc = cb_context:doc(Context),
