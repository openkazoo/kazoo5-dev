...
create_request(Context) ->
    ...
    From = kzd_sms:from(Payload, DefaultCID),
...
reconcile_to(Context, <<"offnet">>) ->
    ...
    case reconcile_numbers(AccountId, To) of
        To -> Context;
        Reconciled -> cb_context:set_doc(Context, kzd_sms:set_to(Doc, Reconciled))
    end.
...
set_route_type(Context) ->
    set_route_type(Context, kzd_sms:to(cb_context:doc(Context))).
...
validate_route_type(Context) ->
    validate_route_type(Context, kzd_sms:to(cb_context:doc(Context))).
