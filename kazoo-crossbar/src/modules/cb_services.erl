...
to_csv({Req, Context}) ->
    {Req, to_response(Context, cb_context:req_nouns(Context))}.

-spec to_response(cb_context:context(), req_nouns()) ->
          cb_context:context().
to_response(Context, [{<<"services">>, [?SUMMARY]}, {?KZ_ACCOUNTS_DB, _}|_]) ->
