...
-define(DEFAULT_NOT_REG_TYPES, [<<"application">>
                               ,<<"cellphone">>
                               ,<<"sip_url">>
                               ,<<"landline">>
                               ,<<"smartphone">>
...
-define(NOT_REG_TYPES
       ,kapps_config:get_ne_binaries(?MOD_CONFIG_CAT, <<"not_registrable_device_types">>, ?DEFAULT_NOT_REG_TYPES)
       ).
...
load_device_summary(Context, [{<<"devices">>, []}, {<<"users">>, [UserId]} | _]) ->
    Selector = [{'start', [{<<"doc_type">>, kzd_devices:type()}, {<<"owner_id">>, UserId}]}
               ,{'end', [{<<"doc_type">>, kzd_devices:type()}, {<<"owner_id">>, UserId}]}
               ],
    WithRegs = kz_term:is_true(cb_context:req_param(Context, <<"with_status">>)),
    maybe_with_device_status(Context, <<"crossbar_listings/by_type_ownerid">>, Selector, WithRegs);
load_device_summary(Context, _ReqNouns) ->
    Selector = [{'start', [{<<"doc_type">>, kzd_devices:type()}]}
               ,{'end', [{<<"doc_type">>, kzd_devices:type()}]}
               ],
    WithRegs = kz_term:is_true(cb_context:req_param(Context, <<"with_status">>)),
    maybe_with_device_status(Context, <<"crossbar_listings/by_type_id">>, Selector, WithRegs).

maybe_with_device_status(Context, View, Selector, 'true') ->
    ValueFun = fun(JObj) -> kz_json:get_value(<<"value">>, JObj) end,
    load_device_status(Context, View, Selector, ValueFun);
maybe_with_device_status(Context, View, Selector, 'false') ->
...
validate_device(DeviceId, Context) ->
    Routines = [fun prepare_mac_address/2
               ...
               ,fun validate_call_failover/2
               ,fun validate_call_forward/2
               ],
...
load_device_status(Context) ->
    ...
    ValueFun = fun(JObj) -> kz_json:from_list([{<<"device_id">>, kz_doc:id(JObj)}]) end,
    load_device_status(Context, <<"crossbar_listings/by_type_id">>, Selector, ValueFun).

-spec load_device_status(cb_context:context(), kz_term:ne_binary(), kz_view:selector(), fun((kz_json:object()) -> kz_json:object())) ->
          cb_context:context().
load_device_status(Context, View, Selector, ValueFun) ->
    NotRegTypes = ?NOT_REG_TYPES,

    AccountRealm = kz_term:to_lower_binary(
                     kzd_accounts:fetch_realm(cb_context:account_id(Context))
                    ),
    RegUsernames = lookup_regs(AccountRealm, 'sets'),
...
normalize_reg_devices(JObj, Acc, RegUsernames, NotRegTypes, ValueFun, AccountRealm) ->
    Value = ValueFun(JObj),
    Username = kz_json:get_ne_binary_value([<<"value">>, <<"username">>], JObj),

    case lists:member(kz_json:get_value([<<"value">>, <<"device_type">>], JObj), NotRegTypes) of
        'true' ->
            [kzd_registrations:set_registrable(Value, 'false') | Acc];
        'false' when Username =:= 'undefined' ->
            Prop = [{fun kzd_registrations:set_registered/2, 'false'}
                   ,{fun kzd_registrations:set_registrable/2, 'true'}
                   ],
            [kz_json:set_values(Prop, Value) | Acc];
        'false' ->
            To = <<(kz_term:to_lower_binary(Username))/binary, "@", AccountRealm/binary>>,
            Prop = [{fun kzd_registrations:set_registered/2, gb_sets:is_member(To, RegUsernames)}
                   ,{fun kzd_registrations:set_registrable/2, 'true'}
                   ],
            [kz_json:set_values(Prop, Value) | Acc]
    end.
...
lookup_regs(AccountRealm, ReturnType) ->
    Regs = search_registrations(AccountRealm),
    process_regs(Regs, ReturnType).

-spec process_regs(kz_term:ne_binaries(), 'sets' | 'list') ->
          kz_json:objects() | gb_sets:set(kz_term:ne_binaries()).
process_regs(Registrations, 'sets') ->
    gb_sets:from_list(Registrations);
...
-spec search_registrations(kz_term:ne_binary()) -> kz_term:ne_binaries().
search_registrations(AccountRealm) ->
    [kz_term:to_lower_binary(Username)
     || Username <- kapi_registration:search_realm_regs(AccountRealm, <<"summary">>)
    ].
...
%% @end
%%------------------------------------------------------------------------------
-spec get_mac_address(cb_context:context()) -> kz_term:api_ne_binary().
get_mac_address(Context) ->
    kz_network_utils:cleanse_mac_address(
...
get_mdn(JObj) ->
    case kzd_devices:mobile_mdn(JObj) of
        'undefined' -> 'undefined';
...
prepare_mac_address(_DeviceId, Context) ->
    case get_mac_address(Context) of
        'undefined' -> Context;
        MacAddress ->
            JObj = kzd_devices:set_mac_address(cb_context:req_data(Context), MacAddress),
...
prepare_provisioner_fields(_DeviceId, Context) ->
    maybe_prepare_provisioner_fields(Context, cb_context:req_data(Context)).

maybe_prepare_provisioner_fields(Context, ReqData) ->
    maybe_prepare_provisioner_fields(Context, ReqData, kzd_devices:provision(ReqData)).

maybe_prepare_provisioner_fields(Context, ReqData, 'undefined') ->
    update_req_with_provisioner_fields(Context, ReqData, ReqData);
maybe_prepare_provisioner_fields(Context, ReqData, _Provision) ->
    Funs = [{fun kzd_devices:provision_combo_keys/1, fun kzd_devices:set_provision_combo_keys/2}
           ,{fun kzd_devices:provision_feature_keys/1, fun kzd_devices:set_provision_feature_keys/2}
           ,{fun kzd_devices:provision_endpoint_brand/1, fun kzd_devices:set_provision_endpoint_brand/2}
           ,{fun kzd_devices:provision_id/1, fun kzd_devices:set_provision_id/2}
           ],
    JObj = lists:foldl(fun prune_null_provisioner_field/2, ReqData, Funs),
    update_req_with_provisioner_fields(Context, ReqData, JObj).
...
-type getter_fun() :: fun((kzd_devices:doc()) -> kz_json:api_json_term()).
-type setter_fun() :: fun((kzd_devices:doc(), kz_json:api_json_term()) -> kzd_devices:doc()).
-spec prune_null_provisioner_field({getter_fun(), setter_fun()}, kzd_devices:doc()) ->
          kzd_devices:doc().
prune_null_provisioner_field({Getter, Setter}, ReqData) ->
    case Getter(ReqData) of
        'undefined' -> ReqData;
        <<Value/binary>> ->
            Setter(ReqData, Value);
        Value ->
            FilteredValue = kz_json:filter(fun filter_null_fields/1, Value),
            Setter(ReqData, FilteredValue)
    end.
...
error_mdn_changed(Context) ->
    _OldMDN = kzd_devices:mobile_mdn(cb_context:fetch(Context, 'db_doc')),
...
check_mdn_taken(MDN, DeviceId, Context) ->
    case knm_numbers:get(MDN, knm_options:mdn_options()) of
        {'ok', [], [{MDN, 'not_found'}]} ->
...
-spec check_mac_address(kz_term:api_ne_binary(), cb_context:context()) -> cb_context:context().
...
get_mac_addresses(DbName) ->
    ...
    [kz_network_utils:cleanse_mac_address(MAC) || MAC <- MACs].

-spec get_current_mac_address(kz_term:api_binary(), cb_context:context()) -> kz_term:api_ne_binary().
...
validate_device_creds(DeviceId, Context) ->
    ...
    case kzd_devices:sip_method(ReqData, <<"password">>) of
        <<"password">> ->
            check_device_password(DeviceId, Context, ReqData);
        <<"ip">> ->
            check_device_ip(kzd_devices:sip_ip(ReqData), DeviceId, Context);
        _Else ->
            Context
    end.

-spec check_device_password(kz_term:api_ne_binary(), cb_context:context(), kz_json:object()) ->
          cb_context:context().
check_device_password(DeviceId, Context, ReqData) ->
    Realm = kzd_devices:sip_realm(ReqData, cb_context:account_realm(Context)),
    Username = kzd_devices:sip_username(ReqData),
...
check_sip_creds_unique(AccountId, Realm, Username, DeviceId) ->
    are_creds_locally_unique(AccountId, Username, DeviceId)
        andalso are_creds_global_unique(Realm, Username, DeviceId).

-spec are_creds_locally_unique(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) -> boolean().
are_creds_locally_unique(AccountId, Username, DeviceId) ->
...
-spec are_creds_global_unique(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) -> boolean().
are_creds_global_unique(Realm, Username, DeviceId) ->
...
on_successful_validation('undefined', Context) ->
    Props = [{<<"pvt_type">>, kzd_devices:type()}],
...
%%------------------------------------------------------------------------------
%% @doc Verify that if call failover is enabled, the call forward failover is non-
%% empty.
%% @end
%%------------------------------------------------------------------------------
-spec validate_call_failover(kz_term:api_ne_binary(), cb_context:context()) -> cb_context:context().
validate_call_failover(_DeviceId, Context) ->
    ReqData = cb_context:req_data(Context),
    CallFailover = kzd_devices:call_failover(ReqData, kz_json:new()),
    CallFailoverNumber = kzd_call_failover:number(CallFailover),
    case kzd_call_failover:enabled(CallFailover)
        andalso not kz_term:is_ne_binary(CallFailoverNumber)
    of
        'true' ->
            Msg = kz_json:from_list(
                    [{<<"message">>, <<"Call failover number cannot be empty when call failover is enabled">>}
                    ,{<<"cause">>, CallFailoverNumber}
                    ]),
            cb_context:add_validation_error([<<"call_failover.number">>]
                                           ,<<"required">>
                                           ,Msg
                                           ,Context
                                           );
...
%% @end
%%------------------------------------------------------------------------------
-spec validate_call_forward(kz_term:api_ne_binary(), cb_context:context()) -> cb_context:context().
validate_call_forward(_DeviceId, Context) ->
    case kzd_devices:call_forward(cb_context:req_data(Context)) of
        'undefined' -> Context;
        CallForward -> validate_call_forward(_DeviceId, CallForward, Context)
    end.

-spec validate_call_forward(kz_term:api_ne_binary(), kz_json:object(), cb_context:context()) -> cb_context:context().
validate_call_forward(_DeviceId, CallForward, Context) ->
    case kzd_call_forward:validate(CallForward) of
        'ok' -> Context;
        Errors -> cb_context:add_doc_validation_errors(Context, Errors)
    end.
...
was_mdn_changed_and_user_is_super_admin(Context, NewMDN) ->
    ...
    kzd_devices:mobile_mdn(cb_context:fetch(Context, 'db_doc')) =/= NewMDN
...
add_mdn(MDN, Context) ->
    ...
    MobileField =
        kz_json:from_list(
          [{<<"provider">>, kapps_config:get_ne_binary(?CONFIG_CAT, <<"mdn_provider">>, <<"tower-of-power">>)}
    ...
    Options = props:set_values([{'assign_to', cb_context:account_id(Context)}
                               ,{'public_fields', PublicFields}
                               ,{'module_name', ?CARRIER_MDN}
                               ,{'dry_run', 'false'}
                               | knm_options:mdn_options()
                               ]
                              ,cb_phone_numbers:default_knm_options(Context)
                              ),
    ...
    case knm_numbers:create(MDN, Options) of
        {'ok', [_JObj]} ->
            ...
            Context;
        Else ->
            lager:info("failed to create ~s: ~p", [MDN, Else]),
...
remove_mdn(MDN, Context) ->
    case knm_numbers:get(MDN, knm_options:mdn_options()) of
        {'ok', [JObj]} ->
            %% FIXME: ?CARRIER_MDN without knm_ maybe?
            IsMdnCarrier = <<"mdn">> =:= kz_json:get_ne_binary_value([<<"_read_only">>, <<"carrier_module">>], JObj),
            case kz_json:get_ne_value(<<"mobile">>, JObj) of
                ...
                Mobile ->
                    ...
                    _ = knm_numbers:delete(MDN, knm_options:mdn_options()),
                    Context
            end;
        _Else ->
            lager:debug("unable to fetch mdn ~s for removal: ~p", [MDN, _Else]),
            Context
    end.
...
sync_sip_data(Context, UseTheForce) ->
    ...
    case UseTheForce of
        'false' -> provisioner_util:sync_device(AccountId, OldDoc, NewDoc, ShouldReboot);
        'true' -> provisioner_util:force_sync_device(AccountId, NewDoc, ShouldReboot)
    end.
