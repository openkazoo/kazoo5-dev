...
read(Context, CallId) ->
    case channel_query(cb_context:account_id(Context), CallId) of
        {'error', 'not_found'} ->
...
-spec channel_query(kz_term:ne_binary(), kz_term:ne_binary()) ->
          {'ok', kz_json:object()} |
          {'returned', kz_json:object()} |
          {'error', 'not_found' | 'timeout' | any()}.
channel_query(AccountId, CallId) ->
    Req = [{<<"Call-ID">>, CallId}
          ,{<<"Fields">>, <<"all">>}
          ,{<<"Active-Only">>, 'false'}
          | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
    ...
    Resps = kz_amqp_worker:call_collect(Req
                                       ,fun kapi_call:publish_query_channels_req/1
                                       ,{'ecallmgr', fun custom_query_channels_resp_v/1, 'true'}
                                       ),
    handle_query_resps(AccountId, CallId, Resps).

handle_query_resps(_AccountId, CallId, {'ok', []}) ->
    lager:debug("no channel resp for ~s", [CallId]),
    {'error', 'not_found'};
handle_query_resps(AccountId, CallId, {'ok', StatusJObjs}) ->
    case find_channel(AccountId, CallId, StatusJObjs) of
        'undefined' ->
            lager:warning("trying to get info about a channel ~s not in the account ~s"
                         ,[CallId, AccountId]
                         ),
            {'error', 'not_found'};
        Channel ->
...
handle_query_resps(_AccountId, _CallId, {'returned', JObj, _BR}) ->
    lager:debug("return: ~p", [_BR]),
    {'returned', JObj};
handle_query_resps(_AccountId, _CallId, {'timeout', _Resp}) ->
    lager:debug("timeout: ~p", [_Resp]),
    {'error', 'timeout'};
handle_query_resps(_AccountId, _CallId, {'error', _E}=Error) ->
    lager:debug("error: ~p", [_E]),
    Error.

-spec custom_query_channels_resp_v(kz_term:api_terms()) -> boolean().
custom_query_channels_resp_v(Resp) when is_list(Resp) ->
    kapi_call:query_channels_resp_v(Resp)
        andalso not kz_term:is_empty(props:get_value(<<"Channels">>, Resp));
custom_query_channels_resp_v(Resp) ->
    custom_query_channels_resp_v(kz_json:to_proplist(Resp)).
...
maybe_execute_command(Context, CallId, <<"start_record">>) ->
    maybe_start_record(Context, CallId);
maybe_execute_command(Context, CallId, <<"stop_record">>) ->
    maybe_stop_record(Context, CallId);
...
user_summary(Context, UserId) ->
    maybe_get_user_channels(Context, user_endpoints(Context, UserId)).

-spec maybe_get_user_channels(cb_context:context(), kz_json:objects()) -> cb_context:context().
maybe_get_user_channels(Context, []) ->
...
-spec user_endpoints(cb_context:context(), kz_term:ne_binary()) -> kz_json:objects().
user_endpoints(Context, UserId) ->
    Options = [{'key', [UserId, <<"device">>]}
              ,'include_docs'
              ],
    case kz_datamgr:get_results(cb_context:db_name(Context), <<"attributes/owned">>, Options) of
        {'ok', JObjs} -> JObjs;
        {'error', _Reason} ->
            lager:debug("failed to list user '~s' devices: ~p", [UserId, _Reason]),
            []
    end.
...
group_endpoints_fold(EndpointId, EndpointData, {Acc, Context}) ->
    case kz_json:get_value(<<"type">>, EndpointData) of
        <<"user">> ->
            EPs = user_endpoints(Context, EndpointId),
            {EPs ++ Acc, Context};
        <<"device">> ->
...
%% Kazoo dials `device_id'(or all devices for `owner_id')
%% and if a device answers the call it will be bridged with a remote channel
...
maybe_move(Context, CallId) ->
    Data = kz_json:from_list([{<<"auto_answer">>, cb_context:req_value(Context, <<"auto_answer">>, 'false')}
                             ...
                             ,{<<"timeout">>, cb_context:req_value(Context, <<"timeout">>)}
                             ]),
...
%%------------------------------------------------------------------------------
%% @doc API action for starting or stopping call recording for a channel.
%% For now, I do not check whether the call recording is enabled or not.
%% @end
%%------------------------------------------------------------------------------
-spec maybe_start_record(cb_context:context(), kz_term:ne_binary()) -> cb_context:context().
maybe_start_record(Context, CallID) ->
    API = [{<<"Action">>, <<"record_call">>}
          ,{<<"Call-ID">>, CallID}
          ,{<<"Data">>, kz_json:from_list([{<<"action">>, <<"start">>},
    ...
    lager:notice("attempting to start the call recording~s", [CallID]),
    kz_amqp_worker:cast(API, fun kapi_metaflow:publish_action/1),
    crossbar_util:response_202(<<"recording started">>, Context).

-spec maybe_stop_record(cb_context:context(), kz_term:ne_binary()) -> cb_context:context().
maybe_stop_record(Context, CallID) ->
    API = [{<<"Action">>, <<"record_call">>}
          ,{<<"Call-ID">>, CallID}
          ,{<<"Data">>, kz_json:from_list([{<<"action">>, <<"stop">>}
                                          ])}
          | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
          ],

    lager:notice("attempting to stop the call recording ~s", [CallID]),
    kz_amqp_worker:cast(API, fun kapi_metaflow:publish_action/1),
    crossbar_util:response_202(<<"recording stopped">>, Context).
