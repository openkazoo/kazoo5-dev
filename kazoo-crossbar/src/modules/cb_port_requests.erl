...
-define(PATH_LNP_LOOKUP, <<"lnp_lookup">>).
...
allowed_methods(?PATH_TOKEN_LAST_SUBMITTED) ->
    [?HTTP_GET];
allowed_methods(?PATH_LNP_LOOKUP) ->
    [?HTTP_POST];
...
validate(Context, ?PATH_LNP_LOOKUP) ->
    cb_context:validate_request_data(<<"lnp_lookup_req">>, Context, fun validate_lnp_lookup/1);
validate(Context, Id) ->
...
get(Context, Id, ?PATH_TOKEN_LOA) ->
    ...
    cb_context:set_resp_data(cb_context:set_resp_status(Context, 'success')
                            ,kz_json:encode(cb_context:doc(Context))
                            ).
...
put(Context) ->
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            Metadata = knm_port_request:read_only_public_fields(Doc),
            cb_context:setters(Context1
                              ,[{fun cb_context:set_resp_data/2, knm_port_request:public_fields(Doc)}
                               ,{fun cb_context:add_metadata_values/2, kz_json:to_proplist(Metadata)}
                               ]);
        _ ->
            Context1
    end.
...
post(Context, ?PATH_LNP_LOOKUP) ->
    case cb_modules_util:phonebook_lnp_lookup(Context) of
        {'ok', 'disabled'} ->
            lager:debug("phonebook is disabled"),
            cb_context:add_system_error('datastore_fault', Context);
        {'ok', Response} ->
            Data = kz_json:get_value(<<"data">>, Response, kz_json:new()),
            crossbar_doc:handle_datamgr_success(Data, Context);
        {'error', Error} ->
            handle_phonebook_error(Context, Error)
    end;
post(Context, Id) ->
...
handle_phonebook_error(Context, #{code := Code
    ...
    CommentText = kz_term:to_binary(
                    ["Carrier rejected port request submission. Phonebook error:\n"
    ...
    Options = [{'is_phonebook', 'true'}],
    Context1 = move_state(Context, ?PORT_REJECTED, Metadata, Options),
    case cb_context:resp_status(Context1) of
        'success' ->
            Doc = cb_context:doc(Context1),
        ...
        _ ->
            lager:warning("failed to move port to rejected state, halp"),
            Context1
    end;
handle_phonebook_error(Context, #{code := Code
                                 ,payload := Payload
    ...
    Setters = [{fun cb_context:set_resp_error_code/2, Code}
              ...
              ,{fun cb_context:set_resp_error_msg/2, phonebook_error_message(Error)}
...
handle_phonebook_error(Context, #{payload := Payload}=_Error) ->
    cb_context:add_system_error('datastore_fault', kz_json:get_value(<<"message">>, Payload), Context).

phonebook_error_message(#{req_type := 'lnp_lookup'}) ->
    <<"unable to submit lnp lookup request to carrier">>;
phonebook_error_message(#{req_type := 'add_comment'}) ->
    <<"unable to submit comment to carrier">>;
phonebook_error_message(_) ->
    <<"request is rejected by port automation system">>.
...
patch_then_validate_then_maybe_transition(Context, PortId, ToState) ->
    ...
    %% patch_and_validate always calls ValidateFun even if it fails to load the doc!
    crossbar_doc:patch_and_validate(PortId, set_port_authority(Context1), ValidateFun, LoadOptions).

-spec validate_lnp_lookup(cb_context:context()) -> cb_context:context().
validate_lnp_lookup(Context) ->
    JObj = cb_context:req_data(Context),
    Numbers = [knm_converters:normalize(Number)
               || Number <- kzd_lnp_lookup_req:numbers(JObj)
              ],
    crossbar_doc:handle_datamgr_success(kzd_lnp_lookup_req:set_numbers(JObj, Numbers), Context).
...
validate_port_comments(Context, OnSuccess) ->
    %% this function is not guaranties to be called ONLY if resp_status is success,
    %% read the caller funs comment, we may end up with failed load and if validation passes, this function will be
    %% called. We have no way to check if load in patch_and_validate was successful or not.
...
load_summary_by_types(Context, [Type | Types], JObjs) ->
    maybe_continue_loading(Context, Types, JObjs, load_summary_by_type(Context, Type)).

maybe_continue_loading(OriginalContext, Types, JObjs, LoadedContext) ->
    case cb_context:resp_status(LoadedContext) of
        'success' ->
            RespData = cb_context:resp_data(LoadedContext),
            load_summary_by_types(OriginalContext, Types, JObjs ++ RespData);
        _Failed -> LoadedContext
    end.

-spec load_summary_by_type(cb_context:context(), kz_term:ne_binary()) -> cb_context:context().
load_summary_by_type(Context, Type) ->
    lager:debug("loading summary for port requests with type ~s", [Type]),
    ...
    Options = summary_by_type_options(Context, Type, IsRanged, AuthorityType),
    case IsRanged of
        'true' -> crossbar_view:load_time_range(Context, View, Options);
        'false' -> crossbar_view:load(Context, View, Options)
    end.

summary_by_type_options(Context, Type, IsRanged, AuthorityType) ->
...
load_summary_by_number(Context, Number) ->
    Options = summary_by_number_options(Number),
    crossbar_view:load(Context, ?LISTING_BY_NUMBER, Options).

summary_by_number_options(Number) ->
    [{'keymap', knm_converters:normalize(Number)}
...
last_submitted(Context) ->
    Options = last_submitted_options(Context),
    crossbar_view:load(Context, <<"port_requests/listing_submitted">>, Options).

last_submitted_options(Context) ->
...
normalize_view_results(ViewResult, Acc) ->
    Doc = kz_json:get_json_value(<<"doc">>, ViewResult),
    Normalized = knm_port_request:public_fields(Doc),
    [add_metadata_fields(Doc, Normalized)
    | Acc
    ].

-spec add_metadata_fields(kz_json:object(), kz_json:object()) -> kz_json:object().
add_metadata_fields(Doc, PublicJObj) ->
    kz_json:set_value(<<"account_id">>, kz_doc:account_id(Doc), PublicJObj).
...
load_attachment(AttachmentId, Context) ->
    ...
    Headers =
        #{<<"content-disposition">> => <<"attachment; filename=", AttachmentId/binary>>
         ,<<"content-type">> => kz_doc:attachment_content_type(cb_context:doc(Context1), AttachmentId)
         },
...
-spec move_state(cb_context:context(), kz_term:ne_binary(), knm_port_request:transition_metadata(), kz_term:proplist()) -> cb_context:context().
move_state(Context, PortState, Metadata, Options0) ->
    Options = [{'port_authority_id', cb_context:fetch(Context, 'port_authority_id')}
              | Options0
              ],
    try knm_port_request:attempt_transition(cb_context:doc(Context), Metadata, PortState, Options) of
        {'ok', PortRequest} ->
            lager:debug("loaded new port request state ~s", [PortState]),
            cb_context:set_resp_status(cb_context:set_doc(Context, PortRequest), 'success');
