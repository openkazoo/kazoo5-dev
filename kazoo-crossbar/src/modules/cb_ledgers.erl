...
put(Context, Action) ->
    ...
    Setters =
        props:filter_empty(
          ...
          ,{fun kz_ledger:set_draft/2
           ,kz_json:is_true(<<"draft">>, ReqData)
           }
          ]
         ),
...
%% @end
%%------------------------------------------------------------------------------
-spec normalize_view_results(kzd_ledgers:doc(), kz_json:objects()) ->
          kz_json:objects().
...
%% @end
%%------------------------------------------------------------------------------
-spec normalize_ledger_jobj(kazoo_data:key_range(), kz_json:object()) -> kz_json:object().
