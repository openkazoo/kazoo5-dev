...
-define(COLLECTION, ?NUMBERS_COLLECTION). %% ?NUMBERS_COLLECTION is defined within crossbar.hrl file.
...
validate(Context, _Number, ?ACTIVATE) ->
    BucketName = <<(cb_context:account_id(Context))/binary, "/", ?PHONE_NUMBERS_CONFIG_CAT/binary>>,
    Cost = cb_modules_util:token_cost(Context, 1),
    case cb_modules_util:consume_tokens(Context, Cost, BucketName) of
        {'true', Context1} ->
            validate_request(Context1);
        {'false', Context1} ->
...
post(Context, ?COLLECTION) ->
    ...
    Opts = [{'assign_to', cb_context:account_id(Context)}
           | default_knm_options(Context)
           ],
    Options = force_collection_threshold(Context, Opts, Numbers),
    ...
    set_response(Results, Context, CB, props:is_true('batch_run', Options, 'false'));
...
put(Context, ?COLLECTION) ->
    {Numbers, ReqData} = take_collection_numbers(Context),
    Opts = props:set_values([{'assign_to', cb_context:account_id(Context)}
...
delete(Context, ?COLLECTION) ->
    ...
    Opts = default_knm_options(Context),
    Options = force_collection_threshold(Context, Opts, Numbers),
    CB = fun() -> ?MODULE:delete(cb_context:set_accepting_charges(Context), ?COLLECTION) end,
    set_response(release_or_delete(Context, Numbers, Options), Context, CB, props:is_true('batch_run', Options, 'false'));
delete(Context, Number) ->
    ...
    case release_or_delete(Context, Number, Options) of
        ...
        {'ok', [], [{_, Error}]}=Result ->
            ...
            of
                'true' -> reply_number_not_found(Number, Context);
...
-spec maybe_find_port_number(cb_context:context(), kz_term:ne_binary(), boolean()) ->
          cb_context:context().
maybe_find_port_number(Context, Number, 'false') ->
    reply_number_not_found(Number, Context);
maybe_find_port_number(Context, Number, 'true') ->
    ...
    case kz_datamgr:get_single_result(?KZ_PORT_REQUESTS_DB, ?PORT_NUM_LISTING, ViewOption) of
        {'error', _} -> reply_number_not_found(Number, Context);
        {'ok', Result} ->
            Port = kz_json:get_value(<<"value">>, Result),
            {'ok', PN} = normalize_port_number(Port, Num, cb_context:auth_account_id(Context)),
            Values = [{<<"port_id">>, kz_json:get_value(<<"port_id">>, Port)}
                     ,{<<"port_state">>, kz_json:get_value(<<"port_state">>, Port)}
                     ],
            port_number_summary(cb_context:add_metadata_values(Context, Values)
                               ,knm_phone_number:to_public_json(PN)
                               ,knm_phone_number:is_authorized(PN)
                               )
    end.

-spec port_number_summary(cb_context:context(), kz_json:object(), boolean()) -> cb_context:context().
port_number_summary(Context, PhoneNumberJObj, 'true') ->
    crossbar_doc:handle_datamgr_success(PhoneNumberJObj, Context);
port_number_summary(Context, PhoneNumberJObj, 'false') ->
    reply_number_not_found(PhoneNumberJObj, Context).
...
find_numbers(Context, AccountId, ResellerId) ->
    ...
    Options = props:filter_undefined(
                ...
                ,{'is_reseller', cb_context:is_reseller(Context)}
...
validate_collection_request(Context, Numbers)
  when is_list(Numbers) ->
    UniqNormalized = lists:usort([knm_converters:normalize(N) || N <- Numbers]),
    case length(Numbers) =:= length(UniqNormalized) of
...
identify(Context, Num) ->
    case knm_numbers:lookup_account(Num) of
        ...
        {'error', 'not_found'} ->
            reply_number_not_found(Num, Context);
        {'error', Reason} ->
...
set_response(Result, Context, CB) ->
    set_response(Result, Context, CB, 'undefined').

-spec set_response(knm_numbers:return(), cb_context:context(), cb(), kz_term:api_boolean()) -> cb_context:context().
set_response({'ok', [JObj]}, Context, _, 'undefined') ->
    %% single number operation
    crossbar_doc:handle_datamgr_success(JObj, Context);
set_response({'ok', JObjs}, Context, _, BatchRun) ->
    %% collection operation (multi numbers)
    Resp = kz_json:from_list(
             [{<<"success">>, kz_json:from_list([{kz_doc:id(JObj), JObj} || JObj <- JObjs])}
             ,{<<"error">>, []}
             ]),
    maybe_reconcile(Context, BatchRun),
    crossbar_util:response(Resp, Context);
set_response({'ok', [], [{Number, Failed}]}, Context, _, 'undefined') ->
    %% single number operation
    set_error(Number, Failed, Context);
set_response({'ok', Success, FailedProp}, Context, _, BatchRun) ->
    %% collection operation (multi numbers)
    Resp = kz_json:from_list(
             [{<<"success">>, kz_json:from_list([{kz_doc:id(JObj), JObj} || JObj <- Success])}
             ,{<<"error">>, normalize_knm_error(FailedProp)}
             ]),
    maybe_reconcile(Context, BatchRun),
    crossbar_util:response(Resp, Context);
set_response({'dry_run', Quotes}, Context, CB, _BatchRun) ->
    case kz_term:is_empty(Quotes) of
        'false' -> crossbar_util:response_402(Quotes, Context);
        'true' -> CB()
    end.

maybe_reconcile(Context, 'true') ->
    %% batch_run was true, so knm was ignoring services, reconciling it now
    _ = kz_process:spawn(fun crossbar_services:reconcile/1, [Context]),
    lager:debug("executing service reconcilating update in the background");
maybe_reconcile(_Context, _) ->
    %% false or undefined
    'ok'.

-spec normalize_knm_error(knm_errors:proplist()) -> kz_json:object().
normalize_knm_error(FailedProp) ->
    kz_json:from_list(
      lists:map(fun({Number, Error}) when is_atom(Error) ->
                        {Number, knm_errors:to_json(Error, Number)};
                   (Other) -> Other
                end
               ,FailedProp
               )
...
-spec set_error(kz_term:ne_binary(), knm_errors:reason(), cb_context:context()) -> cb_context:context().
set_error(Number, 'not_found', Context) ->
    reply_number_not_found(Number, Context);
set_error(Number, Reason, Context) when is_atom(Reason) ->
    Data = knm_errors:to_json(Reason, Number),
...
-spec reply_number_not_found(kz_term:ne_binary() | kz_json:object(), cb_context:context()) ->
          cb_context:context().
reply_number_not_found(<<Number/binary>>, Context) ->
...
reply_number_not_found(PhoneNumberJObj, Context) ->
    reply_number_not_found(kz_doc:id(PhoneNumberJObj), Context).
...
%% @end
%%------------------------------------------------------------------------------
-spec force_collection_threshold(cb_context:context(), knm_options:options(), kz_term:ne_binaries()) -> knm_options:options().
force_collection_threshold(_Context, Options, Numbers) when is_list(Numbers) ->
    case collection_threshold() of
        'infinity' ->
            Options;
        Threshold ->
            case length(Numbers) >= Threshold of
                'true' ->
                    lager:info("collection length is bigger than threshold ~b, forcing 'batch_run' and 'dry_run'", [Threshold]),
                    Force = [{'dry_run', 'false'} %% avoid crossbar repeating the request with setting accept_charging again
                            ,{'batch_run', 'true'} %% skip cache reads and directly fetch from db
                            ],
                    props:set_values(Force, Options);
                'false' ->
                    Options
            end
    end.

-spec collection_threshold() -> timeout().
collection_threshold() ->
    case kapps_config:get(?PHONE_NUMBERS_CONFIG_CAT, <<"collection_batch_run_threshold">>) of
        'undefined' -> 'infinity';
        'infinity' -> 'infinity';
        <<"infinity">> -> 'infinity';
        IntegerIHope -> kz_term:safe_cast(IntegerIHope, 'infinity', fun kz_term:to_integer/1)
    end.
