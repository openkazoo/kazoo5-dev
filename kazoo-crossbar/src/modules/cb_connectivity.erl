...
post(Context, _) ->
    ...
    case kzd_trunkstore:save(Doc) of
        {'ok', Saved} ->
            C1 = crossbar_doc:handle_datamgr_success(Saved, Context),
            track_assignment('post', C1),
            C1;
        {'error', Error} ->
            lager:error("trunkstore save => ~p", [Error]),
...
put(Context) ->
    ...
    case kzd_trunkstore:save(Doc) of
        {'ok', Saved} ->
            ...
            crossbar_doc:handle_datamgr_success(Saved, Context);
        {'error', Error} ->
            lager:error("trunkstore save => ~p", [Error]),
            cb_context:add_system_error('datastore_fault', Context)
    end.
...
delete(Context, Id) ->
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            _ = kzd_trunkserver:delete(cb_context:account_id(Context), Id),
...
read(Id, Context) ->
    crossbar_doc:load(Id, Context, ?TYPE_CHECK_OPTION(kzd_connectivity:type())).
...
on_successful_validation('undefined', Context) ->
    Doc = kz_doc:set_type(cb_context:doc(Context), kzd_connectivity:type()),
    cb_context:set_doc(Context, Doc);
on_successful_validation(Id, Context) ->
    crossbar_doc:load_merge(Id, Context, ?TYPE_CHECK_OPTION(kzd_connectivity:type())).
...
summary(Context) ->
    Options = [{'doc_type', kzd_connectivity:type()}
              ,{'mapper', crossbar_view:get_id_fun()}
              ],
    Selector = [{'start', [{<<"doc_type">>, kzd_connectivity:type()}]}
               ,{'end', [{<<"doc_type">>, kzd_connectivity:type()}]}
               ],
