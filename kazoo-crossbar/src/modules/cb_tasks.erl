...
put(Context) ->
    ...
    of
        {'ok', TaskJObj} ->
            TaskId = kz_doc:id(TaskJObj),
            _SavedContext = save_attached_data(set_db(Context), TaskId, CSVorJSON, IsCSV),
            crossbar_util:response(kz_json:new(), cb_context:add_metadata_values(Context, kz_json:to_proplist(TaskJObj)));
...
patch(Context, TaskId) ->
    ...
    Resp = kz_amqp_worker:call(Req
                              ,fun kapi_tasks:publish_start_req/1
                              ,fun kapi_tasks:start_resp_v/1
                              ),
    handle_resp(Context, TaskId, Resp).

-spec handle_resp(cb_context:context(), path_token(), kz_amqp_worker:request_return()) ->
          cb_context:context().
handle_resp(Context, TaskId, {'ok', Resp}) ->
    case kz_json:get_value(<<"Reply">>, Resp) of
        <<"already_started">> ->
            Msg = kz_json:from_list([{<<"reason">>, <<"task is started and running">>}
                                    ,{<<"cause">>, TaskId}
                                    ]),
...
handle_resp(Context, _, {'returned', _Request, BasicReturn}) ->
    lager:debug("no tasks apps running: ~p", [BasicReturn]),
    crossbar_util:response('error', <<"The backend is not configured for this request">>, 500, Context);
handle_resp(Context, _, {'timeout', _Timeout}) ->
    lager:debug("timed out waiting for a response: ~p", [_Timeout]),
    crossbar_util:response_datastore_timeout(Context);
handle_resp(Context, _, {'error', 'timeout'}) ->
    lager:debug("timed out waiting for a response"),
    crossbar_util:response_datastore_timeout(Context);
handle_resp(Context, _, {'error', _E}) ->
...
read_doc(TaskId, Context, AccountId) ->
    ...
    ReadResult = crossbar_view:load(Context, ?KZ_TASKS_BY_ACCOUNT, Options),
    handle_read_result(TaskId, Context, ReadResult).
    ...
    case cb_context:resp_data(ReadContext) of
        ...
        [TaskJObj] ->
            ...
            cb_context:setters(ReadContext
                              ,[{fun cb_context:set_doc/2, TaskJObj}
                               ,{fun cb_context:set_resp_data/2, kz_json:new()}
                               ,{fun cb_context:add_metadata_values/2, kz_json:to_proplist(TaskJObj)}
                               ]
                              );
        ErrorResp ->
            lager:info("task fetching failed: ~p", [ErrorResp]),
            cb_context:add_system_error('datastore_fault', ErrorResp, ReadContext)
    end.
...
summary(Context) ->
    crossbar_view:load(Context, ?KZ_TASKS_BY_CREATED, summary_view_options(Context)).

-spec summary_view_options(cb_context:context()) -> crossbar_view:options().
summary_view_options(Context) ->
    AccountId = cb_context:account_id(Context),
    [{'startkey', [AccountId]}
    ,{'endkey', [AccountId, kz_time:now_s(), kz_json:new()]}
    ,{'databases', [?KZ_TASKS_DB]}
    ,{'mapper', crossbar_view:get_value_fun()}
    ,'descending'
    ].
...
save_attached_data(Context, TaskId, Records, 'false') ->
    ...
    CSV = kz_term:to_binary(kz_csv:from_jobjs(Records)),
    lager:debug("saving ~s attachment in task ~s", [?KZ_TASKS_ANAME_IN, TaskId]),
