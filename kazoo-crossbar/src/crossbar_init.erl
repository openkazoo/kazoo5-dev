...
%% types lifted from corwbow_router
...
-type dispatch_rules() :: {'persistent_term', 'crossbar_dispatch'}. % cowboy_router:dispatch_rules()

-spec crossbar_routes() -> cowboy_router:routes().
crossbar_routes() ->
    [{'_', paths_list()}].

-spec paths_list() -> [route_path()].
paths_list() ->
    %% api first, ui_wrapper, customs then default catch all path
    [api_path(), ui_wrapper_path()] ++ lists:uniq(custom_paths()) ++ [default_path()].

-spec ui_wrapper_path() -> route_path().
ui_wrapper_path() ->
    {crossbar_ui_wrapper_handler:route_path_match(), 'crossbar_ui_wrapper_handler', []}.

-spec default_path() -> route_path().
default_path() ->
    {'_', 'crossbar_default_handler', #{body => [kazoo_bytes(), "\n\n", deps_bytes()]}}.

kazoo_bytes() ->
    {'ok', KAZOOBytes} = file:read_file(filename:join(code:priv_dir(?APP), "kazoo.txt")),
    KAZOOBytes.

deps_bytes() ->
    deps_bytes(kapps_config:get_is_true(?CONFIG_CAT, <<"display_dependency_list">>, 'true')).

deps_bytes('false') -> <<>>;
deps_bytes('true') ->
    case file:read_file(filename:join(code:priv_dir(?APP), "deps_list.txt")) of
        {'ok', DepsBytes} -> DepsBytes;
        {'error', _} -> <<>>
    end.

-spec custom_paths() -> [route_path()].
custom_paths() ->
    lists:foldl(fun try_get_custom_path/2
               ,[]
               ,kapps_config:get_ne_binaries(?CONFIG_CAT, <<"cowboy_custom_routes_handlers">>, [])
               ).

-spec try_get_custom_path(kz_term:ne_binary(), [route_path()]) -> [route_path()].
try_get_custom_path(CustomHandlerMod, Acc) ->
    case kz_module:is_exported(CustomHandlerMod, 'route_paths', 0) of
        'true' ->
            do_try_get_custom_path(kz_term:to_atom(CustomHandlerMod), Acc);
        'false' ->
            lager:debug("unabled to get custom paths from ~s: route_paths/0 is not exported", [CustomHandlerMod]),
            Acc
    end.

-spec do_try_get_custom_path(atom(), [route_path()]) -> [route_path()].
do_try_get_custom_path(CustomHandlerMod, Acc) ->
    try CustomHandlerMod:route_paths() of
        Paths -> Acc ++ Paths
    catch
        _E:_T:_ST ->
            lager:debug("failed to get custom paths from ~s: ~p", [CustomHandlerMod, {_E,_T}]),
            Acc
    end.

-spec api_path() -> route_path().
api_path() ->
    {<<"/:version/[...]">>, [api_version_constraint()], 'api_resource', []}.

-spec api_version_constraint() -> {'version', cowboy_constraints:constraint()}.
...
start_link() ->
    ...
    persistent_term:put('crossbar_dispatch', Dispatch),
    ...
    maybe_start_plaintext({'persistent_term', 'crossbar_dispatch'}, IPAddress),
    maybe_start_ssl({'persistent_term', 'crossbar_dispatch'}, IPAddress),
    'ignore'.
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_start_plaintext(dispatch_rules(), inet:ip_address()) -> 'ok'.
...
-spec maybe_start_ssl(dispatch_rules(), inet:ip_address()) -> 'ok'.
...
-spec start_ssl(dispatch_rules(), inet:ip_address()) -> 'ok'.
