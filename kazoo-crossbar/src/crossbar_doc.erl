...
-export([handle_json_success/2, handle_json_success/3]).
-export([handle_successful_load/2, handle_successful_load/3, handle_successful_load/4
...
-type doc_normalizer_fun() :: fun((kz_json:object()) -> kz_json:object()).
-type load_option() :: {?OPTION_EXPECTED_TYPE, kz_term:ne_binary()} |
                       {'use_cache', boolean()} |
                       {'doc_normalizer', doc_normalizer_fun()}.
    ...
    case maybe_open_cache_doc(cb_context:db_name(Context), DocId, Options) of
        ...
        {'ok', JObj} ->
            case check_document_type(Context, JObj, Options) of
                'true' ->
                    JObj1 = maybe_normalize_doc(JObj, props:get_value('doc_normalizer', Options)),
                    handle_successful_load(Context, JObj1, Options);
                'false' ->
...
-spec maybe_normalize_doc(kz_json:object(), doc_normalizer_fun()) ->
          kz_json:object().
maybe_normalize_doc(JObj, 'undefined') -> JObj;
maybe_normalize_doc(JObj, Fun) -> Fun(JObj).
...
-spec patch_and_validate_doc(kz_term:ne_binary(), cb_context:context(), validate_fun()) ->
          cb_context:context().
patch_and_validate_doc(Id, Context, ValidateFun) ->
    patch_and_validate_doc(Id, Context, ValidateFun, cb_context:resp_status(Context)).
...
patch_and_validate_doc(Id, Context, ValidateFun, _RespStatus) ->
    %% FIXME: Why calling ValidateFun when load has failed?
...
load_attachment(<<DocId/binary>>, AName, AttachmentOptions, Context) ->
    Options = props:delete_keys(['extra_resp_headers'], AttachmentOptions),
    case kz_datamgr:fetch_attachment(cb_context:db_name(Context), DocId, AName, Options) of
        {'error', Error} ->
            handle_datamgr_errors(Error, <<DocId/binary, "/", AName/binary>>, Context);
        {'ok', AttachBin} ->
            lager:debug("loaded attachment ~s from doc ~s from db ~s"
                       ,[AName, DocId, cb_context:db_name(Context)]
                       ),
            Context1 = load(DocId, Context, Options),
            attachment_response(Context1, DocId, AttachBin, AName, AttachmentOptions, cb_context:resp_status(Context1))
    end;
load_attachment(Doc, AName, Options, Context) ->
    load_attachment({kz_doc:type(Doc), kz_doc:id(Doc)}, AName, Options, Context).

-spec attachment_response(cb_context:context(), kz_term:ne_binary(), binary(), kz_term:ne_binary(), kz_term:proplist(), crossbar_status()) ->
          cb_context:context().
attachment_response(Context, DocId, AttachBin, AName, Options, 'success') ->
    Headers = attachment_resp_headers(Context, DocId, AName, Options),
    cb_context:setters(Context
                      ,[{fun cb_context:set_resp_data/2, AttachBin}
                       ,{fun cb_context:set_resp_etag/2, rev_to_etag(cb_context:doc(Context))}
                       ,{fun cb_context:add_resp_headers/2, Headers}
                       ]);
attachment_response(Context, _, _, _, _, _) ->
    Context.

-spec attachment_resp_headers(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
          cowboy:http_headers().
attachment_resp_headers(Context, DocId, AName, Options) ->
    Headers = maps:from_list(props:get_value('extra_resp_headers', Options, [])),
    CT = kz_doc:attachment_content_type(cb_context:doc(Context)
                                       ,AName
                                       ,maps:get(<<"content-type">>, Headers, <<"application/octet-stream">>)
                                       ),
    Medianame = case kz_mime:to_extension(CT) of
                    <<>> -> DocId;
                    Ext -> <<DocId/binary, ".", Ext/binary>>
                end,
    CD = maps:get(<<"content-disposition">>, Headers, <<"attachment; filename=", Medianame/binary>>),
...
save_attachment(DocId, Name, Contents, Context, Options) ->
    ...
    case kz_datamgr:put_attachment(cb_context:db_name(Context), DocId, AName, Contents, Opts1) of
        {'error', 'conflict'} ->
            ...
            %% flushing the cache to force fetching the new doc
            %% and making sure the cache is updated. Next get attachment request (if any) then
            %% have the correct document to fetch attachments.
            kzs_cache:flush_cache_doc(cb_context:db_name(Context), DocId),
            Context1 = load(DocId, Context, props:delete('content_type', Options)),
            check_attachment_conflict(Context1, AName, Options, cb_context:resp_status(Context1));
        {'error', Error} ->
            lager:debug("error putting attachment into ~s: ~p"
                       ,[cb_context:db_name(Context), Error]
                       ),
            maybe_delete_doc(Context, DocId, Options),
...
-spec check_attachment_conflict(cb_context:context(), kz_term:ne_binary(), kz_term:proplist(), crossbar_status()) ->
          cb_context:context().
check_attachment_conflict(Context, AName, Options, 'success') ->
    case kz_doc:attachment(cb_context:doc(Context), AName) of
        'undefined' ->
            lager:debug("attachment does appear to be missing, reporting error"),
            maybe_delete_doc(Context, cb_context:doc(Context), Options),
            handle_datamgr_errors('conflict', AName, Context);
        _Attachment ->
            lager:debug("attachment ~s was in document, considering it successful", [AName]),
            Context
    end;
check_attachment_conflict(Context, _, _, _) ->
    Context.

-spec maybe_delete_doc(cb_context:context(), kz_term:ne_binary() | kz_json:object(), kz_term:proplist()) -> 'ok'.
maybe_delete_doc(Context, DocOrId, Options) ->
    case {props:is_true('keep_doc', Options, 'false')
         ,kz_json:is_json_object(DocOrId)
         }
    of
        {'true', _} -> 'ok';
        {'false', 'true'} ->
            lager:debug("maybe deleting document ~s if doc has no other attachments", [kz_doc:id(DocOrId)]),
            AccountDb = cb_context:db_name(Context),
            delete_if_empty_attachments(AccountDb, {'ok', DocOrId});
        {'false', 'false'} ->
            lager:debug("maybe deleting document ~s if doc has no other attachments", [DocOrId]),
            AccountDb = cb_context:db_name(Context),
            delete_if_empty_attachments(AccountDb, kz_datamgr:open_doc(AccountDb, DocOrId))
    end.

-spec delete_if_empty_attachments(kz_term:ne_binary(), {'ok', kz_json:object()} | kz_datamgr:data_error()) -> 'ok'.
delete_if_empty_attachments(AccountDb, {'ok', JObj}) ->
    case kz_doc:attachments(JObj) of
        'undefined' ->
            _ = kz_datamgr:del_doc(AccountDb, JObj),
            lager:debug("no attachment for ~s document, deleting doc", [kz_doc:id(JObj)]);
        Attachments ->
            lager:debug("document ~s has ~b attachments, not deleting doc"
                       ,[kz_doc:id(JObj), length(kz_json:get_keys(Attachments))]
                       )
    end;
delete_if_empty_attachments(_AccountDb, {'error', _Reason}) ->
    lager:debug("failed to open doc in ~s and check if it has attachments: ~p"
               ,[_AccountDb, _Reason]
               ).
...
delete(Context, ?HARD_DELETE) ->
    ...
    do_delete(Context, kz_doc:set_deleted(Doc, 'true'), fun kz_datamgr:del_doc/2).
...
-spec rev_to_etag(kz_json:object() | kz_json:objects() | kz_term:ne_binary()) ->
          cb_context:resp_etag().
rev_to_etag(JObjs) when is_list(JObjs) -> 'automatic';
rev_to_etag(Rev) when is_binary(Rev) -> Rev;
rev_to_etag(JObj) -> kz_doc:revision(JObj).
...
handle_json_success(JObj, Context, ?HTTP_DELETE) ->
    PublicJObj = kz_doc:public_fields(JObj),
    cb_context:setters(Context
                      ,[{fun cb_context:set_doc/2, JObj}
                       ,{fun cb_context:set_resp_status/2, 'success'}
                       ,{fun cb_context:set_resp_data/2, PublicJObj}
                       ,{fun cb_context:set_resp_etag/2, rev_to_etag(JObj)}
                       ,{fun cb_context:add_metadata_value/3, <<"deleted">>, 'true'}
                       ]);
...
handle_datamgr_errors('memory_exceeded', _View, Context) ->
...
-spec rm_read_only(kz_json:object(), kz_json:json_proplist(), cb_context:context()) ->
          kz_json:json_proplist().
rm_read_only(JObj, Updates, _Context) ->
    case kz_doc:read_only(JObj, 'undefined') of
        'undefined' -> Updates;
        _ -> [{[<<"_read_only">>], 'null'} | Updates]
    end.
...
extract_included_docs_fold(JObj, {Docs, Context}) ->
    ...
    %% open_docs could return `null' even without error if the doc is hard deleted
