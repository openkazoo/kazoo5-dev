...
-define(DEFAULT_CUSTOM_ROUTES, [<<"schemas">>
                               ,<<"braintree">>
                               ,<<"system_configs">>
                               ,<<"configs">>
                               ]
...
%% The arguments/payload provided to a `request_data' event binding.
-type request_data_args() :: {cowboy_req:req()
                             ,cb_context:context()
                             ,cowboy_content_type()
                             ,kz_json:object()
                             }.

-export_type([pull_response_return/0
             ,req_fun/0
             ,req_fun_1/0
             ,req_fun_2/0
             ,req_fun_3/0
...
-type pull_response_return() :: {pull_response(), cowboy_req:req(), cb_context:context()}.
...
get_query_string_data(QS0, Req) ->
    ...
    lager:debug("query string: ~s", [kz_json:encode(QS)]),
    {QS, Req}.
...
get_req_data(Context, Req0, 'undefined', QS) ->
    lager:debug("undefined content type when getting req data, assuming ~s", [?DEFAULT_CONTENT_TYPE]),
    Ctx = cb_context:set_req_header(Context, <<"content-type">>, ?DEFAULT_CONTENT_TYPE),
    get_req_data(Ctx, Req0, ?DEFAULT_CONTENT_TYPE, QS);
...
maybe_extract_multipart(Context, Req0, QS) ->
    try
        ...
        _Exc:_Reason:ST ->
            lager:debug("failed to extract multipart ~s: ~p", [_Exc, _Reason]),
...
-spec try_json(binary(), kz_json:object(), cb_context:context(), cowboy_req:req()) ->
          {cb_context:context(), cowboy_req:req()} |
          stop_return().
try_json(ReqBody, QS, Context, Req) ->
...
-type cowboy_read_part_response() :: {'ok', cowboy:http_headers(), cowboy_req:req()} |
                                     {'done', cowboy_req:req()} |
                                     cowboy_req:req().

-spec extract_multipart(cb_context:context(), cowboy_read_part_response(), kz_json:object()) ->
          {cb_context:context(), cowboy_req:req()} |
          stop_return().
...
handle_file_contents(Context, ContentType, Req, FileContents) ->
    MultiPartHeaders0 = cb_context:fetch(Context, <<"multipart_headers">>, #{}),
    MultiPartHeaders = maps:merge(MultiPartHeaders0
                                 ,#{<<"content_type">> => ContentType
                                   ,<<"content_length">> =>
                                        maps:get(<<"content-length">>, MultiPartHeaders0, cowboy_req:header(<<"content-length">>, Req))
                                   }
                                 ),
    ...
    case maps:get(TransferKey, MultiPartHeaders, cowboy_req:header(TransferKey, Req)) of
        <<"base64">> ->
            ...
            decode_base64(Context, ContentType, Req, FileContents, MultiPartHeaders);
        _Else ->
            lager:debug("unexpected transfer encoding: '~s'", [_Else]),
            FileHeaders = kz_json:from_map(MultiPartHeaders),
            FileJObj = kz_json:from_list([{<<"headers">>, FileHeaders}
                                         ,{<<"contents">>, FileContents}
                                         ]),
            lager:debug("request is a file upload of type: ~s", [ContentType]),

            Filename = uploaded_filename(Context, ContentType, Req, MultiPartHeaders),
            {cb_context:add_req_file(Context, {Filename, FileJObj}), Req}
    end.

-spec uploaded_filename(cb_context:context(), kz_term:ne_binary(), cowboy_req:req(), cowboy:http_headers()) -> kz_term:ne_binary().
uploaded_filename(Context, ContentType, Req, MultiPartHeaders) ->
    case cb_context:req_value(Context, <<"filename">>) of
        'undefined' ->
            Disposition = maps:get(<<"content-disposition">>, MultiPartHeaders, cowboy_req:header(<<"content-disposition">>, Req)),
            maybe_use_disposition_filename(ContentType, Disposition);
        Filename ->
            lager:debug("found filename on request: ~s", [Filename]),
            Filename
    end.

maybe_use_disposition_filename(ContentType, 'undefined') ->
    default_filename(ContentType);
maybe_use_disposition_filename(ContentType, ContentDisposition) ->
    get_filename_disposition(ContentType, binary:split(ContentDisposition, <<$;>>, ['global'])).

get_filename_disposition(ContentType, []) ->
    default_filename(ContentType);
get_filename_disposition(ContentType, [Bin|Bins]) ->
    case binary:replace(Bin, [<<$\s>>, <<$">>], <<>>, ['global']) of
        <<"filename=", Filename/binary>> ->
            case kz_term:is_ne_binary(Filename) of
                'true' -> maybe_add_extension(Filename, ContentType);
                'false' -> default_filename(ContentType)
            end;
        _ ->
            get_filename_disposition(ContentType, Bins)
    end.

-spec default_filename(kz_term:ne_binary()) -> kz_term:ne_binary().
default_filename(ContentType) ->
    maybe_add_extension(<<"uploaded_file_", (kz_term:to_binary(kz_time:now_s()))/binary>>, ContentType).
...
maybe_add_extension(A, CT) ->
    case kz_term:is_empty(filename:extension(A)) of
        'false' -> A;
        'true' ->
            case kz_mime:to_extension(CT) of
                <<>> -> A;
                Ext -> <<A/binary, ".", Ext/binary>>
            end
    end.
...
decode_base64(Context, CT, Req, Base64Data) ->
    %% same as handle_file_contents/4
    MultiPartHeaders0 = cb_context:fetch(Context, <<"multipart_headers">>, #{}),
    MultiPartHeaders = maps:merge(MultiPartHeaders0
                                 ,#{<<"content_type">> => CT
                                   ,<<"content_length">> =>
    ...
    decode_base64(Context, CT, Req, Base64Data, MultiPartHeaders).

-spec decode_base64(cb_context:context(), kz_term:ne_binary(), cowboy_req:req(), binary(), cowboy:http_headers()) ->
          {cb_context:context(), cowboy_req:req()} |
          stop_return().
decode_base64(Context, CT, Req, Base64Data, MultiPartHeaders) ->
    try kz_attachment:decode_base64(Base64Data) of
        {EncodedType, FileContents} ->
            ...
            Headers = kz_json:from_map(
                        maps:merge(MultiPartHeaders
                                  ,#{<<"content_type">> => ContentType
            ...
            Filename = uploaded_filename(Context, ContentType, Req, MultiPartHeaders),
            lager:debug("request is a base64 file upload of type: ~s (~s)", [Filename, ContentType]),
            {cb_context:add_req_file(Context, {Filename, FileJObj}), Req}
    catch
        _T:_E ->
...
-type get_json_return() :: {'undefined', cowboy_req:req()} | decode_json_return().
-type decode_json_return() :: {kz_json:object(), cowboy_req:req()} |
...
-spec decode_json_body(binary(), cowboy_req:req()) -> decode_json_return().
decode_json_body(ReqBody, Req) ->
    try kz_json:unsafe_decode(ReqBody) of
        JObj ->
            lager:debug("request has a json payload: ~ts", [kz_log:redactor(ReqBody)]),
...
parse_path_tokens(_Context, [<<>>], Events) -> Events;
parse_path_tokens(_Context, [<<"scopes">>=Mod|T], Events) ->
    [{Mod, T} | Events];
...
parse_path_tokens(Context, [Mod|T], Events) ->
    case is_cb_module(Context, Mod) of
        'false' ->
            case is_custom_route_module(Mod) of
                'true' ->
                    parse_path_tokens_module(Context, [Mod|T], Events, 'true');
                'false' ->
                    []
            end;
        'true' ->
            IsCustomRoute = is_custom_route_module(Mod),
            parse_path_tokens_module(Context, [Mod|T], Events, IsCustomRoute)
    end.

-spec parse_path_tokens_module(cb_context:context(), kz_json:path(), cb_mods_with_tokens(), boolean()) ->
          cb_mods_with_tokens() |
          {'stop', cb_context:context()}.
parse_path_tokens_module(_Context, [CustomRouteModule | Params], Events, 'true') ->
    lager:debug("reached to custom route module '~s', passing the remaining path as params", [CustomRouteModule]),
    [{CustomRouteModule, Params} | Events];
parse_path_tokens_module(Context, [Module | Token], Events, 'false') ->
    {Params, List2} = lists:splitwith(fun(Elem) -> not is_cb_module(Context, Elem) end, Token),
...
-spec which_cb_module(cb_context:context(), kz_term:ne_binary()) -> kz_term:api_ne_binary().
which_cb_module(Context, Elem) ->
    ...
    which_cb_module(Modules).

-spec which_cb_module(kz_term:ne_binaries()) -> kz_term:api_ne_binary().
which_cb_module([]) -> 'undefined';
which_cb_module([Module | Modules]) ->
    case kz_module:is_exported(Module, 'init', 0) of
        'true' -> Module;
        'false' -> which_cb_module(Modules)
    end.

-spec is_cb_module(cb_context:context(), kz_term:ne_binary()) -> boolean().
is_cb_module(Context, Elem) ->
    case which_cb_module(Context, Elem) of
        'undefined' -> 'false';
        _CbModule -> 'true'
    end.
...
is_early_authentic(Req, Context) ->
    ...
    Results = crossbar_bindings:pmap(Event, Context),
    Failed = crossbar_bindings:failed(Results),
    case crossbar_bindings:succeeded(Results) of
        [] ->
            Context1 = merge_buckets(Context, collect_remaining_buckets(Failed)),
            {'true', Req, Context1};
        ['true'|T] ->
            prefer_new_context(T, Req, Context, 'true', collect_remaining_buckets(Failed));
        [{'true', Context1}|T] ->
            lager:debug("one true context"),
            {'true', Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed))};
        [{'stop', Context1}|T] ->
            lager:debug("pre-authn stopped"),
            ?MODULE:stop(Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed)))
    end.
...
is_authentic(Req, Context0, _ReqVerb) ->
    ...
    case cb_context:auth_doc(Context0) of
        'undefined' ->
            Results = crossbar_bindings:pmap(Event, Context0),
            Failed = crossbar_bindings:failed(Results),
            case crossbar_bindings:succeeded(Results) of
                [] ->
                    Context1 = merge_buckets(Context0, collect_remaining_buckets(Failed)),
                    is_authentic_noun(Req, Context1, cb_context:req_nouns(Context0));
                ['true'|T] ->
                    prefer_new_context(T, Req, Context0, 'undefined', collect_remaining_buckets(Failed));
                [{'true', Context1}|T] ->
                    {'true', Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed))};
                [{'stop', Context1}|T] ->
...
-spec is_authentic_noun(cowboy_req:req(), cb_context:context(), req_nouns()) ->
          {{'false', <<>>} | 'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
is_authentic_noun(Req, Context, []) ->
    lager:debug("failed to authenticate"),
    ?MODULE:stop(Req, cb_context:add_system_error('invalid_credentials', Context));
is_authentic_noun(Req, Context, [{Mod, Params} | _ReqNouns]) ->
    Event = create_event_name(Context, <<"authenticate.", Mod/binary>>),
    Payload = [Context | Params],
    Results = crossbar_bindings:pmap(Event, Payload),
    Failed = crossbar_bindings:failed(Results),
    case crossbar_bindings:succeeded(Results) of
        [] ->
            lager:debug("failed to authenticate : ~p", [Mod]),
            Context1 = merge_buckets(Context, collect_remaining_buckets(Failed)),
            ?MODULE:stop(Req, cb_context:add_system_error('invalid_credentials', Context1));
        ['true'|T] ->
            prefer_new_context(T, Req, Context, 'undefined', collect_remaining_buckets(Failed));
        [{'true', Context2}|T] ->
            {'true', Req, merge_buckets(Context2, collect_remaining_buckets(T ++ Failed))};
        [{'stop', Context2}|T] ->
            lager:debug("authn stopped"),
            ?MODULE:stop(Req, merge_buckets(Context2, collect_remaining_buckets(T ++ Failed)))
    end.

-spec prefer_new_context(kz_term:proplist(), cowboy_req:req(), cb_context:context(), 'undefined' | boolean(), bucket_collection()) ->
          {boolean(), cowboy_req:req(), cb_context:context()} |
          stop_return().
prefer_new_context([], Req, Context, 'false', CollectBuckets) ->
    {'false', Req, merge_buckets(Context, CollectBuckets)};
prefer_new_context([], Req, Context, _Return, CollectBuckets) ->
    {'true', Req, merge_buckets(Context, CollectBuckets)};

prefer_new_context([{'true', Context1}|T], Req, _Context, _Return, CollectBuckets) ->
    {'true', Req, merge_buckets(Context1, collect_remaining_buckets(T, CollectBuckets))};
prefer_new_context(['true'|T], Req, Context, _Return, CollectBuckets) ->
    prefer_new_context(T, Req, Context, 'true', CollectBuckets);

prefer_new_context(['false'|T], Req, Context, 'undefined', CollectBuckets) ->
    prefer_new_context(T, Req, Context, 'false', CollectBuckets);
prefer_new_context([{'false', Context1}|T], Req, Context, 'undefined', CollectBuckets) ->
    prefer_new_context(T, Req, Context, 'false', collect_buckets(Context1, CollectBuckets));
prefer_new_context(['false'|T], Req, Context, 'false', CollectBuckets) ->
    prefer_new_context(T, Req, Context, 'false', CollectBuckets);
prefer_new_context([{'false', Context1}|T], Req, Context, 'false', CollectBuckets) ->
    prefer_new_context(T, Req, Context, 'false', collect_buckets(Context1, CollectBuckets));

prefer_new_context([{'stop', Context1}|T], Req, _Context, _Return, CollectBuckets) ->
    lager:debug("authn stopped"),
    Context2 = merge_buckets(Context1, collect_remaining_buckets(T, CollectBuckets)),
    ?MODULE:stop(Req, Context2).

-type bucket_collection() :: #{consumed_token_buckets => non_neg_integer()
                              ,token_bucket_names => kz_term:ne_binaries()
                              }.
-spec new_bucket_collect() -> bucket_collection().
new_bucket_collect() ->
    #{consumed_token_buckets => 0
     ,token_bucket_names => []
     }.

-spec collect_buckets(cb_context:context(), bucket_collection()) -> bucket_collection().
collect_buckets(Context, #{consumed_token_buckets := ConsumedTokens
                          ,token_bucket_names := BucketNames
                          }
               ) ->
    #{consumed_token_buckets => cb_context:consumed_token_buckets(Context) + ConsumedTokens
     ,token_bucket_names => lists:usort(cb_context:token_bucket_names(Context) ++ BucketNames)
     }.

-spec collect_remaining_buckets(kazoo_bindings:map_results()) -> bucket_collection().
collect_remaining_buckets(Results) ->
    collect_remaining_buckets(Results, new_bucket_collect()).

-spec collect_remaining_buckets(kazoo_bindings:map_results(), bucket_collection()) -> bucket_collection().
collect_remaining_buckets([], CollectBuckets) ->
    CollectBuckets;
collect_remaining_buckets([{'stop', _}|T], CollectBuckets) ->
    %% not counting stop, we only have one result when stop in succeeded.
    collect_remaining_buckets(T, CollectBuckets);
collect_remaining_buckets([{'EXIT', _}|T], CollectBuckets) ->
    collect_remaining_buckets(T, CollectBuckets);
collect_remaining_buckets([{_, Context}|T], CollectBuckets) ->
    collect_remaining_buckets(T, collect_buckets(Context, CollectBuckets));
collect_remaining_buckets([_|T], CollectBuckets) ->
    collect_remaining_buckets(T, CollectBuckets).

-spec merge_buckets(cb_context:context(), bucket_collection()) -> cb_context:context().
merge_buckets(Context, #{consumed_token_buckets := ConsumedTokens
                        ,token_bucket_names := BucketNames
                        }) ->
    CtxConsumedTokens = cb_context:consumed_token_buckets(Context),
    CtxBucketNames = cb_context:token_bucket_names(Context),

    Setters = [{fun cb_context:set_consumed_token_buckets/2, CtxConsumedTokens + ConsumedTokens}
              ,{fun cb_context:set_token_bucket_names/2, lists:usort(CtxBucketNames ++ BucketNames)}
              ],
...
is_permitted(Req, Context) ->
    is_permitted_scope(Req, Context).

-spec is_permitted_scope(cowboy_req:req(), cb_context:context()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
is_permitted_scope(Req, Context0) ->
    lager:debug("checking for enforced scopes"),
    case maybe_enforce_scopes(Req, Context0) of
        {'true', Req1, Context1} ->
            is_permitted_verb(Req1, Context1, cb_context:req_verb(Context1));
        {'stop', Req1, Context1} ->
            lager:debug("allowed_scopes stopped"),
            ?MODULE:stop(Req1, Context1)
    end.
...
is_permitted_verb(Req, Context0, _ReqVerb) ->
    ...
    case crossbar_bindings:succeeded(Results) of
        [] ->
            Context1 = merge_buckets(Context0, collect_remaining_buckets(Failed)),
            is_permitted_nouns(Req, Context1, cb_context:req_nouns(Context1));
        ['true'|T] ->
            Context2 = merge_buckets(Context0, collect_remaining_buckets(T ++ Failed)),
            is_permitted_verb_on_module(Req, Context2, cb_context:req_nouns(Context0));
        [{'true', Context1}|T] ->
            Context2 = merge_buckets(Context0, collect_remaining_buckets(T ++ Failed)),
            is_permitted_verb_on_module(Req, Context2, cb_context:req_nouns(Context1));
        [{'stop', Context1}|T] ->
            lager:debug("authz stopped"),
            ?MODULE:stop(Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed)))
    end.

-spec is_permitted_verb_on_module(cowboy_req:req(), cb_context:context(), req_nouns()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
is_permitted_verb_on_module(Req, Context0, [{Mod, Params} | _ReqNouns]) ->
    ...
    case crossbar_bindings:succeeded(Results) of
        [{'stop', Context1}|T] ->
            lager:debug("authz stopped"),
            ?MODULE:stop(Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed)));
        Other -> {'true', Req, merge_buckets(Context0, collect_remaining_buckets(Other ++ Failed))}
    end.

-spec is_permitted_nouns(cowboy_req:req(), cb_context:context(), req_nouns()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
is_permitted_nouns(Req, Context, [{<<"404">>, []}]) ->
    ?MODULE:stop(Req, cb_context:add_system_error('not_found', Context));
is_permitted_nouns(Req, Context, []) ->
    lager:debug("no one authz'd the request"),
    ?MODULE:stop(Req, cb_context:add_system_error('forbidden', Context));
is_permitted_nouns(Req, Context0, [{Mod, Params} | _ReqNouns]) ->
    Event = create_event_name(Context0, <<"authorize.", Mod/binary>>),
    Payload = [Context0 | Params],
    Results = crossbar_bindings:pmap(Event, Payload),
    Failed = crossbar_bindings:failed(Results),
    case crossbar_bindings:succeeded(Results) of
        [] ->
            ...
            Context1 = merge_buckets(Context0, collect_remaining_buckets(Failed)),
            ?MODULE:stop(Req, cb_context:add_system_error('forbidden', Context1));
        ['true'|T] ->
            {'true', Req, merge_buckets(Context0, collect_remaining_buckets(T ++ Failed))};
        [{'true', Context1}|T] ->
            {'true', Req, merge_buckets(Context1, collect_remaining_buckets(T ++ Failed))};
        [{'stop', Context1}|T] ->
            lager:debug("authz stopped"),
...
-define(EXECUTE_ERROR_MSG, <<"request execution failed">>).
...
execute_request(Req, Context, Mod, Params, Verb) ->
    ...
    Setters = [{fun cb_context:set_resp_status/2, 'fatal'}
              ,{fun cb_context:set_resp_error_msg/2, ?EXECUTE_ERROR_MSG}
    ...
    case cb_context:is_context(Context2) of
        ...
        'false' ->
            execute_request_failure(Req, Context1, Context2)
    end.
...
execute_request_results(Req, Context, 'accepted') ->
    {'true', Req, Context};
execute_request_results(Req, Context, RespStatus) ->
    case {RespStatus
         ,cb_context:resp_error_msg(Context)
         ,cb_context:resp_error_code(Context)
         }
    of
        {'fatal', ?EXECUTE_ERROR_MSG, 500} ->
            %% binding fold is crashed, wiping the resp data so we don't leak
            %% possible sensitive or immature response
            ?MODULE:stop(Req, cb_context:set_resp_data(Context, kz_json:new()));
        _ ->
...
cleanup_file(ConnPid, File) ->
    ...
    case file:delete(File) of
        ...
        {'error', 'enoent'} ->
            lager:error("file ~s not found after ~pms", [File, kz_time:elapsed_ms(Start)])
    end.
...
create_push_response(Req0, Context, Fun) ->
    case should_use_explicit_error_code(cb_context:resp_error_code(Context)) of
        {'true', _CustomHTTPStatus} ->
            lager:debug("using explicit ~p HTTP status code for reply", [_CustomHTTPStatus]),
            stop(Req0, Context);
        'false' ->
...
-spec should_use_explicit_error_code(kz_term:api_non_neg_integer()) ->
          {'true', non_neg_integer()} | 'false'.
should_use_explicit_error_code(?DEFAULT_RESP_ERROR_CODE) ->
    %% Not explicit HTTP status set, ignore.
    'false';
should_use_explicit_error_code('undefined') ->
    %% Places like `crossbar_util:response/2' will set 'resp_error_code=undefined'. Ignore.
    'false';
should_use_explicit_error_code(CustomHTTPStatus) when is_integer(CustomHTTPStatus) ->
    {'true', CustomHTTPStatus}.
...
get_token_obj(Context) ->
    kz_json:from_list([{<<"consumed">>, cb_context:consumed_token_buckets(Context)}
...
create_default_resp_envelope(Context) ->
    BaseResp = [{<<"metadata">>, cb_context:get_metadata(Context)}
    ...
    Resp = case cb_context:response(Context) of
               {'ok', RespData} ->
                   success_envelope(Context, BaseResp, RespData);
               {'error', {ErrorCode, ErrorMsg, RespData}} ->
                   error_envelope(BaseResp, RespData, ErrorCode, ErrorMsg)
           end,
    maybe_delete_range_keys(kz_json:set_values(Resp, cb_context:resp_envelope(Context))
...
conditional_resp_data(Context) ->
    Fs = [fun maybe_add_version/2
         ,fun maybe_add_auth_token/2
         ],
    lists:foldl(fun(F, Acc) -> F(Context, Acc) end, [], Fs).

maybe_add_version(Context, Acc) ->
    case cb_context:is_authenticated(Context) of
        'true' -> props:set_value(<<"version">>, kapps_util:kazoo_version(), Acc);
        'false' -> Acc
    end.

-spec should_include_auth_token(cb_context:context()) -> boolean().
should_include_auth_token(Context) ->
    kz_app_config:is_true({?APP, cb_context:auth_account_id(Context)}, <<"include_auth_token">>, true).

maybe_add_auth_token(Context, Acc) ->
    maybe_add_auth_token(Context, Acc, should_include_auth_token(Context)).

maybe_add_auth_token(Context, Acc, 'true') ->
    props:set_value(<<"auth_token">>, cb_context:auth_token(Context), Acc);
maybe_add_auth_token(Context, Acc, 'false') ->
    case kz_term:is_true(cb_context:fetch(Context, 'include_token')) of
        'true' -> props:set_value(<<"auth_token">>, cb_context:auth_token(Context), Acc);
        'false' -> Acc
    end.

-spec close_chunk_json_envelope(cowboy_req:req(), cb_context:context()) -> 'ok'.
close_chunk_json_envelope(Req, Context) ->
    Paging = kz_json:to_proplist(maybe_delete_range_keys(cb_context:resp_envelope(Context)
                                                        ,cb_context:should_paginate(Context)
                                                        )
                                ),
    Trailer =
    ...
    Encoded = encode_trailer(maybe_add_auth_token(Context, Trailer)),
    'ok' = cowboy_req:stream_body(<<"], ", (kz_binary:join(Encoded))/binary, "}">>, 'fin', Req).

-spec maybe_delete_range_keys(kz_json:object(), boolean()) -> kz_json:object().
maybe_delete_range_keys(Resp, 'false') ->
    lager:debug("pagination disabled, removing resp envelope keys"),
    kz_json:delete_keys([<<"start_key">>, <<"page_size">>, <<"next_start_key">>], Resp);
maybe_delete_range_keys(Resp, 'true') ->
    Resp.
...
default_stop(Req0, Context) ->
    ...
    Req5 = set_resp_headers(Req4, Context),

    lager:debug("replying with ~p", [StatusCode]),
    Req6 = cowboy_req:reply(StatusCode, Req5),
    {'stop', Req6, cb_context:set_resp_status(Context, 'stop')}.
...
-spec maybe_enforce_scopes(cowboy_req:req(), cb_context:context()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
maybe_enforce_scopes(Req, Context) ->
    TokenType = cb_context:auth_token_type(Context),
    lager:debug("token type is: ~p~n", [TokenType]),
    maybe_enforce_scopes(Req, Context, TokenType).

-spec maybe_enforce_scopes(cowboy_req:req(), cb_context:context(), atom()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
maybe_enforce_scopes(Req, Context, 'x-auth-token') ->
    lager:debug("jwt token detected, enforcing scopes"),
    case kz_json:get_ne_binary_value(<<"method">>, cb_context:auth_doc(Context)) of
        'undefined' ->
            {'true', Req, Context};
        AuthModule ->
            [{Mod, _Params} | _] = cb_context:req_nouns(Context),
            Event = create_event_name(Context, <<"allowed_scopes.",Mod/binary>>),
            RequiredScopes = crossbar_bindings:pmap(Event, AuthModule),
            enforce_scopes(Req, Context, RequiredScopes)
    end;
maybe_enforce_scopes(Req, Context, _TokenType) ->
    lager:debug("legacy token detected skipping scope enforcement"),
    {'true', Req, Context}.

-spec enforce_scopes(cowboy_req:req(), cb_context:context(), kz_term:ne_binaries()) ->
          {'true', cowboy_req:req(), cb_context:context()} |
          stop_return().
enforce_scopes(Req, Context, []) ->
    {'true', Req, Context};
enforce_scopes(Req, Context, [RequiredScopes|_]) ->
    case kz_auth_scope:all(cb_context:auth_token(Context), RequiredScopes) of
        'true' ->
            lager:debug("all scopes match"),
            {'true', Req, Context};
        'false' ->
            lager:debug("all scopes match failed"),
            {'stop', Req, Context}
    end.
