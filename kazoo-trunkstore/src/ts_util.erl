...
-export([maybe_ensure_cid_valid/5
        ...
        ,onnet_options/2, onnet_options/3
        ]).
...
lookup_did_from_db(DID, AccountId) ->
    case kz_datamgr:get_result_doc(AccountId, ?TS_VIEW_DIDLOOKUP, DID) of
        {'ok', JObj} ->
            handle_found_did(DID, AccountId, JObj);
        {'error', _}=E ->
...
-spec merge_account(kz_json:object()) -> kz_json:object().
merge_account(ServerDoc) ->
    {'ok', AccountJObj} = kzd_accounts:fetch(kz_doc:account_id(ServerDoc)),
    TrunkAccount = kzd_trunkstore:account(ServerDoc),
    Restriction = kz_json:get_value(<<"call_restriction">>, AccountJObj, kz_json:new()),
    Props = [{<<"call_restriction">>, Restriction}
            ,{<<"account">>, merge_account_attributes(AccountJObj, TrunkAccount)}
            ],
    kz_json:set_values(Props, ServerDoc).

-spec merge_account_attributes(kzd_accounts:doc(), kz_json:object()) -> kz_json:object().
...
-spec get_call_duration(kz_json:object()) -> kz_term:api_integer().
...
-spec delay(kz_json:objects() | kz_term:api_binaries()) ->
          kz_term:api_integer().
delay(L) ->
    kz_term:safe_cast(simple_extract(L), 'undefined', fun kz_term:to_integer/1).
...
simple_extract([Int | _T]) when is_integer(Int) -> Int;
simple_extract([B | _T]) when is_boolean(B) -> B;
simple_extract([B | _T]) when is_list(B) -> B;
...
maybe_restrict_call(#ts_callflow_state{} = State, Command) ->
    Number = props:get_value(<<"To-DID">>, Command),
    Classification = knm_converters:classify(Number),
    lager:debug("trunkstore classified number as ~p", [Classification]),
    Opts = onnet_options(State, kz_json:new()),
    lager:debug("trunkstore lookup_user_flag results: ~s", [kz_json:encode(Opts)]),
    <<"deny">> =:= kz_json:get_value([<<"call_restriction">>, Classification, <<"action">>], Opts).

-spec onnet_options(ts_callflow:state(), Default) -> kz_json:object() | Default.
onnet_options(State, Default) ->
    AccountId = ts_callflow:get_account_id(State),
    AuthorizingId = ts_callflow:get_autorizing_id(State),
    onnet_options(AccountId, AuthorizingId, Default).

-spec onnet_options(kz_term:api_binary(), kz_term:api_binary(), Default) -> kz_json:object() | Default.
onnet_options('undefined', _, Default) -> Default;
onnet_options(_, 'undefined', Default) -> Default;
onnet_options(AccountId, AuthorizingId, Default) ->
    case kz_datamgr:open_cache_doc(AccountId, AuthorizingId) of
        {'ok', Options} -> merge_account(Options);
        _Else -> Default
    end.
