...
send_park(#ts_callflow_state{route_req_jobj=JObj
    ...
    lager:info("trunkstore knows how to route this call, sending park route response to ~s", [ServerId]),
    _ = kz_amqp_worker:cast(Resp
                           ,fun(API) -> kapi_route:publish_resp(ServerId, API) end
                           ,Worker
                           ),
...
-spec wait_for_win(state(), pos_integer(), kapps_call_command:received_event()) ->
          {'won' | 'lost', state()}.
wait_for_win(State, Timeout, {'ok', JObj}) ->
    case kz_api:event_name(JObj) of
        <<"route_win">> -> route_won(State, JObj);
        _Other -> wait_for_win(State, Timeout)
    end;
...
route_won(#ts_callflow_state{amqp_worker=Worker, kapps_call=Call}=State, RouteWin) ->
    ...
    lager:info("trunkstore has received a route win, taking control of the call"),
...
-spec wait_for_bridge(state(), kz_term:api_integer(), kapps_call_command:received_event()) ->
          {'hangup' | 'error' | 'bridged', state()}.
...
process_event_for_bridge(State, JObj, {<<"call_event">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"bridge">>}) ->
    ...
    of
        'true' ->
            ...
            {'hangup', State};
        'false' ->
            lager:info_unsafe("bridge failed: ~s",[kz_json:encode(JObj)]),
            {'error', State}
    end;
process_event_for_bridge(State, JObj, {<<"error">>, _, <<"bridge">>}) ->
    ...
    case not was_bridge_dialplan_no_endpoints_error(JObj)
        andalso was_bridge_blocked(JObj)
    of
        'true' ->
...
process_event_for_bridge(_State, _JObj
...
-spec was_bridge_dialplan_no_endpoints_error(kz_json:object()) -> boolean().
was_bridge_dialplan_no_endpoints_error(JObj) ->
    kz_json:get_ne_binary_value(<<"Dialplan-Error">>, JObj) =:= <<"no_endpoints">>.
...
-spec get_autorizing_id(state()) -> kz_term:api_binary().
get_autorizing_id(#ts_callflow_state{route_req_jobj=JObj}) ->
    kz_json:get_ne_binary_value([<<"Custom-Channel-Vars">>, <<"Authorizing-ID">>], JObj).
