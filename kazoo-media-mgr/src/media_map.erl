...
prompt_path(AccountId, PromptId, L) ->
    ...
    case kz_json:get_first_defined(language_keys(Language), Langs) of
        'undefined' when ?KZ_MEDIA_DB =:= AccountId ->
            lager:debug("failed to find prompt ~s in ~s, using default", [PromptId, Language]),
            default_prompt_path(PromptId, Language);
        'undefined' ->
            lager:debug("failed to find prompt ~s in ~s", [PromptId, Language]),
...
%%------------------------------------------------------------------------------
%% @doc Parse the media_map prompt ID from media doc ID.
...
maybe_add_prompt(AccountId, JObj, PromptId) ->
    ...
    Lang = kz_term:to_lower_binary(
             kz_json:get_value(<<"language">>, JObj, media_util:prompt_language(AccountId))
            ),
    ...
    Languages = kz_json:set_value(Lang
                                 ,media_util:prompt_path(kz_doc:account_id(JObj, ?KZ_MEDIA_DB)
...
default_language_keys(Language) ->
    DefaultLanguage = media_util:default_prompt_language(),
