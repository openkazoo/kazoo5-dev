...
prepare_proxy(#media_store_path{db = Db
    ...
    case media_cache_sup:find_file_server(Db, Id, Attachment) =:= {'error', 'no_file_server'} of
...
start_media_file_cache(Db, Id, Attachment) ->
    case media_cache_sup:start_file_server(Db, Id, Attachment) of
        {'ok', _FileServer} ->
...
maybe_proxy(JObj, #media_store_path{db = Db
    ...
    StreamType = media_util:convert_stream_type(kz_json:get_value(<<"Stream-Type">>, JObj)),
    NewOptions = props:delete('transcribe', Options),
    case kz_datamgr:attachment_url(Db, Id, Attachment, [{'stream_type',StreamType} | NewOptions]) of
        {'error', 'not_found'} ->
        ...
        <<URI/binary>> ->
            lager:debug("using URI ~s", [kz_log:redactor(URI)]),
            URI
    end.
...
proxy_uri(#media_store_path{db = Db
    ...
    UrlParts = [media_util:proxy_base_url(StreamType)
               ,StreamType
               ,Path
               ,File
               ],
