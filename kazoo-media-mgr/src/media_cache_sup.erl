...
start_file_server(Id, Doc, Attachment, Name) ->
    ChildSpec = ?WORKER_NAME_ARGS_TYPE(Name, 'media_file_cache', [Id, Doc, Attachment], 'temporary'),
...
find_tts_server(Id, JObj) ->
    ChildSpec = ?WORKER_NAME_ARGS_TYPE(Id, 'media_tts_cache', [Id, JObj], 'temporary'),
    case supervisor:start_child(?SERVER, ChildSpec) of
        {'ok', _Pid}=OK -> OK;
        {'error', {'already_started', Pid}} -> {'ok', Pid};
        {'error', 'already_present'} ->
