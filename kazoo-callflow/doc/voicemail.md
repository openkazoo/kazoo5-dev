...
When checking voicemails, it is possible for the caller to use the
`callback` feature to have the system place a call to the caller ID
number on the voicemail. While convenient, a compromised voicemail box
can be used to initiate fraudulent calls.

System administrators can toggle two configurations to manage this
feature:

1. `voicemail.should_disable_callback`: if set to `true` no callers
   will be able to use the callback feature
2. `voicemail.should_disable_offnet_callback`: if set to `true`, the
   caller *must* be calling from an authorized device (not from
   outside the account).
