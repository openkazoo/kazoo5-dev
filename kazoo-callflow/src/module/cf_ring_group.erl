...
attempt_endpoints(Endpoints, Data, Call0) ->
    ...
    case kapps_call_command:b_bridge(Endpoints, Timeout, Strategy, IgnoreEarlyMedia
                                    ,kz_media_util:media_path(Ringback, kapps_call:account_id(Call))
                                    ,'undefined', IgnoreForward, FailOnSingleReject, set_presence(Data, Call)
                                    )
    of
        {'ok', Reply} ->
            lager:info("completed successful bridge to the ring group - call finished normally"),
            after_success_bridge(kz_call_event:channel_answer_state(Reply), should_hard_stop(Call, Data));
        {'fail', _}=F ->
...
-spec after_success_bridge(kz_term:api_ne_binary(), boolean()) -> 'hard_stop' | 'hangup' | 'stop'.
after_success_bridge(<<"hangup">>, _) -> 'hard_stop';
after_success_bridge(_AnswerState, 'true') -> 'hangup';
after_success_bridge(_AnswerState, 'false') -> 'stop'.

-spec get_endpoints(kz_json:object(), kapps_call:call()) -> kz_json:objects().
get_endpoints(Data, Call) ->
    get_endpoints(Data, Call, kz_json:get_ne_binary_value(<<"group_id">>, Data)).

-spec get_endpoints(kz_json:object(), kapps_call:call(), kz_term:api_ne_binary()) -> kz_json:objects().
get_endpoints(Data, Call, 'undefined') ->
    maybe_order_endpoints_by_delay(strategy(Data)
                                  ,receive_endpoints(start_builders(Data, Call))
                                  );
get_endpoints(Data, Call, GroupId) ->
    lager:debug("merging data to group ~s settings", [GroupId]),
    AccountDb = kapps_call:account_db(Call),
    case kz_datamgr:open_cache_doc(AccountDb, GroupId) of
        {'ok', JObj} ->
            NewData = kz_json:merge(JObj, Data),
            maybe_order_endpoints_by_delay(strategy(NewData)
                                          ,receive_endpoints(start_builders(NewData, Call))
                                          );
        {'error', _Reason} ->
...
resolve_endpoint_ids(Data, Call, Members) ->
    ...
    FilteredEndpoints = [{Weight, {Id, kz_json:set_value(<<"source">>, kz_term:to_binary(?MODULE), Member)}}
                         || {_Type, Id, _Delay, _Timeout, Weight, Member} <- ResolvedEndpoints,
...
%% @end
-spec is_endpoint_resolved(kz_term:api_binary(), delay(), timeout_t(), endpoint_intermediates() | endpoint_intermediate()) -> boolean().
