...
-type after_forward_action() :: 'prompt' | 'save'.
-type forward_message_action() :: 'append' | 'forward' | 'return'.
...
-define(MAX_MESSAGE_MENU_LOOPS, 3).
-define(DEFAULT_SEEK_DURATION, 10 * ?MILLISECONDS_IN_SECOND).

-define(MSG_KEY_CALLER_ID_NUMBER, <<"caller_id_number">>).
-define(MSG_KEY_TIMESTAMP, <<"timestamp">>).

-define(PROMPT_DELETED, <<"vm-deleted">>).
-define(PROMPT_MESSAGE_MENU, <<"vm-message_menu">>).
-define(PROMPT_MESSAGE_NUMBER, <<"vm-message_number">>).
-define(PROMPT_RECEIVED, <<"vm-received">>).
-define(PROMPT_SAVED, <<"vm-saved">>).
-define(PROMPT_NOT_AVAILABLE, <<"vm-not_available">>).
...
-record(mailbox, {mailbox_id :: kz_term:api_ne_binary()
                 ...
                 ,after_forward_action = 'save' :: after_forward_action()
...
handle(Data, Call0) ->
    #mailbox{language=Lang}=Mailbox = get_mailbox_profile(Data, Call0),
    Call = kapps_call:set_language(Lang, Call0),
    ...
    case kz_json:get_ne_binary_value(<<"action">>, Data, <<"compose">>) of
        <<"compose">> ->
            ...
            case compose_voicemail(Mailbox, Call) of
                'ok' ->
        ...
        <<"check">> ->
            kapps_call_command:b_answer(Call),
            case check_mailbox(Mailbox, Call) of
...
-spec check_mailbox(mailbox(), boolean(), kapps_call:call(), non_neg_integer()) ->
          'ok' | {'error', 'channel_hungup'}.
check_mailbox(#mailbox{max_login_attempts=MaxLoginAttempts}, _IsOwner, Call, Loop)
  when Loop > MaxLoginAttempts ->
    %% if we have exceeded the maximum loop attempts then terminate
    %% this call
...
check_mailbox(#mailbox{exists='false'
    ...
    %% if the callflow did not define the mailbox to check then
    %% request the mailbox ID from the user
    case find_mailbox(Box, Call, ?DEFAULT_FIND_BOX_PROMPT, Loop) of
        {'ok', PossibleBox, NewLoop} -> check_mailbox(PossibleBox, Call, NewLoop);
        {'error', 'not_found'} ->
            %% can't find mailbox, set Loop to max to play abort
            %% prompts in above function clause
            check_mailbox(Box, Call, MaxLoginAttempts + 1)
    end;
check_mailbox(#mailbox{is_setup='false'}=Box, 'true', Call, _Loop) ->
    %% If this is the owner of the mailbox calling in and it is not
    %% setup then jump right to the setup wizard
    lager:info("caller is the owner of this mailbox, and it has not been setup yet"),
    main_menu(Box, Call);
check_mailbox(#mailbox{announcement_only='true'
                      ,require_pin='false'
                      }=Box
             ,'true'
             ,Call
...
check_mailbox(#mailbox{require_pin='false'}=Box, 'true', Call, _Loop) ->
    %% If this is the owner of the mailbox calling in and it doesn't
    %% require a pin then jump right to the main menu
    lager:info("caller is the owner of this mailbox, and requires no pin"),
    check_mailbox_menu(Box, Call);
check_mailbox(#mailbox{pin = <<>>}, _IsOwner, Call, _Loop) ->
    %% If the caller is not the owner or the owner with require pin
    %% set but the voicemail box has no pin set then terminate this
    %% call.
...
-spec check_mailbox_menu(mailbox(), kapps_call:call()) ->
          'ok' | {'error', 'channel_hungup'}.
...
-spec find_destination_mailbox(mailbox(), kapps_call:call(), kz_term:ne_binary(), non_neg_integer()) ->
          {'ok', mailbox()} | {'error', 'not_found'}.
...
find_destination_mailbox(#mailbox{max_login_attempts=MaxLoginAttempts}=Box, Call, SrcBoxId, Loop) ->
    case find_mailbox(#mailbox{}, Call, <<"vm-enter_forward_id">>, Loop) of
        ...
        {'ok', DestBox, _NewLoop} -> {'ok', DestBox};
        {'error', 'not_found'} ->
...
record_voicemail(AttachmentName
                ,#mailbox{max_message_length=MaxMessageLength}=Box
                ,Call
                ) ->
    ...
    case kapps_call_command:b_record(AttachmentName
                                    ,?ANY_DIGIT
                                    ,kz_term:to_binary(MaxMessageLength)
                                    ,silence_threshold(Box)
                                    ,silence_hits(Box)
                                    ,Call
                                    )
    of
        {'ok', Msg} ->
            handle_record_result(AttachmentName, Box, Call, Msg);
        {'error', _R} ->
            lager:info("error while attempting to record a new message: ~p", [_R])
    end.

handle_record_result(AttachmentName
                    ,#mailbox{media_extension=Ext}=Box
                    ,Call
                    ,Msg
                    ) ->
    ...
    of
        ...
        {'ok', _Selection} ->
            ...
            _ = kapps_call_command:prompt(?PROMPT_SAVED, Call),
...
%%------------------------------------------------------------------------------
%% @doc Returns the complete message-and-envelope audio prompt.
%% @end
%%------------------------------------------------------------------------------
-spec message_prompt(kz_json:objects(), binary(), non_neg_integer(), mailbox()) ->
          kapps_call_command:audio_macro_prompts().
message_prompt([H|_]=Messages, Message, Count, #mailbox{skip_envelope='false'}=Box) ->
    message_number(Messages, Count) ++ message_content(Message, Box) ++ message_envelope(H, Box) ++ message_menu();
message_prompt(Messages, Message, Count, #mailbox{skip_envelope='true'}=Box) ->
    lager:debug("mailbox is set to skip playing message envelope"),
    message_number(Messages, Count) ++ message_content(Message, Box) ++ message_menu().

%%------------------------------------------------------------------------------
%% @doc Returns the message number audio macro.
%% @end
%%------------------------------------------------------------------------------
-spec message_number(kz_json:objects(), non_neg_integer()) -> kapps_call_command:audio_macro_prompts().
message_number(Messages, Count) ->
    [{'prompt', ?PROMPT_MESSAGE_NUMBER}
    ,{'say', kz_term:to_binary(Count - length(Messages) + 1), <<"number">>}
    ].

%%------------------------------------------------------------------------------
%% @doc Returns the message envelope audio macro.
%% @end
%%------------------------------------------------------------------------------
-spec message_envelope(kz_json:object(), mailbox()) -> kapps_call_command:audio_macro_prompts().
message_envelope(Message, #mailbox{timezone=Timezone, envelope_type = <<"caller_and_time">>}) ->
    CallerId = kz_json:get_ne_binary_value(?MSG_KEY_CALLER_ID_NUMBER, Message),
    [{'prompt', <<"vm-you_have">>}
    ,{'prompt', ?PROMPT_RECEIVED}
    ,{'prompt', <<"vm-a_call_from">>}
    ,{'say', CallerId}
    ,{'prompt', <<"vm-on">>}
    ,{'say', get_unix_epoch(kz_json:get_integer_value(?MSG_KEY_TIMESTAMP, Message), Timezone), <<"current_date_time">>}
    ];

message_envelope(Message, #mailbox{timezone = Timezone}) ->
    [{'prompt', ?PROMPT_RECEIVED}
    ,{'say', get_unix_epoch(kz_json:get_integer_value(?MSG_KEY_TIMESTAMP, Message), Timezone), <<"current_date_time">>}
    ].

%%------------------------------------------------------------------------------
%% @doc Returns the message content audio macro.
%% @end
%%------------------------------------------------------------------------------
-spec message_content(binary(), mailbox()) -> kapps_call_command:audio_macro_prompts().
message_content(Message, #mailbox{keys=Keys
                                 ,is_ff_rw_enabled=AllowFfRw
                                 }) ->
    message_content(Message, AllowFfRw, Keys).

%%------------------------------------------------------------------------------
%% @doc Returns the message content audio macro.
%% @end
%%------------------------------------------------------------------------------
-spec message_content(binary(), boolean(), vm_keys()) -> kapps_call_command:audio_macro_prompts().
message_content(Message, 'true'=_AllowFfRw, #keys{rewind=RW, fastforward=FF}=_Keys) ->
    [{'play', Message, ?ANY_DIGIT -- [RW, FF]}];
message_content(Message, 'false', _Keys) ->
    [{'play', Message}].

%%------------------------------------------------------------------------------
%% @doc Returns the message menu audio macro.
%% @end
%%------------------------------------------------------------------------------
-spec message_menu() -> kapps_call_command:audio_macro_prompts().
message_menu() ->
    [{'prompt', ?PROMPT_MESSAGE_MENU}].

%%------------------------------------------------------------------------------
%% @doc Plays a list of messages, handling each message and traversing the list
%% based on the user's menu choices.
%% @end
%%------------------------------------------------------------------------------
...
%%------------------------------------------------------------------------------
%% @doc Plays the current message and the menu, then handles the message and
%% continues traversing the message list based on the user's menu choice.
%% @end
%%------------------------------------------------------------------------------
-spec play_messages(kz_json:objects(), kz_json:objects(), non_neg_integer(), mailbox(), kapps_call:call()) ->
          'ok' | 'complete'.
play_messages([CurrentMessage|_]=Messages
             ,PrevMessages
             ,Count
             ,Box
             ,Call
    ...
    play_message_menu(Prompt, Messages, PrevMessages, Count, Box, Call);
play_messages([], _, _, _, _) ->
    lager:info("all messages in folder played to caller"),
    'complete'.

%%------------------------------------------------------------------------------
%% @doc Plays the message menu prompt and handles the user's choice.
%% @end
%%------------------------------------------------------------------------------
-spec play_message_menu(kapps_call_command:audio_macro_prompts(), kz_json:objects(), kz_json:objects(), non_neg_integer(), mailbox(), kapps_call:call()) ->
          'ok' | 'complete'.
play_message_menu(Prompt
                 ,[CurrentMessage|NextMessages]=Messages
                 ,PrevMessages
                 ,Count
                 ,#mailbox{seek_duration=SeekDuration
                          ,mailbox_id=BoxId
                          }=Box
                 ,Call
                 ) ->
    AccountId = kapps_call:account_id(Call),
    case play_message_menu(Prompt, Box, Call) of
        {'ok', 'keep'} ->
            lager:info("caller chose to save the message"),
            _ = kapps_call_command:flush(Call),
            {_, SavedMessage} = save_message(CurrentMessage, AccountId, Call),
        ...
        {'ok', 'forward'} ->
            ...
            _ = kapps_call_command:flush(Call),
            forward_message(Messages, PrevMessages, Count, Box, Call);
        {'ok', 'rewind'} ->
        ...
        {'error', 'no_selection'} ->
            _ = kapps_call_command:b_prompt(<<"fault-too_many_invalid_entries">>, Call),
            lager:info("hanging up channel after ~p attempts to get a message action", [?MAX_MESSAGE_MENU_LOOPS]),
...
callback_not_available(Messages, PrevMessages, Count, Box, Call) ->
    _ = kapps_call_command:audio_macro([{'prompt', ?PROMPT_NOT_AVAILABLE}], Call),
    play_messages(Messages, PrevMessages, Count, Box, Call).
...
%% @end
%%------------------------------------------------------------------------------
-spec forward_message(kz_json:objects(), kz_json:objects(), non_neg_integer(), mailbox(), kapps_call:call()) ->
          'ok' | 'complete'.
forward_message([CurrentMessage|NextMessages]=Messages, PrevMessages, Count, Box, Call) ->
    AccountId = kapps_call:account_id(Call),
    case maybe_forward_message(CurrentMessage, Box, Call) of
        'prompt' ->
            play_message_menu(message_menu(), Messages, PrevMessages, Count, Box, Call);
        'save' ->
            {_, SavedMessage} = save_message(CurrentMessage, AccountId, Call),
            play_messages(NextMessages, [SavedMessage|PrevMessages], Count, Box, Call)
    end.
...
%%------------------------------------------------------------------------------
%% @doc Forwards the current message if the user enters a valid vmbox number.
%% @end
%%------------------------------------------------------------------------------
-spec maybe_forward_message(kz_json:object(), mailbox(), kapps_call:call()) -> after_forward_action().
maybe_forward_message(Message, #mailbox{mailbox_id=SrcBoxId}=SrcBox, Call) ->
    lager:info("enter destination mailbox number"),
    _ = kapps_call_command:b_flush(Call),
    case find_destination_mailbox(#mailbox{}, Call, SrcBoxId, 1) of
        {'ok', DestBox} ->
            forward_message(Message, SrcBox, DestBox, Call);
        _ ->
            %% The user didn't enter a valid destination mailbox within the retry limit.
            lager:info("message not forwarded, returning to message menu..."),
            'prompt'
    end.

%%------------------------------------------------------------------------------
%% @doc Forwards a message, optionally allowing the user to prepend a message.
%% @end
%%------------------------------------------------------------------------------
-spec forward_message(kz_json:object(), mailbox(), mailbox(), kapps_call:call()) -> after_forward_action().
forward_message(Message, #mailbox{mailbox_id=SrcBoxId
                                 ,after_forward_action=AfterForwardAction
    ...
    of
        'false' ->
            ...
            AfterForwardAction;
        {'ok', 'return'} ->
            'prompt'; %% The user decided not to forward, so prompt again.
        {'ok', 'append'} ->
        ...
        {'ok', 'forward'} ->
            ...
            AfterForwardAction;
        {'error', _R} ->
            %% The error has already been handled, but the call may now be disconnected.
            lager:error("error during forward message menu: ~p", [_R]),
            'save'
    end.

-spec forward_message_menu(mailbox(), kapps_call:call()) ->
          {'error', 'channel_hungup' | 'channel_unbridge' | kz_json:object()} |
          {'ok', forward_message_action()}.
...
record_forward(AttachmentName, Message, SrcBoxId, #mailbox{media_extension=Ext
    ...
    case kapps_call_command:b_record(AttachmentName, ?ANY_DIGIT, kz_term:to_binary(MaxMessageLength), Call) of
        {'ok', Msg} ->
            ...
            of
                'false' ->
                    forward_message_task(AttachmentName, Length, Message, SrcBoxId, DestBox, Call);
                {'ok', 'record'} ->
                    record_forward(tmp_file(Ext), Message, SrcBoxId, DestBox, Call);
                {'ok', _Selection} ->
...
forward_message_task(AttachmentName, Length, Message, SrcBoxId, #mailbox{mailbox_number=BoxNum
    ...
    To = <<BoxNum/binary, "@", (kapps_call:to_realm(Call))/binary>>,
...
%%------------------------------------------------------------------------------
%% @doc Handles a user request to save a message.
%% @end
%%------------------------------------------------------------------------------
-spec save_message(kz_json:object(), kz_term:api_binary(), kapps_call:call()) -> kvm_message:db_ret().
save_message(Message, AccountId, Call) ->
    _ = kapps_call_command:b_prompt(?PROMPT_SAVED, Call),
    kvm_message:set_folder(?VM_FOLDER_SAVED, Message, AccountId).

%%------------------------------------------------------------------------------
%% @doc Handles a user request to delete a message.
%% @end
%%------------------------------------------------------------------------------
-spec delete_message(kz_json:object(), kz_term:api_binary(), kz_term:api_ne_binary(), kapps_call:call()) -> kvm_message:db_ret().
delete_message(Message, AccountId, BoxId, Call) ->
    _ = kapps_call_command:b_prompt(?PROMPT_DELETED, Call),
    MessageId = kz_json:get_ne_binary_value(<<"media_id">>, Message),
    ...
    kvm_message:set_folder({?VM_FOLDER_DELETED, 'false'}, Message, AccountId).

%%------------------------------------------------------------------------------
%% @doc Loops over the message menu after the first play back until the
...
-type message_menu_returns() :: {'ok', 'keep' | 'delete' | 'return' | 'replay' | 'prev' | 'next' | 'forward' | 'rewind' | 'fastforward' | 'callback'} |
                                {'error', 'channel_hungup' | 'channel_unbridge' | 'no_selection' | kz_json:object()}.

-spec play_message_menu(kapps_call_command:audio_macro_prompts(), mailbox(), kapps_call:call()) ->
          message_menu_returns().
play_message_menu(Prompt, Box, Call) ->
    play_message_menu(Prompt, Box, Call, ?MAX_MESSAGE_MENU_LOOPS).

-spec play_message_menu(kapps_call_command:audio_macro_prompts(), mailbox(), kapps_call:call(), non_neg_integer()) ->
          message_menu_returns().
play_message_menu(_Prompt, _Box, _Call, 0) ->
    {'error', 'no_selection'};
play_message_menu(Prompt, #mailbox{keys=#keys{replay=Replay
                                             ,keep=Keep
                                             ,forward=Forward
                                             ,delete=Delete
                                             ,prev=Prev
    ...
    of
        ...
        _ ->
            _ = kapps_call_command:b_prompt(<<"menu-invalid_entry">>, Call),
            play_message_menu(message_menu(), Box, Call, RetryLoop-1)
    end.
...
config_menu(_Box, _Call, Loop) when Loop > ?MAX_CONFIG_MENU_LOOPS ->
    lager:debug("no config menu option selected after ~p tries", [Loop - 1]);
config_menu(#mailbox{interdigit_timeout=Interdigit}=Box, Call, Loop) ->
...
get_mailbox_profile(Data, Call) ->
    ...
    case get_mailbox_doc(AccountId, Id, Data, Call) of
        {'ok', MailboxJObj} ->
            build_mailbox_profile(Data, Call, AccountId, MailboxJObj);
        {'error', R} ->
            lager:info("failed to load voicemail box ~s, ~p", [Id, R]),
            #mailbox{language = kapps_call:language(Call)}
    end.

build_mailbox_profile(Data, Call, AccountId, MailboxJObj) ->
    MailboxId = kz_doc:id(MailboxJObj),
    lager:info("loaded voicemail box ~s", [MailboxId]),
    Default = #mailbox{mailbox_doc=MailboxJObj},
    ...
    {NameMediaId, OwnerId, OwnerLanguage} = owner_info(AccountId, MailboxJObj),
    ...
    AfterForwardAction = after_forward_action(MailboxJObj),
    IsFfRwEnabled = is_ff_rw_enabled(MailboxJObj),

    #mailbox{mailbox_id = MailboxId
            ,mailbox_doc = MailboxJObj
            ,exists = 'true'
            ,keys = populate_keys(Call)
            ,announcement_only =
                 kzd_vmboxes:announcement_only(MailboxJObj, Default#mailbox.announcement_only)
            ,skip_instructions =
                 kzd_vmboxes:skip_instructions(MailboxJObj, Default#mailbox.skip_instructions)
            ,skip_greeting =
                 kzd_vmboxes:skip_greeting(MailboxJObj, Default#mailbox.skip_greeting)
            ,skip_envelope =
                 kzd_vmboxes:skip_envelope(MailboxJObj, Default#mailbox.skip_envelope)
            ,envelope_type =
                 kzd_vmboxes:envelope_type(MailboxJObj, Default#mailbox.envelope_type)
            ,pin =
                 kzd_vmboxes:pin(MailboxJObj, <<>>)
            ,timezone =
                 kzd_vmboxes:timezone(MailboxJObj)
            ,mailbox_number =
                 kzd_vmboxes:mailbox(MailboxJObj, kapps_call:request_user(Call))
            ,require_pin = should_require_pin(MailboxJObj)
            ,check_if_owner =
                 kzd_vmboxes:check_if_owner(MailboxJObj, CheckIfOwner)
            ...
            ,owner_id =
                 OwnerId
            ,is_setup =
                 kzd_vmboxes:is_setup(MailboxJObj, 'false')
            ,max_message_count =
            ...
            ,media_extension = kzd_vmboxes:media_extension(MailboxJObj)
            ,forward_type = kzd_vmboxes:vm_message_forward_type(MailboxJObj)
...
should_require_pin(MailboxJObj) ->
    case ?FORCE_REQUIRE_PIN of
        'true' -> 'true';
        'false' -> kzd_vmboxes:require_pin(MailboxJObj)
    end.
...
%%------------------------------------------------------------------------------
%% @doc Returns the after_forward_action() value for a mailbox.
%% @end
%%------------------------------------------------------------------------------
-spec after_forward_action(kz_json:object()) -> after_forward_action().
after_forward_action(MailboxJObj) ->
    case kzd_vmboxes:after_forward_action(MailboxJObj) of
        <<"prompt">> -> 'prompt';
        <<"save">> -> 'save'
    end.
...
-spec owner_info(kz_term:ne_binary(), kz_json:object()) ->
          {kz_term:api_binary(), kz_term:api_binary(), kz_term:ne_binary()}.
owner_info(AccountId, MailboxJObj) ->
    owner_info(AccountId, MailboxJObj, kz_json:get_ne_value(<<"owner_id">>, MailboxJObj)).

-spec owner_info(kz_term:ne_binary(), kz_json:object(), kz_term:api_binary()) ->
          {kz_term:api_binary(), kz_term:api_binary(), kz_term:ne_binary()}.
owner_info(AccountId, MailboxJObj, 'undefined') ->
    {kz_json:get_ne_value(?RECORDED_NAME_KEY, MailboxJObj)
    ,'undefined'
    ,get_account_prompt_language(AccountId)
    };
owner_info(AccountId, MailboxJObj, OwnerId) ->
    AccountLanguage = get_account_prompt_language(AccountId),

    case kz_datamgr:open_cache_doc(AccountId, OwnerId) of
        {'ok', OwnerJObj} ->
            {kz_json:find(?RECORDED_NAME_KEY, [OwnerJObj, MailboxJObj])
            ,OwnerId
            ,kzd_users:language(OwnerJObj, AccountLanguage)
            };
        {'error', 'not_found'} ->
...
get_account_prompt_language(AccountId) ->
    ...
    kzd_accounts:language(AccountJObj, DefaultLanguage).
...
-spec get_mailbox_doc(kz_term:ne_binary(), kz_term:api_binary(), kz_json:object(), kapps_call:call()) ->
          {'ok', kzd_vmboxes:doc()} |
          {'error', any()}.
get_mailbox_doc(AccountId, Id, Data, Call) ->
    CaptureGroup = kapps_call:kvs_fetch('cf_capture_group', Call),
    case kz_term:is_empty(Id) of
        'false' ->
            lager:info("opening vm box ~s", [Id]),
            kz_datamgr:open_cache_doc(AccountId, Id);
        'true' when CaptureGroup =:= <<>>; CaptureGroup =:= 'undefined' ->
            get_user_mailbox_doc(AccountId, Data, Call);
        'true' ->
            lager:info("searching for vm box by capture group '~s'", [CaptureGroup]),
            Opts = [{'key', CaptureGroup}
                   ,'include_docs'
                   ,'first_when_multiple'
                   ],
            case kz_datamgr:get_single_result(AccountId, <<"attributes/mailbox_number">>, Opts) of
                {'ok', JObj} ->
...
-spec get_user_mailbox_doc(kz_term:ne_binary(), kz_json:object(), kapps_call:call()) ->
          {'ok', kzd_vmboxes:doc()} |
          {'error', any()}.
get_user_mailbox_doc(AccountId, Data, Call) ->
    get_user_mailbox_doc(AccountId, Data, Call, kapps_call:owner_id(Call)).

-spec get_user_mailbox_doc(kz_term:ne_binary(), kz_json:object(), kapps_call:call(), kz_term:api_binary()) ->
          {'ok', kzd_vmboxes:doc()} |
          {'error', any()}.
get_user_mailbox_doc(AccountId, Data, Call, 'undefined') ->
    DeviceId = kapps_call:authorizing_id(Call),
    case kz_datamgr:open_cache_doc(AccountId, DeviceId) of
        {'ok', DeviceJObj} ->
            case kzd_devices:owner_id(DeviceJObj) of
                ...
                OwnerId ->
                    get_user_mailbox_doc(AccountId, Data, Call, OwnerId)
            end;
        {'error', _} ->
            lager:debug("unknown device used to check voicemail", []),
            {'error', "request voicemail box number"}
    end;
get_user_mailbox_doc(_AccountId, Data, Call, OwnerId) ->
    ...
    case kz_attributes:owned_by_docs(OwnerId, <<"vmbox">>, Call) of
        ...
        [Box] when SingleMailboxLogin ->
            ...
            {'ok', Box};
        Boxes ->
            lager:debug("found ~p vmboxes assigned to owner ~s", [length(Boxes), OwnerId]),
            maybe_match_callerid(Boxes, Data, Call)
    end.

-spec maybe_match_callerid([kzd_vmboxes:doc()], kz_json:object(), kapps_call:call()) ->
          {'ok', kzd_vmboxes:doc()} |
          {'error', any()}.
maybe_match_callerid(Boxes, Data, Call) ->
...
-spec try_match_callerid([kzd_vmboxes:doc()], kz_term:ne_binary()) ->
          {'ok', kzd_vmboxes:doc()} |
          {'error', any()}.
...
send_mwi_update(#mailbox{account_id=AccountId
                        ,mailbox_id=BoxId
                        }) ->
    kvm_mwi:notify_vmbox(AccountId, BoxId).

-spec silence_threshold(mailbox()) -> pos_integer().
silence_threshold(#mailbox{mailbox_doc=Doc}) ->
    silence_threshold(kzd_vmboxes:silence_threshold(Doc)
                     ,kapps_config:get_integer(?CF_CONFIG_CAT
                                              ,[?KEY_VOICEMAIL, <<"silence_threshold">>]
                                              )
                     ).

-spec silence_threshold(kz_term:api_integer(), kz_term:api_integer()) -> pos_integer().
silence_threshold('undefined', 'undefined') -> 200;
silence_threshold('undefined', ConfigThreshold) -> ConfigThreshold;
silence_threshold(BoxThreshold, _ConfigThreshold) -> BoxThreshold.


-spec silence_hits(mailbox()) -> pos_integer().
silence_hits(#mailbox{mailbox_doc=Doc}) ->
    silence_hits(kzd_vmboxes:silence_hits(Doc)
                ,kapps_config:get_integer(?CF_CONFIG_CAT
                                         ,[?KEY_VOICEMAIL, <<"silence_hits">>]
                                         )
                ).

-spec silence_hits(kz_term:api_integer(), kz_term:api_integer()) -> pos_integer().
silence_hits('undefined', 'undefined') -> 5;
silence_hits('undefined', ConfigThreshold) -> ConfigThreshold;
silence_hits(BoxThreshold, _ConfigThreshold) -> BoxThreshold.
