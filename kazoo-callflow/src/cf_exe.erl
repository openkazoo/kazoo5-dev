...
%% API
-export([start_link/1]).
-export([amqp_send/3, amqp_send/4, amqp_call/4]).
...
-record(state, {call = kapps_call:new() :: kapps_call:call()
               ...
               ,hard_stop = 'false' :: boolean()
               ,cause = 'undefined' :: kz_term:api_binary()
               }).
-type state() :: #state{}.
...
start_link(#{request := JObj} = Map) ->
    kz_log:put_callid(JObj),
    lager:debug("starting new callflow executor"),
...
-spec add_call_binding(kapps_call:call() | pid(), kz_term:ne_binary()) -> 'ok'.
add_call_binding(Srv, <<CallId/binary>>) when is_pid(Srv) ->
    gen_server:call(Srv, {'add_call_binding', CallId});
add_call_binding(Call, CallId) ->
    add_call_binding(cf_exe_pid(Call), CallId).
...
%% @doc callback helper for kapps_call
...
-spec amqp_send(pid() | kapps_call:call(), term(), kz_term:api_terms(), kz_amqp_worker:publish_fun()) -> 'ok'.
amqp_send(Srv, Target, API, PubFun) when is_pid(Srv), is_function(PubFun, 2) ->
    gen_server:cast(Srv, {'amqp_send', Target, API, PubFun});
amqp_send(Call, Target, API, PubFun) when is_function(PubFun, 2) ->
    amqp_send(cf_exe_pid(Call), Target, API, PubFun).
...
handle_call({'add_call_binding', CallId}, _From, State) ->
    _ = kz_events:bind_call_id(CallId),
    lager:info("added event binding for call id ~s", [CallId]),
    {'reply', 'ok', State};
...
handle_cast({'callid_update', NewCallId}, #state{call=Call}=State) ->
    lager:info("updating callid to ~s, catch you on the flip side", [NewCallId]),
    kz_log:put_callid(NewCallId),
    _ = kz_events:unbind_call_id(kapps_call:call_id_direct(Call)),
    _ = kz_events:bind_call_id(NewCallId),
...
handle_cast({'add_event_listener', {Mod, Args}}, #state{call=Call}=State) ->
    NewCall = kapps_call:kvs_store('consumer_channel', kz_amqp_channel:consumer_channel(), Call),
    _EvtL = cf_util:start_event_listener(NewCall, Mod, Args),
...
handle_cast({'amqp_send', API, PubFun}, #state{queue=Q}=State) ->
    ...
    {'noreply', State};
handle_cast({'amqp_send', Target, API, PubFun}, #state{queue=Q}=State) ->
    amqp_send_message(Target, API, PubFun, Q),
    {'noreply', State};
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_event(kz_call_event:payload(), state()) -> 'ok'.
handle_event(JObj, #state{cf_module_pid=PidRef
    ...
    case {kz_api:event_type(JObj), kz_call_event:call_id(JObj)} of
        ...
        {{<<"call_event">>, <<"CHANNEL_TRANSFEREE">>}, _} ->
            handle_channel_transferee(Call, JObj);
        {{<<"call_event">>, <<"CHANNEL_REPLACED">>}, _} ->
            handle_channel_replaced(Call, JObj, Notify);
        {{<<"call_event">>, <<"CHANNEL_DIRECT">>}, _} ->
            lager:info("this callflow executor is the target of a direct bowout, terminate immediately"),
            hard_stop(Call);
        {{<<"call_event">>, <<"CHANNEL_TRANSFEROR">>}, _} ->
            handle_channel_transferor(Call, JObj);
        ...
        {_Evt, CallId} ->
            lager:debug("relaying ~p to ~p", [_Evt, Notify]),
...
terminate(_Reason, #state{call=Call
    ...
    maybe_hangup_call(State#state{call = kapps_call:clear_helpers(Call)}),
    run_destroy_handlers(kapps_call:clear_helpers(Call), HangupInfo, DestroyHandlers);
terminate(_Reason, #state{call=Call
                         ,cf_module_pid={Pid, _}
                         ,hangup_info=HangupInfo
                         ,termination_handlers=DestroyHandlers
...
-spec find_cf_module(kz_json:object()) -> 'undefined' | 'skip' | {'local' | 'remote', atom()}.
find_cf_module(Flow) ->
    find_cf_module(Flow, kz_json:get_ne_binary_value(<<"module">>, Flow)).

find_cf_module(Flow, 'undefined') ->
    lager:warning("no module found in flow: ~p", [Flow]),
    'undefined';
find_cf_module(Flow, <<Module/binary>>) ->
...
-spec amqp_send_message(kz_term:ne_binary(), kz_term:api_terms(), kz_amqp_worker:publish_fun(), kz_term:ne_binary()) -> 'ok'.
amqp_send_message(Target, API, PubFun, AMQPQueue) ->
    Req = add_server_id(AMQPQueue, API),
    PubFun(Target, Req).
...
-spec handle_channel_transferor(kapps_call:call(), kz_call_event:payload()) -> 'ok'.
handle_channel_transferor(Call, CallEvent) ->
    ...
    case OrgFetchId =:= NewFetchId of
        'false' -> 'ok';
        'true'  ->
            log_transferor(kz_call_event:transfer_destination(CallEvent)),
            hard_stop(Call)
    end.

log_transferor('undefined') ->
    lager:info("this callflow executor is transfering, terminate immediately");
log_transferor(To) ->
    lager:info("this callflow executor is transfering to ~s, terminate immediately", [To]).

-spec handle_channel_transferee(kapps_call:call(), kz_call_event:payload()) -> 'ok'.
handle_channel_transferee(Call, CallEvent) ->
    OrgFetchId = kapps_call:custom_channel_var(<<"Fetch-ID">>, Call),
    NewFetchId = kz_call_event:custom_channel_var(CallEvent, <<"Fetch-ID">>),
    case OrgFetchId =:= NewFetchId of
        'true'  -> transfer(Call);
...
-spec handle_channel_replaced(kapps_call:call(), kz_call_event:payload(), kz_term:pids()) -> 'ok'.
...
-spec handle_channel_bridged(pid(), kz_term:pids(), kz_call_event:payload(), kapps_call:call()) -> 'ok'.
handle_channel_bridged(Self, Notify, CallEvent, Call) ->
    gen_server:cast(Self, {'set_call', kapps_call:set_call_bridged('true', Call)}),
    kz_process:spawn(fun maybe_announce_call_is_being_recorded/1, [Call]),
    relay_message(Notify, CallEvent).

-spec maybe_announce_call_is_being_recorded(kapps_call:call()) -> 'ok'.
maybe_announce_call_is_being_recorded(Call) ->
    maybe_announce_call_is_being_recorded(Call, kapps_call:is_recording(Call)).

-spec maybe_announce_call_is_being_recorded(kapps_call:call(), boolean()) -> 'ok'.
maybe_announce_call_is_being_recorded(Call, 'true') ->
    {'ok', Account} = kzd_accounts:fetch(kapps_call:account_id(Call)),
    AccCallRecConf = kz_endpoint:merge_call_recording(kzd_accounts:call_recording_account(Account)),
    ShouldPlayPrompt = kz_json:is_true([kapps_call:direction(Call) %% Direction: inbound | outbound.
                                       ,kapps_call:inception_type(Call) %% Origin: onnet | offnet
                                       ,<<"should_announce_when_recording">>
                                       ]
                                      ,AccCallRecConf
                                      ),
    maybe_announce_call_is_being_recorded(Call, 'true', ShouldPlayPrompt);
maybe_announce_call_is_being_recorded(Call, 'false') ->
    lager:debug("call is not being recorded, not trying to play prompt"),
    maybe_announce_call_is_being_recorded(Call, 'false', 'false').

-spec maybe_announce_call_is_being_recorded(kapps_call:call(), boolean(), boolean()) -> 'ok'.
maybe_announce_call_is_being_recorded(Call, 'true', 'true') ->
    PromptId = <<"general-call_is_being_recorded">>,
    lager:debug("playing ~s prompt", [PromptId]),
    CallId = kapps_call:call_id_direct(Call),
    Prompt = kapps_call:get_prompt(Call, PromptId),
    Commands = [kz_json:from_list([{<<"Application-Name">>, <<"noop">>}
                                  ,{<<"Call-ID">>, CallId}
                                  ,{<<"Msg-ID">>, kapps_call_command:noop_id()}
                                  ])
               ,kz_json:from_list([{<<"Application-Name">>, <<"play">>}
                                  ,{<<"Media-Name">>, Prompt}
                                  ,{<<"Terminators">>, ?ANY_DIGIT}
                                  ,{<<"Leg">>, 'undefined'}
                                  ,{<<"Call-ID">>, CallId}
                                  ,{<<"Endless-Playback">>, 'false'}
                                  ])
               ],
...
maybe_announce_call_is_being_recorded(_Call, _, _) ->
    'ok'.

-spec handle_usurp(pid(), kapps_call:call(), kz_call_event:payload()) -> 'ok'.
...
maybe_hangup_call(#state{call=Call
    ...
    case kapps_call:is_call_forward(Call) of
        'true' -> send_cfwd_response(Call, Cause);
        'false' -> send_response(Call, Cause)
    end;
maybe_hangup_call(#state{}) -> 'ok'.

-spec send_cfwd_response(kapps_call:call(), kz_term:api_ne_binary()) -> 'ok'.
send_cfwd_response(_Call, 'undefined') -> 'ok';
send_cfwd_response(Call, Cause) ->
    case kz_call_response:get_response(Cause, Call) of
        'undefined' -> 'ok';
        Response -> send_cfwd_response(Call, Cause, Response)
    end.

-spec send_cfwd_response(kapps_call:call(), kz_term:ne_binary(), kz_json:object()) -> 'ok'.
send_cfwd_response(Call, Cause, Response) ->
    CallId = kapps_call:call_id_direct(Call),
    Queue = kapps_call:control_queue(Call),
    Code = kz_json:get_value(<<"Code">>, Response),
    case kz_call_response:send(CallId, Queue, Code, Cause) of
        {'error', 'no_response'} ->
            hangup_call(Call, Cause);
        {'ok', _NoopId} ->
            lager:debug("sent default response for ~s (~s)", [Cause, _NoopId])
    end.

-spec send_response(kapps_call:call(), kz_term:api_ne_binary()) -> 'ok'.
send_response(Call, Cause) ->
...
-spec handle_channel_pivoted(kz_types:server_ref(), kz_term:api_pid_ref(), kz_call_event:payload(), kapps_call:call()) -> 'ok'.
handle_channel_pivoted(Self, PidRef, JObj, Call) ->
    case kz_json:get_value(<<"Application-Data">>, JObj) of
        ...
        <<FlowBin/binary>> ->
            ...
            {'ok', _} = cf_util:b_flush_control_queue(Call),
