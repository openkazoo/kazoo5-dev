...
-spec init(kazoo_couch:couch_version()) -> 'ok'.
init('couchdb_3') ->
...
set_node_api_port(Node) ->
    case kapps_config:get_integer(<<"kazoo_couch">>, <<"api_port">>, 'undefined', Node) of
        'undefined' ->
            _ = kapps_config:set_node(<<"kazoo_couch">>, <<"api_port">>, ?API_PORT, Node),
...
set_node_admin_port(Node) ->
    case kapps_config:get_integer(<<"kazoo_couch">>, <<"admin_port">>, 'undefined', Node) of
        'undefined' ->
            _ = kapps_config:set_node(<<"kazoo_couch">>, <<"admin_port">>, ?ADMIN_PORT, Node),
...
do_compact_node(Node, Heuristic, APIConn, AdminConn, Databases) ->
    ...
    lists:foldl(fun(Database, Acc) ->
                        ...
                        [Row | _] = NewAcc = do_compact_node_db(Node, Heuristic, APIConn, AdminConn, Database, Acc),
                        lager:debug("finished compacting ~p db on node ~p", [Database, Node]),
                        'ok' = kt_compaction_reporter:finished_db(CallId, Database, Row),
                        NewAcc
                end
               ,[]
               ,Databases
...
-spec get_all_dbs_and_sort_by_disk() -> dbs_and_sizes().
get_all_dbs_and_sort_by_disk() ->
    sort_by_disk_size(get_dbs_and_sizes()).

-spec get_dbs_and_sizes() -> dbs_and_sizes().
get_dbs_and_sizes() ->
    {'ok', Dbs} = kz_datamgr:db_info(),
    get_dbs_sizes(Dbs).

-spec get_dbs_sizes(kz_term:ne_binaries()) -> dbs_and_sizes().
get_dbs_sizes(Dbs) ->
    #{'server' := {_App, #server{}=Conn}} = kzs_plan:plan(),
    F = fun(Db, State) ->
...
-spec get_db_disk_and_data_fold(kazoo_couch:server()
                               ,kz_term:ne_binary()
                               ,{dbs_and_sizes(), non_neg_integer()}
                               ,pos_integer()
                               ) -> {dbs_and_sizes(), pos_integer()}.
...
-spec do_get_db_disk_and_data_fold(kazoo_couch:server()
                                  ,kz_term:ne_binary()
                                  ,{dbs_and_sizes(), non_neg_integer()}
                                  ) -> {dbs_and_sizes(), pos_integer()}.
...
-spec sort_by_disk_size(dbs_and_sizes()) -> dbs_and_sizes().
...
-spec list_and_sort_dbs_for_compaction() -> dbs_and_sizes().
...
db_to_trigger(Db) ->
    Classifiers = [{fun kzs_util:is_account_db/1, ?TRIGGER_ACCOUNT}
                  ,{fun kzs_util:is_account_mod/1, ?TRIGGER_ACCOUNT_MOD}
