...
to_schema_doc(M, Usage) ->
    ...
    Schema = kz_ast_util:schema_path(<<"metaflows.", Base/binary, ".json">>, code:priv_dir('konami_pro')),
...
process() ->
    ?LOG_DEBUG("processing konami_pro data usage: "),
...
process_match_mfa(Acc, _VarName, M, F, As) ->
    ?LOG_DEBUG("processing match mfa ~p = ~p:~p/~p~n", [_VarName, M, F, length(As)]),
...
process_mfa(#usage{data_var_name=DataName
    ...
    ?LOG_DEBUG("adding set_values usage ~p, ~p~n", [KeyValues, DataName]),
...
arg_list_has_data_var(DataName, Aliases, [?MOD_FUN_ARGS('kz_json'
    ...
    ?LOG_DEBUG("  checking set_value for Data: ~p~n", [Args]),
...
arg_list_has_data_var(DataName, Aliases, [?MOD_FUN_ARGS('kz_json'
    ...
    ?LOG_DEBUG("  checking set_values for Data: ~p~n", [Args]),
    case arg_list_has_data_var(DataName, Aliases, Args) of
        {DataName, _} -> ?LOG_DEBUG("  sublist had ~p~n", [DataName]), {DataName, T};
        'undefined' -> arg_list_has_data_var(DataName, Aliases, T);
        {Alias, _} -> ?LOG_DEBUG("  sublist had alias ~p~n", [Alias]), {Alias, T}
...
arg_to_key(?FUN_ARGS(_F, _As)) ->
    ?LOG_DEBUG("ignoring arg ~p(~p)~n", [_F, _As]),
    'undefined';
...
process_mfa_clause(#usage{data_var_name=DataName}=Acc
    ...
    ?LOG_DEBUG("  processing mfa clause for ~p(~p)~n  ~p~n", [DataName, DataIndex, lists:nth(DataIndex, Args)]),
    case lists:nth(DataIndex, Args) of
        ?VAR('_') -> Acc;
        ?EMPTY_LIST -> Acc;
        ?VAR(DataName) ->
            ?LOG_DEBUG("  processing ~p clause ~p~n", [DataName, Body]),
            process_clause_body(Acc, Body);
