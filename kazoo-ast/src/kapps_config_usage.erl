...
-spec update_schema(kz_json:key(), kz_json:object(), file:filename_all()) -> 'ok'.
update_schema(Name, AutoGenSchema, PrivDir) ->
...
process_modules(Modules) when is_list(Modules) ->
    io:format("processing kapps_config/kapps_account_config usage: "),
    Options = [{'expression', fun expression_to_schema/2}
              ,{'module', fun add_module_app_config/2}
              ,{'accumulator', new_acc()}
              ,{'application', fun add_app_config/2}
              ,{'after_module', fun add_schemas_to_bucket/2}
              ],
...
add_module_app_config(Module, Acc) ->
    case print_dot(Module, Acc) of
        {'skip', NewAcc} -> NewAcc;
        NewAcc ->
            add_module_app_config(Module, NewAcc, application:get_application(Module))
    end.

add_module_app_config(_Module, Acc, {'ok', App}) ->
    add_app_config(App, Acc);
add_module_app_config(_Module, Acc, 'undefined') ->
    ?SUP_LOG_ERROR("failed to get application of module ~p~n", [_Module]),
    Acc.
%%    exit(1).
...
config_to_schema(_Source, 'get_all_kvs', _Args, Schemas) ->
    Schemas;
config_to_schema(_Source, 'flush', _Args, Schemas) ->
    Schemas;
config_to_schema(_Source, 'migrate', _Args, Schemas) ->
    Schemas;
config_to_schema(_Source, 'get_node_value', _Args, Schemas) ->
    Schemas;
config_to_schema(_Source, 'get_category', _Args, Schemas) ->
    Schemas;
config_to_schema(_Source, 'fetch_category', _Args, Schemas) ->
    Schemas;
...
config_to_schema(_Source, 'get_with_strategy', _Args, Schemas) ->
    Schemas;
...
config_key_to_schema(Source, F, Document, Key, Default, #{app_schemas := Schemas}=Acc) ->
    ...
    ?DEBUG("setting '~s' from source ~s: ~p~n", [kz_binary:join(Path), Source, Properties]),
...
key_to_key_path(?LIST(?VAR(Name), ?EMPTY_LIST)) ->
    {'pattern', Name};
...
key_to_key_path(?LIST(Head, Tail)) ->
    case key_to_key_path(Tail) of
        'undefined' -> 'undefined';
        {'pattern', _Name} ->
            [kz_ast_util:binary_match_to_binary(Head), ?FIELD_PATTERNS, <<".+">>];
...
guess_type('get_integers', _) -> [<<"integer">>];
...
guess_type('set_list_string', _) -> [<<"string">>];
guess_type('set_list_string_append', _) -> [<<"string">>];
guess_type('set_list_string_remove', _) -> [<<"string">>];
...
guess_type_by_default(?LIST(?BINARY_MATCH(_), ?EMPTY_LIST)) -> [<<"string">>];
guess_type_by_default(?LIST(Head, ?EMPTY_LIST)) -> [guess_type_by_default(Head)];
...
guess_properties(<<Document/binary>>, SourceModule, <<Key/binary>>, Type, Default) ->
    DescriptionKey = description_key(Document, Key),

    Description = build_description(DescriptionKey),
...
guess_properties(Document, Source, [<<Key/binary>>], Type, Default) ->
...
guess_properties(Document, Source, [_Key, ?FIELD_PATTERNS|_]=Keys, Type, Default) ->
    JustKeys = [K || K <- Keys, ?FIELD_PATTERNS =/= K],
    guess_properties(Document, Source, kz_binary:join(JustKeys, <<".">>), Type, Default).

build_description(DescriptionKey) ->
