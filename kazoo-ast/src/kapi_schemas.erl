...
%% -define(DEBUG(Fmt, Args), ?LOG_DEBUG(Fmt, Args)).
...
to_schemas(App) when is_atom(App) ->
    kz_json:foreach(fun update_schema/1, process_app(App));
to_schemas(Modules) ->
    _ = [to_schema(Module) || Module <- Modules,
                              is_kapi_module(Module)
        ],
    'ok'.

is_kapi_module(Module) ->
    case filename:basename(Module, ".erl") of
        [$k, $a, $p, $i, $_ | _] -> 'true';
        _ -> 'false'
    end.
...
maybe_update_schema(PrivDir, GeneratedJObj, ID) ->
    ?DEBUG("adding ~s to ~s: ~s~n", [ID, PrivDir, kz_json:encode(GeneratedJObj)]),
    ...
    case kz_json:are_equal(ExistingJObj, UpdatedSchema) of
        'true' -> 'ok';
        'false' ->
            write_schema(Path, kz_json:encode(UpdatedSchema))
    end.

write_schema(Path, JSON) ->
    case file:write_file(Path, JSON) of
        'ok' -> ?DEBUG("wrote ~s", [Path]);
        {'error', 'enoent'} ->
...
-spec load_app(atom()) -> 'ok' | {'error', any()}.
load_app(App) ->
...
maybe_merge(KAPI, API, Schema, Schemas) ->
    ?DEBUG("setting ~s.~p schema~n", [KAPI, API]),
...
guess_field_default(<<"Endpoint-Actions">>) ->
    kz_json:from_list([{<<"type">>, <<"object">>}]);
guess_field_default(<<"Cost-Parameters">>) ->
    kz_json:from_list([{<<"type">>, <<"object">>}
...
validator_properties(_Fields, {'function', '-fun.has_cost_parameters/1-', 1}) ->
    kz_json:from_list([{<<"type">>, <<"object">>}
                      ,{<<"properties">>, cost_parameters_schema()}
                      ]);
...
validator_properties(_Fields, {'kz_api', 'event_name', 1}) ->
    kz_json:from_list([{<<"type">>, <<"string">>}]);
validator_properties(_Fields, {'kz_api_term', Function, 1}) ->
    kz_api_term:validator_properties(Function);
