...
-export([get/0
        ...
        ,schema_to_doc/2, schema_to_doc/3
        ,ref_tables_to_doc/1
...
-include_lib("ast/include/kz_ast.hrl").
-include_lib("kazoo_stdlib/include/kz_types.hrl").
-include_lib("kazoo_stdlib/include/kz_log.hrl").
-include_lib("kazoo_web/include/kazoo_web.hrl").
-include_lib("kazoo_documents/include/kazoo_documents.hrl").

-include_lib("ast/src/kz_ast.hrl").

-define(REF_PATH(App)
       ,code:lib_dir(App), "doc", "ref"
       ).
-define(REF_PATH(App, Module)
       ,filename:join([?REF_PATH(App), <<Module/binary,".md">>])
       ).
...
filters_to_ref_doc(Filters) ->
    ...
    'ok' = file:write_file(?REF_PATH('crossbar', <<"filters">>)
                          ,[?FILTER_DOC
                           ,?FILTER_HEADER
...
api_to_ref_doc([]) -> ?DEV_LOG("nothing to process");
api_to_ref_doc({Module, Paths}) ->
    api_to_ref_doc(Module, Paths, module_version(Module)).

api_to_ref_doc(Module, Config, ?CURRENT_VERSION) ->
    {App, Paths} = props:take_value('app', Config, 'crossbar'),
    ...
    DocPath = ?REF_PATH(App, EndpointName),
    'ok' = file:write_file(DocPath, Doc);
api_to_ref_doc(_Module, _Paths, _Version) ->
    ?DEV_LOG("ignoring version ~p", [_Version]).
...
methods_to_section('undefined', _Path, Acc) ->
    ?LOG_DEBUG("skipping path ~p~n", [_Path]),
    Acc;
...
%% @end
%%------------------------------------------------------------------------------
-spec schema_to_doc(kz_term:ne_binary(), atom() | kz_term:ne_binary()) -> 'ok'.
schema_to_doc(Schema, App) when is_atom(App) ->
    ?LOG_DEBUG("finding doc for schema ~s in app ~s", [Schema, App]),
    Doc = <<Schema/binary, ".md">>,
    PathToDoc = filename:join([code:lib_dir(App), "doc", Doc]),
    case filelib:is_regular(PathToDoc) of
        'false' ->
            ?LOG_WARNING("failed to find matching doc ~s", [PathToDoc]);
        'true' ->
            schema_to_doc(Schema, PathToDoc)
    end;
schema_to_doc(Schema, DocName) ->
    AppDir = kz_term:to_binary(filename:basename(filename:dirname(filename:dirname(DocName)))),
    schema_to_doc(Schema, DocName, AppDir).

-spec schema_to_doc(kz_term:api_object() | kz_term:ne_binary(), kz_term:ne_binary(), atom() | kz_term:ne_binary()) -> 'ok'.
schema_to_doc(<<Schema/binary>>, DocName, AppName) ->
    schema_to_doc(kz_ast_util:load_ref_schema(Schema), DocName, AppName);
schema_to_doc(SchemaJObj, DocName, <<AppName/binary>>) ->
    schema_to_doc(SchemaJObj, DocName, kz_term:to_atom(AppName, 'true'));
schema_to_doc('undefined', _DocName, _App) ->
    ?LOG_DEBUG("no schema to process for doc ~s in ~s~n", [_DocName, _App]);
schema_to_doc(SchemaJObj, DocName, App) ->
    DocFile = doc_file(DocName, App),

    {'ok', DocContents} = file:read_file(DocFile),
    case binary:split(DocContents, <<?SCHEMA_SECTION/binary, "\n">>) of
        [Before, After] ->
            Data = [Before, kz_ast_util:schema_to_table(SchemaJObj, App), After],
            'ok' = file:write_file(DocFile, Data);
        _Else ->
            ?LOG_DEBUG("file ~s appears to have a schema section already~n", [DocFile])
    end.

doc_file(DocName, App) ->
    case filelib:is_regular(DocName) of
        'true' -> DocName;
        'false' ->
            filename:join([code:lib_dir(App), "doc", DocName])
    end.
...
write_swagger_file(Props, <<"swagger2">>) ->
    ?LOG_DEBUG("~nsaving swagger 2.0 to file ~s~n", [?SWAGGER_2_JSON_FILE]),
...
write_swagger_file(Props, <<"oas3">>) ->
    ...
    ?LOG_DEBUG("~nsaving OpenAPI 3 Parameters to file ~s~n", [?OAS3_PARAMETERS_FILE]),
    ...
    ?LOG_DEBUG("saving OpenAPI 3 Schemas to file ~s~n", [?OAS3_SCHEMAS_FILE]),
    ...
    ?LOG_DEBUG("saving OpenAPI 3 Paths to their files~n", []),
    'ok' = maps:fold(fun write_oas3_path/3, 'ok', props:get_value([<<"oas3">>, <<"paths">>], Props)),

    ?LOG_DEBUG("saving OpenAPI 3 to file ~s~n", [?OAS3_YML_FILE]),
...
write_oas3_path(Endpoint, Meta, 'ok') ->
...
add_operation_summary(Operation, EndpointName, _EPMeta, Path, Method) ->
...
add_operation_id(Operation, _EndpointName, _EPMeta, Path, Method) ->
...
add_operation_parameters(Operation, _EPName, _EPMeta, Path, Method) ->
    Operation#{<<"parameters">> =>
                   [kz_json:to_map(Param)
                    || Param <- make_parameters(Path, Method, 'undefined', <<"oas3">>)
                   ]
              }.

-spec add_operation_request_body(map(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()) -> map().
add_operation_request_body(Operation, EndpointName, EndpointMeta, _Path, Method)
  when Method =:= <<"put">>;
       Method =:= <<"post">>;
       Method =:= <<"patch">> ->
...
-spec to_swagger_path(kz_json:key(), kz_json:object(), kz_json:object(), kz_term:api_object()) -> kz_json:object().
...
make_parameters(Path, Method, SchemaParameter, OasVersion) ->
    [Parameter
     || F <- [fun (_P, M) -> maybe_add_schema(M, SchemaParameter, OasVersion) end
             ,fun (P, _M) -> auth_token_param(P, OasVersion) end
             ,fun (P, _M) -> path_params(P, OasVersion) end
             ],
        Parameter <- F(Path, Method),
        not kz_term:is_empty(Parameter)
    ].

-spec maybe_add_schema(kz_term:ne_binary(), kz_term:api_object(), kz_term:ne_binary()) -> kz_json:objects().
maybe_add_schema(_Method, 'undefined', _OasVsn) ->
    [];
maybe_add_schema(_Method, _Schema, <<"oas3">>) ->
    [];
maybe_add_schema(Method, Schema, _OasVersion)
  when Method =:= <<"put">>;
       Method =:= <<"post">> ->
    [Schema];
maybe_add_schema(_Method, _Parameters, _OasVsn) ->
    [].
...
-spec auth_token_param(kz_term:ne_binary(), kz_term:ne_binary()) -> kz_json:objects().
auth_token_param(Path, OasVersion) ->
...
-spec path_params(kz_term:ne_binary(), kz_term:ne_binary()) -> kz_json:objects().
path_params(Path, OasVersion) ->
...
format_pc_callback({_Path, []}, Acc, _EndpointName, _ModuleName, _Callback) ->
    ?LOG_DEBUG("module ~s supported path ~s~nm: ~p c: ~p~n"
...
format_path_tokens(<<"/">>) -> [];
format_path_tokens(<<Token/binary>>) ->
...
path_name(Module) ->
    case grep_cb_module(Module) of
        ...
        {'match', [<<"pushes">>=Name]} -> <<?ACCOUNTS_PREFIX"/"?ENDPOINTS_PREFIX"/", Name/binary>>;
...
process_application(App, Acc) ->
    ...
    ?LOG_DEBUG("processing ~s modules: ", [App]),
...
process_api_module(File, Module) ->
    ...
    try process_api_ast(Module, AST)
        ...
        ?STACKTRACE(_E, _R, ST)
        ?LOG_DEBUG("failed to process ~p(~p): ~s: ~p\n", [File, Module, _E, _R]),
        ?LOG_DEBUG("~p\n", [ST]),
        'undefined'
        end.
...
process_api_ast_functions(Module, Functions) ->
    {Module
    ,[process_api_ast_function(F, Cs) || {F, _A, Cs} <- Functions]
    }.

-spec process_api_ast_function(atom(), [erl_parse:abstract_clause()]) ->
          allowed_methods() |
          content_types_provided().
process_api_ast_function('allowed_methods', Clauses) ->
    Methods = find_http_methods(Clauses),
    {'allowed_methods', Methods};
process_api_ast_function('content_types_provided', Clauses) ->
...
grep_cb_module(<<"cb_", Module/binary>>) ->
    Reverse = kz_binary:reverse(Module),
    case re:run(Reverse, <<"^([0-9]+v)_(.*)\$">>, [{'capture', 'all_but_first', 'binary'}]) of
        'nomatch' ->
            {'match', [Module]};
        {'match', [Version, Name]} ->
            {'match', [kz_binary:reverse(Name), kz_binary:reverse(Version)]}
    end;
grep_cb_module(_) ->
    'no_match'.
...
def_path_param(OasVersion, <<"{APP_TYPE}">>=P) -> generic_id_path_param(P, OasVersion);
...
def_path_param(OasVersion, <<"{SCOPE}">>=P) -> base_path_param(P, OasVersion);
def_path_param(OasVersion, <<"{SCOPE_RESTRICTION}">>=P) -> base_path_param(P, OasVersion);
...
def_path_param(OasVersion, Param) ->
    case kz_binary:reverse(Param) of
        <<"}DI_", _/binary>> ->
            generic_id_path_param(Param, OasVersion);
        _Else ->
            ?SUP_LOG_ERROR("~s/" ?FILE ":~p: No Swagger definition of path parameter '~s'\n"
                          ,[code:lib_dir('ast'), ?LINE, Param]
                          ),
            halt(1)
    end.
