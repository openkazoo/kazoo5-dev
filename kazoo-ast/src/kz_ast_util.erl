...
-export([module_ast/1
        ...
        ,create_schema/1
        ,schema_to_table/1, schema_to_table/2
        ,load_ref_schema/1
...
-spec ensure_file_exists(binary()) -> 'ok'.
...
-spec create_schema(binary()) -> 'ok'.
create_schema(Path) ->
    SkelJObj = skel_schema(),
    'ok' = kz_os:make_dir(Path, 'true'),
    'ok' = file:write_file(Path, [kz_json:encode(SkelJObj), "\n"]).

skel_schema() ->
    kz_json:from_list([{<<"$schema">>, <<"http://json-schema.org/draft-04/schema#">>}
                      ,{<<"_id">>, <<"skel">>}
...
project_apps() ->
    ...
    lists:foldl(fun add_if_loadable/2, [], Core ++ Apps).

add_if_loadable(App, Acc) ->
    case application:load(App) of
        'ok' -> [App | Acc];
        {'error', {'already_loaded', App}} -> [App | Acc];
        {'error', _Reason} ->
            ?LOG_DEBUG("  app ~p not loaded: ~p~n", [App, _Reason]),
            Acc
    end.

siblings_of(App) ->
    Ignore = [<<".git">>, <<".erlang.mk">>, <<".circleci">>],
    [dir_to_app_name(Dir)
     || Dir <- filelib:wildcard(filename:join([code:lib_dir(App), "..", "*"])),
        filelib:is_dir(Dir),
        not lists:member(kz_term:to_binary(filename:basename(Dir)), Ignore)
    ].
...
app_modules(App) ->
    case application:get_key(App, 'modules') of
        ...
        'undefined' ->
            case application:load(App) of
                'ok' -> app_modules(App);
                {'error', {'already_loaded', App}} -> app_modules(App);
                _ -> []
            end
    end.
...
schema_to_table(Schema) ->
    schema_to_table(Schema, 'crossbar').

-spec schema_to_table(kz_term:ne_binary() | kz_json:object(), atom()) -> iolist().
schema_to_table(<<"#/definitions/", _/binary>>=_S, _App) -> [];
schema_to_table(Schema=?NE_BINARY, App) ->
    case kz_json_schema:fload(Schema, App) of
        {'ok', JObj} -> schema_to_table(JObj, App);
        {'error', 'not_found'} ->
            ?LOG_DEBUG("failed to find ~s~n", [Schema]),
            throw({'error', 'no_schema'})
    end;
schema_to_table(SchemaJObj, _App) ->
    try schema_to_table_refs(SchemaJObj, []) of
        [Table|RefTables] ->
        ...
        'throw':'no_type':ST ->
            ?LOG_DEBUG("failed to build table from schema ~s~n", [kz_doc:id(SchemaJObj)]),
            ?LOG_DEBUG("~p~n", [ST]),
            throw('no_type')
    end.

schema_to_table_refs(SchemaJObj, BaseRefs) ->
    ...
    [schema_description(Description), [?TABLE_HEADER, Reversed], "\n"]
        ++ [{RefSchemaName, RefTable}
            || RefSchemaName <- WithSubRefs,
               ...
               (RefTable = schema_to_table_refs(RefSchema, WithSubRefs)) =/= []
           ].
...
property_to_row(SchemaJObj, Names, Settings, {Table, Refs}) ->
    SchemaType =
        try schema_type(Settings)
                ...
                ?LOG_DEBUG("no schema type in ~s for path ~p: ~p~n"
...
settings_description(Settings) ->
    maybe_description(Settings, kz_json:get_ne_binary_value(<<"description">>, Settings)).

maybe_description(Settings, 'undefined') ->
    maybe_description_from_ref(Settings, kz_json:get_value(<<"$ref">>, Settings));
maybe_description(_Settings, Description) ->
    Description.

maybe_description_from_ref(Settings, 'undefined') ->
    maybe_description_from_one_or_any(kz_json:get_first_defined([<<"oneOf">>, <<"anyOf">>], Settings));
maybe_description_from_ref(_Settings, RefSchemaName) ->
...
maybe_description_from_one_or_any('undefined') ->
    <<" ">>;
maybe_description_from_one_or_any([]) ->
    <<" ">>;
maybe_description_from_one_or_any(Schemas) ->
    kz_json:find(<<"description">>, Schemas, <<" ">>).

-spec maybe_add_ref(kz_term:ne_binaries(), kz_term:api_ne_binary() | kz_json:object()) -> kz_term:ne_binaries().
maybe_add_ref(Refs, 'undefined') ->Refs;
maybe_add_ref(Refs, <<RefId/binary>>) ->
    case lists:member(RefId, Refs) of
        'false' -> [RefId | Refs];
        'true' -> Refs
    end;
maybe_add_ref(Refs, Settings) ->
    maybe_add_ref(Refs, Settings, get_ref(Settings)).

maybe_add_ref(Refs, Settings, 'undefined') ->
    maybe_add_one_of_refs(Refs, kz_json:get_list_value(<<"oneOf">>, Settings, []));
maybe_add_ref(Refs, _Settings, <<RefId/binary>>) ->
    maybe_add_ref(Refs, RefId).

maybe_add_one_of_refs(Refs, []) ->
    Refs;
maybe_add_one_of_refs(Refs, OneOfs) ->
    lists:foldl(fun(OneOf, Acc) -> maybe_add_ref(Acc, OneOf) end, Refs, OneOfs).
...
schema_type(Settings, <<"integer">>) ->
    schema_integer_type(Settings);
schema_type(Settings, Types) when is_list(Types) ->
    kz_binary:join([schema_type(Settings, Type) || Type <- Types], <<" | ">>);
schema_type(_Settings, Type) ->
    <<Type/binary, "()">>.

schema_integer_type(Settings) ->
    schema_integer_type(get_minimum(Settings), get_maximum(Settings)).

schema_integer_type('undefined', 'undefined') -> <<"integer()">>;
schema_integer_type(Min, 'undefined') ->
    <<"integer(", (kz_term:to_binary(Min))/binary, "..)">>;
schema_integer_type('undefined', Max) ->
    <<"integer(..", (kz_term:to_binary(Max))/binary, ")">>;
schema_integer_type(Min, Max) ->
    <<"integer(", (kz_term:to_binary(Min))/binary, "..", (kz_term:to_binary(Max))/binary, ")">>.
...
maybe_schema_type_from_anyof(Settings) ->
    case kz_json:get_list_value(<<"anyOf">>, Settings) of
        'undefined' ->
            maybe_schema_type_from_allof(Settings);
        AnyOf ->
...
maybe_schema_type_from_allof(Settings) ->
    case kz_json:get_list_value(<<"allOf">>, Settings) of
        'undefined' ->
            ?LOG_DEBUG("no type: ~p~n", [Settings]),
            throw('no_type');
        AllOf ->
            SchemaTypes = [schema_type(AllOfJObj, get_type(AllOfJObj))
                           || AllOfJObj <- AllOf
                          ],
            kz_binary:join(SchemaTypes, <<" || ">>)
    end.
...
schema_array_type(Settings) ->
    case {kz_json:get_ne_binary_value([<<"items">>, <<"$ref">>], Settings)
         ,kz_json:get_ne_value([<<"items">>, <<"type">>], Settings)
         }
    of
        ...
        {Ref, _Type} ->
            <<"array(", (schema_ref_type(Ref))/binary, ")">>
    end.
...
maybe_sub_properties_to_row(SchemaJObj, <<"array">>, Names, Settings, {Table, Refs}) ->
    ItemType = kz_json:get_ne_binary_value([<<"items">>, <<"type">>], Settings),
    ItemRef = kz_json:get_ne_binary_value([<<"items">>, <<"$ref">>], Settings),
    ?LOG_DEBUG("array item type/ref for ~p: ~p/~p", [Names, ItemType, ItemRef]),
    case {ItemType, ItemRef} of
        ...
        {<<"object">> = _Type, RefId} ->
            ?LOG_DEBUG("array(~s): ~p", [RefId, Settings]),
            ...
            maybe_sub_properties_to_row(SchemaJObj
                                       ...
                                       ,Names ++ ["[]"]
                                       ,kz_json:get_json_value(<<"items">>, Settings, kz_json:new())
                                       ,{Table, maybe_add_ref(Refs, RefId)}
                                       );
        ...
        _Type ->
            ?LOG_DEBUG("unhandled array type ~p", [_Type]),
            {Table, Refs}
    end;
...
get_minimum(Schema) ->
    kz_json:get_integer_value(<<"minimum">>, Schema).

get_maximum(Schema) ->
    kz_json:get_integer_value(<<"maximum">>, Schema).
