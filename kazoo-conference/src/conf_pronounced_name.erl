    ...
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), UserId) of
        {'ok', UserDoc} ->
            case kz_json:get_value(?PRONOUNCED_NAME_KEY, UserDoc) of
                'undefined' -> 'undefined';
                DocId -> {'media_doc_id', kapps_call:account_id(Call), DocId}
            end;
        _ -> 'undefined'
    end.
...
save_recording(RecordName, MediaDocId, Call) ->
    ...
    _ = kapps_call_command:store_file(<<"/tmp/", RecordName/binary>>, get_new_attachment_url(RecordName, MediaDocId, Call), Call),
