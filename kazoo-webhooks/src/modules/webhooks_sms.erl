...
handle_sms(Payload, _Props) ->
    HookEvent = kz_api:event_name(Payload),
    ...
    case webhooks_util:find_webhooks(?HOOK_NAME, AccountId) of
        ...
        Hooks ->
            ...
            V2Event = format_v2_event(Payload, HookEvent),
...
format_event(Payload, AccountId) ->
    NumberRegex = kapps_config:get_ne_binary(<<"webhooks">>, <<"sms_number_format_regex">>),
    From = webhooks_util:format_number(kz_im:from(Payload), NumberRegex),
    To = webhooks_util:format_number(kz_im:to(Payload), NumberRegex),
