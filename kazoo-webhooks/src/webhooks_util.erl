...
-export([fire_hooks/2, fire_hooks/3]).
...
-define(PRIVATE_AMQP_FIELDS, [<<"Node">>
                             ,<<"Message-ID">>
                             ,<<"Server-ID">>
                             ,<<"Origin-Cache">>
                             ,<<"App-Name">>
...
-define(SENSITIVE_AMQP_FIELDS, [<<"Password">>
                               ]).

-define(SENSITIVE_EVENT_FIELDS, ?PRIVATE_AMQP_FIELDS ++ ?SENSITIVE_AMQP_FIELDS).
...
to_json(#webhook{}=Hook) ->
    kz_json:from_list(
      ...
      ,{<<"version">>, kz_term:to_binary(Hook#webhook.version)}
      ,{<<"custom_data">>, Hook#webhook.custom_data}
      ,{<<"custom_request_headers">>, Hook#webhook.custom_request_headers}
      ,{<<"modifiers">>, Hook#webhook.modifiers}
      ,{<<"security">>, Hook#webhook.security}
      ]).
...
fire_hooks(JObj, Hooks) ->
    fire_hooks(JObj, JObj, Hooks).

-spec fire_hooks(kz_json:object(), kz_json:object(), webhooks()) -> 'ok'.
fire_hooks(_, _, []) -> 'ok';
fire_hooks(JObj, V2JObj, [#webhook{version = 'v2'} = Hook | Hooks]) ->
    maybe_fire_hook(maybe_augment_event(Hook, V2JObj), Hook),
    fire_hooks(JObj, V2JObj, Hooks);
fire_hooks(JObj, V2JObj, [Hook | Hooks]) ->
    NewJObj = kz_json:set_value(<<"cluster_id">>, kzd_cluster:id(), JObj),
    maybe_fire_hook(NewJObj, Hook),
    fire_hooks(JObj, V2JObj, Hooks).
...
do_fire(#webhook{uri = ?NE_BINARY = URI
    ...
    Headers = ?HTTP_REQ_HEADERS(Hook) ++ custom_request_headers(CustomRequestHeaders),
...
do_fire(#webhook{uri = ?NE_BINARY = URI
    ...
    Body = kz_json:encode(JObj, ['canonical']),
...
handle_resp(#webhook{hook_event = _HookEvent
    ...
    lager:info("non-200 response code from ~s: ~p on account ~s for event ~s(~s) with interaction id ~s"
              ,[_URI, RespCode, _AccountId, _HookEvent, _HookId, _EventId]
              );
handle_resp(#webhook{hook_event = _HookEvent
                    ,hook_id = _HookId
                    } = Hook
    ...
    lager:warning("failed to fire hook event ~s(~s) interaction id: ~p error: ~p"
                 ,[_HookEvent, _HookId, EventId, _E]
                 ),
    _ = failed_hook(Hook, Debug, Resp),
...
retry_hook(#webhook{uri = _URI
    ...
    lager:info("retries exhausted for event ~s(~s) with interaction id ~s for uri (~s)", [_HookEvent, _HookId, _EventId, _URI]);
...
-spec format_number(kz_term:api_ne_binary() | kz_term:ne_binaries(), kz_term:api_ne_binary()) ->
          kz_term:api_ne_binary() | kz_term:ne_binaries().
format_number(Number, 'undefined') -> Number;
format_number('undefined', _Regex) -> 'undefined';
format_number(Numbers, Regex) when is_list(Numbers) ->
    [format_single_number(Number, Regex) || Number <- Numbers];
format_number(Number, Regex) when is_binary(Number) ->
    format_single_number(Number, Regex).

-spec format_single_number(kz_term:ne_binary(), kz_term:ne_binary()) -> kz_term:ne_binary().
format_single_number(Number, Regex) ->
...
jobj_to_rec(Webhook) ->
    #webhook{id = hook_id(Webhook)
            ...
            ,version = kz_term:to_atom(kzd_webhooks:version(Webhook))
            ,custom_data = kzd_webhooks:custom_data(Webhook)
            ,custom_request_headers = kzd_webhooks:custom_request_headers(Webhook)
            ,modifiers = kzd_webhooks:modifiers(Webhook)
            ,format = kzd_webhooks:format(Webhook)
            ,security = kzd_webhooks:security_settings(Webhook)
            }.
...
enable_account_hooks(Account) ->
    ...
    of
        {'ok', []} -> lager:debug("account ~s has no webhooks configured~n", [AccountId]);
        {'ok', Hooks} -> enable_hooks(Hooks);
        {'error', _E} -> lager:debug("failed to load hooks for account ~s: ~p~n", [AccountId, _E])
    end.

-spec enable_hooks(kz_json:objects()) -> 'ok'.
enable_hooks(Hooks) ->
    case hooks_to_reenable(Hooks) of
        [] -> lager:debug("no hooks to re-enable~n", []);
        Reenable ->
            {'ok', Saved} = kz_datamgr:save_docs(?KZ_WEBHOOKS_DB, Reenable),
            _ = webhooks_disabler:flush_hooks(Reenable),
            lager:debug("re-enabled ~p hooks~nIDs: ", [length(Saved)]),
...
enable_descendant_hooks(Account) ->
    ...
    of
        ...
        {'error', _E} ->
            lager:debug("failed to find descendants for account ~s: ~p~n", [AccountId, _E])
    end.
...
maybe_enable_descendant_hooks(Account) ->
    lager:debug("## checking account ~s for hooks to enable ##~n", [Account]),
    enable_account_hooks(Account).
...
maybe_augment_event(#webhook{security='undefined'}, V2JObj) ->
    add_v2_generic(V2JObj);
maybe_augment_event(#webhook{security=Security}, V2JObj) ->
    maybe_add_hmac(add_v2_generic(V2JObj), kz_json:get_ne_binary_value(<<"sha256_key">>, Security)).

add_v2_generic(V2JObj) ->
    kz_json:set_values([{<<"timestamp">>, kz_time:now_s()}
                        %% ,{<<"cluster_id">>, kzd_cluster:id()} %% not including for now
                       ]
                      ,V2JObj
                      ).

maybe_add_hmac(V2JObj, 'undefined') ->
    V2JObj;
maybe_add_hmac(V2JObj, SHA256) ->
    add_hmac(V2JObj, SHA256).

add_hmac(V2JObj, SHA256) ->
    Timestamp = kz_time:now_s(),
    DataJSON = kz_json:encode(kz_json:get_json_value(<<"data">>, V2JObj), ['canonical']),

    MAC = crypto:mac('hmac', 'sha256', SHA256
                    ,<<(kz_term:to_binary(Timestamp))/binary, DataJSON/binary>>
                    ),
    kz_json:set_value(<<"hmac">>, base64:encode(MAC), V2JObj).

-spec sanitize_event(kapi_conf:doc()) -> kz_json:object().
sanitize_event(Event) ->
    kz_json:normalize_jobj(
      kz_api:public_fields(
        kz_json:delete_keys(?SENSITIVE_EVENT_FIELDS, Event)
       )
     ).

-spec custom_request_headers(kz_term:api_object()) -> kz_term:proplist().
custom_request_headers('undefined') -> [];
custom_request_headers(Headers) -> kz_json:to_proplist(Headers).
