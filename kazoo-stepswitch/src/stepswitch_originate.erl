...
handle_event(JObj, #state{request_handler=RequestHandler
    ...
    case kz_api:event_type(JObj) of
        ...
        {<<"resource">>, <<"originate_started">>} ->
            gen_listener:cast(RequestHandler, {'originate_result', originate_started(JObj, OffnetReq)});
...
build_originate(#state{endpoints=Endpoints
    ...
    CCVUpdates = props:filter_undefined(
                   ...
                   ,{fun maybe_add_authorizing_id/2, AccountId}
                   ]),
    ...
    FailOnSingleReject = kapi_offnet_resource:fail_on_single_reject(OffnetReq, ?DEFAULT_FAIL_ON_SINGLE_REJECT(AccountId)),
    ...
    props:filter_undefined(
      ...
      ,{<<"Switch-Nodename">>, kz_json:get_value(<<"Switch-Nodename">>, OffnetReq)}
...
maybe_add_authorizing_id(JObj, AccountId) ->
    ...
    AuthorizingType = kz_json:get_ne_binary_value(<<"Authorizing-Type">>, JObj),
    maybe_add_authorizing_id(JObj, AccountId, AuthorizingId, AuthorizingType).

-spec maybe_add_authorizing_id(kz_json:object(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) -> kz_json:object().
maybe_add_authorizing_id(JObj, AccountId, 'undefined', 'undefined') ->
    KVs = [{<<"Authorizing-ID">>, AccountId}
          ,{<<"Authorizing-Type">>, <<"account">>}
          ],
    kz_json:set_values(KVs, JObj);
maybe_add_authorizing_id(JObj, AccountId, AccountId, 'undefined') ->
    kz_json:set_value(<<"Authorizing-Type">>, <<"account">>, JObj);
maybe_add_authorizing_id(JObj, AccountId, 'undefined', <<"account">>) ->
    kz_json:set_value(<<"Authorizing-ID">>, AccountId, JObj);
maybe_add_authorizing_id(JObj, _AccountId, _AuthorizingId, _AuthorizingType) ->
    JObj.
...
originate_success(JObj, OffnetReq) ->
    ...
    ResourceResponse =
        kz_json:set_value(<<"Call-ID">>, kz_json:get_value(<<"Call-ID">>, JObj), kz_api:remove_defaults(JObj)),
    [{<<"Msg-ID">>, kz_api:msg_id(OffnetReq)}
    ,{<<"Call-ID">>, kz_json:get_first_defined([<<"Origination-Call-ID">>, <<"Outbound-Call-ID">>], OffnetReq)}
    ,{<<"Call">>, kapps_call:to_json(kapps_call:from_originate_resp(JObj))}
...
-spec originate_started(kz_json:object(), kz_json:object()) -> kz_term:proplist().
originate_started(JObj, OffnetReq) ->
    lager:debug("originate started to execute"),
