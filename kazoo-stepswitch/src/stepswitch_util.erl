...
%% @end
%%------------------------------------------------------------------------------
-spec convert_to_e164_format(kz_term:api_binary(), Default) -> kz_term:ne_binary() | Default.
convert_to_e164_format('undefined', Default) -> Default;
...
correct_shortdial(Number, _CIDNum, _DeniedCallRestrictions, {'false', Length}) ->
    lager:debug("~s's length (~p) doesn't match min_shortdial_destination param's value (~p), skipping"
...
maybe_constrain_shortdial_correction(Length) ->
    Upperbound = kapps_config:get_integer(?SS_CONFIG_CAT
                                         ,<<"max_shortdial_correction">>
                                         ,5
                                         ),
    Lowerbound = kapps_config:get_integer(?SS_CONFIG_CAT
                                         ,<<"min_shortdial_correction">>
                                         ,2
                                         ),
    maybe_constrain_shortdial_correction(Length, Lowerbound, Upperbound).

-spec maybe_constrain_shortdial_correction(integer(), integer(), integer()) -> integer().
maybe_constrain_shortdial_correction(Length, Lowerbound, Upperbound)
  when Length >= Lowerbound
       andalso Length =< Upperbound ->
    Length;
maybe_constrain_shortdial_correction(_Length, _Low, _Up) ->
    0.
...
-spec local_sip_headers(kz_json:object()) -> kz_json:object().
local_sip_headers(Endpoint) ->
    case kz_json:get_value(<<"Custom-SIP-Headers">>, Endpoint) of
...
maybe_add_sip_headers(Endpoint, SIPHeaders) ->
    LocalSIPHeaders = local_sip_headers(Endpoint),
    MergedHeaders = kz_json:merge_jobjs(SIPHeaders, LocalSIPHeaders),

    case kz_term:is_empty(MergedHeaders) of
        true -> Endpoint;
...
maybe_resource_to_endpoints(Resource
    ...
    Updates = [{<<"Name">>, Name}
              ,{<<"Weight">>, Weight}
              ,{<<"Supports-Pidflo">>, stepswitch_resources:get_resrc_supports_pidflo(Resource)}
              ],
