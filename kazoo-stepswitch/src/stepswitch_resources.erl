...
-export([get_resrc_id/1
        ...
        ,get_resrc_supports_pidflo/1
        ,get_resrc_classifier_include_pidflo/1
        ]).
...
-export([set_resrc_id/2
        ...
        ,set_resrc_supports_pidflo/2
        ,set_resrc_classifier_include_pidflo/2
        ]).
...
-define(DIRECT_RESELLER_ID_EXPANDABLE_MACRO, <<"{direct_reseller_id}">>).
...
-record(gateway, {server :: kz_term:api_binary()
                 ...
                 ,format_request_uri = 'false' :: boolean()
                 ,request_realm = 'undefined' :: kz_term:api_binary()
                 }).
...
-record(resrc, {id :: kz_term:api_binary()
               ...
               ,classifier_include_pidflo = 'false' :: boolean()
               ,gateway_strategy = 'sequential' :: gateway_strategy()
               ,ccvs :: kz_term:api_object()
               ,sip_headers :: kz_term:api_object()
               ,sip_interface :: kz_term:api_binary()
               ,supports_pidflo = 'false' :: boolean()
               }).
...
resource_to_props(#resrc{}=Resource) ->
    props:filter_undefined(
      ...
      ,{<<"Supports-Pidflo">>, Resource#resrc.supports_pidflo}
      ]).
...
-spec maybe_match_test_number(rules() , kz_term:ne_binary()) -> boolean().
...
get_local_endpoints(AccountId, Number, OffnetJObj) ->
    ...
    Resources = filter_resources(OffnetJObj, get(AccountId)),
...
get_global_endpoints(Number, OffnetJObj) ->
    ...
    Resources = filter_resources(OffnetJObj, get()),
    resources_to_endpoints(Resources, Number, OffnetJObj).
...
%% @end
%%------------------------------------------------------------------------------
-spec filter_resources(kapi_offnet_resource:req(), resources()) ->
          resources().
filter_resources(OffnetJObj, Resources) ->
    Flags = kapi_offnet_resource:flags(OffnetJObj, []),
    case direct_reseller_id(OffnetJObj) of
        'undefined' ->
            filter_resources_by_flags(Flags, Resources);
        DirectResellerId ->
            %% direct reseller:
            %% if account is reseller get from the account itself
            %% if not get from its reseller
            ShouldForceFlags = kapps_account_config:get(DirectResellerId, ?SS_CONFIG_CAT, <<"force_reseller_outbound_flags">>, 'false'),
            case kz_term:is_true(ShouldForceFlags) of
                'true' ->
                    ResellerFlags = kapps_account_config:get_ne_binaries(DirectResellerId, ?SS_CONFIG_CAT, <<"reseller_outbound_flags">>, []),
                    lager:info("force appending flags from reseller: ~s", [kz_binary:join(ResellerFlags)]),
                    filter_resources_by_flags(lists:uniq(ResellerFlags ++ Flags), Resources);
                'false' ->
                    filter_resources_by_flags(Flags, Resources)
            end
    end.

-spec direct_reseller_id(kapi_offnet_resource:req()) -> kz_term:api_ne_binary().
direct_reseller_id(OffnetJObj) ->
    AccountId = kapi_offnet_resource:account_id(OffnetJObj),
...
filter_resources_by_flags([], Resources) ->
...
-spec filter_resources_by_flags(kz_term:ne_binaries(), resources(), resources()) -> resources().
...
maybe_resource_to_endpoints(#resrc{id=Id
                                  ...
                                  ,proxies=Proxies
                                  ,classifier=Classifier
                                  ,classifier_include_pidflo=IncludePidflo
                                  } = Resource
                           ,Number
                           ,OffnetJObj
                           ,Endpoints
    ...
    case filter_resource_by_rules(Id, Number, Rules, CallerIdNumber, CallerIdRules) of
        ...
        {'ok', NumberMatch} ->
            ...
            CCVUpdates = props:filter_empty(
                           ...
                           ,{<<"Include-Pidflo">>, IncludePidflo}
                           ]),
            Updates = [{<<"Name">>, Name}
                      ,{<<"Weight">>, Weight}
                      ,{<<"Supports-Pidflo">>, get_resrc_supports_pidflo(Resource)}
                      ],
...
-spec check_diversion_fields(kapi_offnet_resource:req()) -> kz_term:api_binary().
...
-spec filter_resource_by_rules(kz_term:ne_binary(), kz_term:ne_binary(), rules(), kz_term:ne_binary(), rules()) ->
...
-spec filter_resource_by_match(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), rules(), kz_term:ne_binary()) ->
          {'ok', kz_term:ne_binary()} |
          {'error', 'no_match'}.
filter_resource_by_match(Id, Number, CallerIdNumber, CallerIdRules, Match) ->
...
-spec evaluate_rules(rules(), kz_term:ne_binary()) ->
          {'ok', kz_term:ne_binary()} |
          {'error', 'no_match'}.
...
-spec evaluate_cid_rules(rules(), kz_term:ne_binary()) ->
          {'ok', kz_term:ne_binary()} |
...
gateway_to_endpoint(DestinationNumber
    ...
    AccountId = kapi_offnet_resource:account_id(OffnetJObj),
    NewSipHeaders = maybe_expand_sip_header_macro_logic(AccountId, SipHeaders),

    kz_json:from_list(
      props:filter_empty(
        [{<<"Route">>, gateway_dialstring(Gateway, DestinationNumber)}
        ,{<<"Outbound-Callee-ID-Name">>, kapi_offnet_resource:outbound_callee_id_name(OffnetJObj, kz_term:to_binary(DestinationNumber))}
        ,{<<"Outbound-Callee-ID-Number">>, kapi_offnet_resource:outbound_callee_id_number(OffnetJObj, kz_term:to_binary(DestinationNumber))}
        ...
        ,{<<"Custom-SIP-Headers">>, NewSipHeaders}
...
-spec maybe_expand_sip_header_macro_logic(kz_term:ne_binary(), kz_json:object()) -> kz_json:object().
maybe_expand_sip_header_macro_logic(AccountId, SipHeaders) ->
    Routines = [{fun expand_direct_reseller_id/2, AccountId}],
    kz_json:exec(Routines, SipHeaders).

-spec expand_direct_reseller_id(kz_term:ne_binary(), kz_json:object()) -> kz_json:object().
expand_direct_reseller_id(AccountId, SIPHeaders) ->
    DirectResellerId = case kz_services_reseller:is_reseller(AccountId) of
                           'true' -> AccountId;
                           'false' -> kz_services_reseller:get_id(AccountId)
                       end,
    replace_direct_reseller_macro(SIPHeaders, DirectResellerId).

-spec replace_direct_reseller_macro(kz_json:object(), kz_term:api_binary()) -> kz_json:object().
replace_direct_reseller_macro(SIPHeaders, DirectResellerId) ->
    {_, UpdatedSIPHeaders} = lists:foldl(fun direct_reseller_id_macro_kv/2
                                        ,{DirectResellerId, []}
                                        ,kz_json:to_proplist(SIPHeaders)
                                        ),
    kz_json:from_list(UpdatedSIPHeaders).

-spec direct_reseller_id_macro_kv(tuple(), {kz_term:api_binary(), kz_term:proplist()}) ->
          {kz_term:api_binary(), kz_term:proplist()}.
direct_reseller_id_macro_kv({_FieldName, ?DIRECT_RESELLER_ID_EXPANDABLE_MACRO}
                           ,{'undefined', Acc}
                           ) ->
    {'undefined', Acc};
direct_reseller_id_macro_kv({FieldName, ?DIRECT_RESELLER_ID_EXPANDABLE_MACRO}
                           ,{DirectResellerId, Acc}
                           ) ->
    {DirectResellerId, [{FieldName, DirectResellerId} | Acc]};
direct_reseller_id_macro_kv(SIPHeader, {DirectResellerId, Acc}) ->
    {DirectResellerId, [SIPHeader|Acc]}.
...
create_classifier_resource(ResourceJObj, ClassifierJObj, Classifier, ConfigClassifier) ->
    DefaultRegex = kz_json:get_ne_binary_value(<<"regex">>, ConfigClassifier),
    ...
    Props =
        props:filter_undefined(
          ...
          ,{<<"rules">>, [kz_json:get_ne_binary_value(<<"regex">>, ClassifierJObj, DefaultRegex)]}
          ,{<<"weight_cost">>, kz_json:get_integer_value(<<"weight_cost">>, ClassifierJObj)}
          ...
          ,{<<"classifier_include_pidflo">>, kz_json:is_true(<<"include_pidflo">>, ClassifierJObj)}
          ]
         ),
...
create_classifier_gateways(ResourceJObj, ClassifierJObj) ->
    Props =
        props:filter_undefined(
          [{<<"suffix">>, kz_json:get_ne_binary_value(<<"suffix">>, ClassifierJObj)}
          ,{<<"prefix">>, kz_json:get_ne_binary_value(<<"prefix">>, ClassifierJObj)}
          ]
         ),
    Gateways =
...
resource_from_jobj(JObj) ->
    Resource = #resrc{id=kz_doc:id(JObj)
                     ...
                     ,classifier_include_pidflo=kz_json:is_true(<<"classifier_include_pidflo">>, JObj)
                     ,gateway_strategy=kz_term:to_atom(kzd_resources:gateway_strategy(JObj, <<"sequential">>), 'true')
                     ,ccvs=maybe_authz_number_module(JObj)
                     ,sip_headers = kzd_resources:cshs(JObj, kz_json:new())
                     ,sip_interface = kzd_resources:custom_sip_interface(JObj)
                     ,supports_pidflo = kzd_resources:supports_pidflo(JObj)
                     },
    Gateways = gateways_from_jobjs(kzd_resources:gateways(JObj, []), Resource),
    Resource#resrc{gateways=Gateways}.

-spec maybe_authz_number_module(kzd_resources:doc()) -> kz_json:object().
maybe_authz_number_module(JObj) ->
    CCVs = kzd_resources:ccvs(JObj, kz_json:new()),
    case kz_json:get_ne_binary_value(<<"pvt_number_module">>, JObj) of
        'undefined' ->  kz_json:delete_key(<<"Authz-Number-Module">>, CCVs);
        AuthzMod -> kz_json:set_value(<<"Authz-Number-Module">>, AuthzMod, CCVs)
    end.
...
resource_codecs(JObj) ->
    ...
    case kz_codecs:audio_codecs(JObj, DefaultAudio)
        ++ kz_codecs:video_codecs(JObj, DefaultVideo)
    of
...
resource_rules_test(JObj) ->
    Rules = kz_json:get_list_value(<<"rules_test">>, JObj, []),
...
resource_grace_period(JObj) when not is_integer(JObj) ->
    kz_math:clamp(100, 0, kzd_resources:grace_period(JObj, ?DEFAULT_WEIGHT)).

-spec resource_weight(kzd_resources:doc() | integer()) -> 1..100.
resource_weight(JObj) when not is_integer(JObj) ->
    kz_math:clamp(100, 1, kzd_resources:weight_cost(JObj, ?DEFAULT_WEIGHT)).
...
gateway_from_jobj(GatewayJObj, #resrc{is_emergency=IsEmergency
    ...
    #gateway{endpoint_type = EndpointType
            ...
            ,realm = kz_json:get_ne_binary_value(<<"realm">>, GatewayJObj)
            ,username = kz_json:get_ne_binary_value(<<"username">>, GatewayJObj)
            ,password = kz_json:get_ne_binary_value(<<"password">>, GatewayJObj)
            ,ccvs = gateway_ccvs(GatewayJObj, CCVs)
            ,sip_headers = gateway_sip_headers(GatewayJObj, CustomSIPHeaders)
            ,sip_interface = kzd_resources:custom_sip_interface(GatewayJObj, SIPInterface)
            ,invite_format = kz_json:get_ne_binary_value(<<"invite_format">>, GatewayJObj, <<"route">>)
            ...
            ,codecs = kz_json:get_list_value(<<"codecs">>, GatewayJObj, Codecs)
            ...
            ,format_request_uri = kz_json:is_true(<<"format_request_uri">>, GatewayJObj)
            ,request_realm = kz_json:get_ne_value(<<"request_realm">>, GatewayJObj)
            }.

-spec gateway_ccvs(kz_json:object(), kz_json:object()) -> kz_json:object().
gateway_ccvs(GatewayJObj, ResourceCCVs) ->
    kz_custom_channel_vars:outbound(kz_json:merge(ResourceCCVs, kzd_resources:ccvs(GatewayJObj, kz_json:new())), kz_json:new()).

-spec gateway_sip_headers(kz_json:object(), kz_json:object()) -> kz_json:object().
gateway_sip_headers(GatewayJObj, ResourceSIPHeaders) ->
    kz_custom_sip_headers:outbound(kz_json:merge(ResourceSIPHeaders, kzd_resources:cshs(GatewayJObj, kz_json:new())), kz_json:new()).
...
endpoint_options(JObj, <<"freetdm">>) ->
    kz_json:from_list(
      [{<<"Span">>, kz_json:get_ne_binary_value(<<"span">>, JObj)}
      ,{<<"Channel-Selection">>, kz_json:get_ne_binary_value(<<"channel_selection">>, JObj, <<"ascending">>)}
      ]);
endpoint_options(JObj, <<"skype">>) ->
    kz_json:from_list(
      [{<<"Skype-Interface">>, kz_json:get_ne_binary_value(<<"interface">>, JObj)}
      ,{<<"Skype-RR">>, kz_json:is_true(<<"skype_rr">>, JObj, 'true')}
      ]);
endpoint_options(JObj, <<"sip">>) ->
    kz_json:from_list(
      [{<<"Route-ID">>, kz_json:get_ne_binary_value(<<"route_id">>, JObj)}]);
...
gateway_dialstring(#gateway{route='undefined'
                           ...
                           ,server=Server
                           ,port=Port
                           ,format_request_uri=FmtReqUri
                           ,request_realm=ReqRelm
                           }
                  ,Number
                  ) ->
    DialStringPort =
    ...
    case FmtReqUri =:= 'true'
        andalso ReqRelm =/= 'undefined'
    of
        'true' ->
            Route = list_to_binary(["sip:", Prefix, Number, Suffix, "@", ReqRelm, DialStringPort]);
        'false' ->
            Route = list_to_binary(["sip:", Prefix, Number, Suffix, "@", Server, DialStringPort])
    end,
...
-spec get_resrc_supports_pidflo(resource()) -> boolean().
get_resrc_supports_pidflo(#resrc{supports_pidflo=Supported}) -> Supported.

-spec get_resrc_classifier_include_pidflo(resource()) -> boolean().
get_resrc_classifier_include_pidflo(#resrc{classifier_include_pidflo=Included}) -> Included.
...
-spec set_resrc_supports_pidflo(resource(), boolean()) -> resource().
set_resrc_supports_pidflo(Resource, Supported) -> Resource#resrc{supports_pidflo=Supported}.

-spec set_resrc_classifier_include_pidflo(resource(), boolean()) -> resource().
set_resrc_classifier_include_pidflo(Resource, Included) -> Resource#resrc{classifier_include_pidflo=Included}.
