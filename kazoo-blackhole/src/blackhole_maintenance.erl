...
%%------------------------------------------------------------------------------
%% @doc Publish a request to start a module.
%% @end
%%------------------------------------------------------------------------------
-spec start_module(module() | kz_term:ne_binary()) -> 'ok'.
start_module(Module) ->
    start_module(Module, 'true').

-spec start_module(module() | kz_term:ne_binary(), kz_term:text() | boolean()) -> 'ok'.
start_module(Module, Persist) ->
    ModuleBin = kz_term:to_binary(Module),
...
%%------------------------------------------------------------------------------
%% @doc Publish a request to stop a module.
%% @end
%%------------------------------------------------------------------------------
-spec stop_module(module() | kz_term:ne_binary()) -> 'ok'.
stop_module(Module) ->
    stop_module(Module, 'true').

-spec stop_module(module() | kz_term:ne_binary(), kz_term:text() | boolean()) -> 'ok'.
stop_module(Module, Persist) ->
    ModuleBin = kz_term:to_binary(Module),
    Req = [{<<"Module">>, ModuleBin}
          ,{<<"Action">>, <<"stop">>}
          ,{<<"Persist">>, kz_term:to_boolean(Persist)}
...
active_sessions_by_ip(<<IPAddr/binary>>) ->
    Contexts = blackhole_tracking:get_contexts_by_ip(IPAddr),
    print_sessions(Contexts).

-spec active_sessions_by_account(kz_term:ne_binary()) -> 'ok'.
active_sessions_by_account(<<AccountId/binary>>) ->
    Contexts = blackhole_tracking:get_contexts_by_account_id(AccountId),
    print_sessions(Contexts).

-spec print_sessions([bh_context:context()] | tuple()) -> 'ok'.
...
print_sessions(Sessions) ->
    ...
    io:format("~22s | ~12s | ~12s | ~32s | ~s~n"
             ,[<<"Peer">>, <<"Uptime">>, <<"PID">>, <<"Account-ID">>, <<"Bindings">>]
             ),
...
print_session(Context) ->
    io:format("~22s | ~12s | ~12s | ~32s | ~s~n"
             ,[bh_context:websocket_session_id(Context)
              ...
              ,kz_binary:join(bh_context:client_bindings(Context))
              ]).
